...
import { useCreateShard, useKeyspace, useRebuildKeyspaceGraph, useReloadSchema, useRemoveKeyspaceCell } from '../../../hooks/api';
...
export const Advanced: React.FC<Props> = ({ clusterID, name }) => {
  ...
  const createShardMutation = useCreateShard(
    ...
    {
      onError: (error) =>
        warn(`There was an error creating shard ${shardName} in keyspace ${name}: ${error}`),
      onSuccess: () => {
