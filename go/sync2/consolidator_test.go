...
func TestConsolidator(t *testing.T) {
	...
	require.Same(t, dup.Result(), orig.Result(), "failed to share the result")

	want = []ConsolidatorCacheItem{{Query: sql, Count: 1}}
	require.Equalf(t, want, con.Items(), "expected consolidator to have one items %v", con.Items())
	...
	require.Equalf(t, want, con.Items(), "expected consolidator to have two items %v", con.Items())
}
