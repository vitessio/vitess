...
func TestBatcher(t *testing.T) {
	...
	startWaiter := func(testcase string, want int) {
		go func() {
			id := b.Wait()
			assert.Equalf(t, want, id, "%s: got %d, want %d", testcase, id, want)
	...
	awaitVal := func(name string, val *atomic.Int32, expected int32) {
		for count := 0; val.Load() != expected; count++ {
			...
			if count > 5 {
				assert.Failf(t, "timed out", "Timed out waiting for %s to be %v", name, expected)
				return
			}
		}
	}
	...
	awaitBatch := func(name string, n int32) {
		...
		assert.EqualValues(t, 0, waitersFinished.Load(), "Waiters finished before being released")
		releaseBatch()
