...
func TestJSON(t *testing.T) {
	testcases := []struct {
		data      []byte
		expected  string
		expectTyp querypb.Type
	}{{
		data:      []byte{},
		expected:  `'null'`,
		expectTyp: sqltypes.TypeJSON,
	}, {
	...
	for _, tcase := range testcases {
		t.Run(tcase.expected, func(t *testing.T) {
			r, typ, err := printJSONData(tcase.data)
			if err != nil {
				...
				if typ != tcase.expectTyp {
					t.Errorf("unexpected type for %v: got %v expected %v", tcase.data, typ, tcase.expectTyp)
				}
			}
