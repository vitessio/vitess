...
// incoming packets.
func (c *Conn) handleNextCommand(handler Handler) (kontinue bool) {
	c.sequence = 0
	...
	case ComInitDB:
		...
		c.recycleReadPacket()
		res := c.execBufferedQuery(handler, "use "+sqlescape.EscapeID(db), false)
		return res != connErr
	case ComQuery:
		queryStart := time.Now()
	...
	case ComSetOption:
		return c.setOption(data)
	case ComPrepare:
		return c.comPrepare(handler, data)
	case ComStmtExecute:
		return c.stmtExecute(handler, data)
	case ComStmtSendLongData:
		return c.sendLongData(data)
	case ComStmtClose:
...
func (c *Conn) stmtReset(data []byte) bool {
...
func (c *Conn) sendLongData(data []byte) bool {
...
func (c *Conn) setOption(data []byte) bool {
...
func (c *Conn) comPrepare(handler Handler, data []byte) bool {
	...
	if err := c.writePrepare(fld, c.PrepareData[c.StatementID]); err != nil {
		log.Error("Error writing prepare data to connection: %v", c.ConnectionID, err)
		return false
	}
	return false
}

func (c *Conn) stmtExecute(handler Handler, data []byte) bool {
	...
	// If the callback was never called, we should have an error
	if !callbackCalled {
...
func (c *Conn) execQuery(query string, handler Handler, more bool) execResult {
	...
	// If the callback was never called, we expect an error.
	if !callbackCalled {
		// This is just a failsafe. Should never happen.
		if err == nil || err == io.EOF {
			err = NewSQLErrorFromError(errors.New("unexpected: query ended without no results and no error"))
