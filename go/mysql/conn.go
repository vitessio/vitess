...
func (c *Conn) handleComStmtFetch(handler Handler, data []byte) (kontinue bool) {
	...
	if !ok || cs == nil {
		log.Errorf("Requested stmtID does not have an open cursor. Client %v, returning error: %v", c.ConnectionID, data)
		if !c.writeErrorAndLog(sqlerror.ERUnknownComError, sqlerror.SSNetError, "error handling packet: %v", data) {
			return false
		}
		return c.endWriterBuffering() == nil
	...
	for cs != nil && numRows != 0 {
		toSend := uint32(len(cs.pending.Rows))
		if toSend > numRows {
			toSend = numRows
		}
	...
	if err := c.endWriterBuffering(); err != nil {
		log.Errorf("Conn %v: endWriterBuffering() failed: %v", c.ID(), err)
		return false
	}
	return true
}
...
func (c *Conn) handleComStmtExecute(handler Handler, data []byte) (kontinue bool) {
	...
	if !ok {
		...
		if !c.writeErrorAndLog(sqlerror.ERUnknownError, sqlerror.SSUnknownSQLState, "error handling packet: %v", "missing result set") {
			log.Errorf("Error writing query error to %s", c)
			return false
		}
		return false
	}
