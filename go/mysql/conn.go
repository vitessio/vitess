...
func newServerConn(conn net.Conn, listener *Listener) *Conn {
	...
	if tcpConn, ok := conn.(*net.TCPConn); ok {
		if err := setTcpConnProperties(tcpConn, listener.connKeepAlivePeriod); err != nil {
			log.ErrorS(fmt.Sprintf("error in setting tcp properties: %v", err))
		} else {
...
func (c *Conn) writeErrorAndLog(errorCode sqlerror.ErrorCode, sqlState string, format string, args ...any) bool {
	if err := c.writeErrorPacket(errorCode, sqlState, format, args...); err != nil {
		log.ErrorS(fmt.Sprintf("Error writing error to %s: %v", c, err))
		return false
	}
	return true
}
...
func (c *Conn) writeErrorPacketFromErrorAndLog(err error) bool {
	...
	if werr != nil {
		log.ErrorS(fmt.Sprintf("Error writing error to %s: %v", c, werr))
		return false
	}
	return true
}
...
func (c *Conn) handleNextCommand(handler Handler) bool {
	...
	if err != nil {
		...
		if err != io.EOF && !strings.Contains(err.Error(), "use of closed network connection") {
			log.ErrorS(fmt.Sprintf("Error reading packet from %s: %v", c, err))
		}
		return false
	}
	if len(data) == 0 {
	...
	default:
		log.ErrorS(fmt.Sprintf("Got unhandled packet (default) from %s, returning error: %v", c, data))
		c.recycleReadPacket()
		if !c.writeErrorAndLog(sqlerror.ERUnknownComError, sqlerror.SSNetError, "command handling not implemented yet: %v", data[0]) {
			return false
		}
...
func (c *Conn) handleComRegisterReplica(handler Handler, data []byte) (kontinue bool) {
	...
	if err != nil {
		log.ErrorS(fmt.Sprintf("conn %v: parseComRegisterReplica failed: %v", c.ID(), err))
		return false
	}
...
func (c *Conn) handleComBinlogDump(handler Handler, data []byte) (kontinue bool) {
	...
	defer func() {
		if err := c.endWriterBuffering(); err != nil {
			log.ErrorS(fmt.Sprintf("conn %v: flush() failed: %v", c.ID(), err))
			kontinue = false
		}
	}()
	...
	if err != nil {
		log.ErrorS(fmt.Sprintf("conn %v: parseComBinlogDumpGTID failed: %v", c.ID(), err))
		return false
	}
...
func (c *Conn) handleComStmtReset(data []byte) bool {
	...
	c.recycleReadPacket()
	if !ok {
		log.ErrorS(fmt.Sprintf("Got unhandled packet from client %v, returning error: %v", c.ConnectionID, data))
	...
	if !ok {
		log.ErrorS(fmt.Sprintf("Commands were executed in an improper order from client %v, packet: %v", c.ConnectionID, data))
	...
	if err := c.writeOKPacket(&PacketOK{statusFlags: c.StatusFlags}); err != nil {
		log.ErrorS(fmt.Sprintf("Error writing ComStmtReset OK packet to client %v: %v", c.ConnectionID, err))
		return false
	}
	return true
}
...
func (c *Conn) handleComStmtExecute(handler Handler, data []byte) (kontinue bool) {
	...
	if !receivedResult {
		...
		if err != nil {
			...
			log.ErrorS(fmt.Sprintf("Error in the middle of a stream to %s: %v", c, err))
			return false
		}
		...
		if !sendFinished {
			if err := c.writeEndResult(false, 0, 0, handler.WarningCount(c)); err != nil {
				log.ErrorS(fmt.Sprintf("Error writing result to %s: %v", c, err))
				return false
			}
		}
	}
...
func (c *Conn) handleComPrepare(handler Handler, data []byte) (kontinue bool) {
	...
	if c.Capabilities&CapabilityClientMultiStatements != 0 {
		...
		if err != nil {
			log.ErrorS(fmt.Sprintf("Conn %v: Error splitting query: %v", c, err))
			return c.writeErrorPacketFromErrorAndLog(err)
		}
		if len(queries) != 1 {
			log.ErrorS(fmt.Sprintf("Conn %v: can not prepare multiple statements", c))
			return c.writeErrorPacketFromErrorAndLog(err)
		}
		query = queries[0]
	}
	...
	if err := c.writePrepare(fld, prepare); err != nil {
		log.ErrorS(fmt.Sprintf("Error writing prepare data to client %v: %v", c.ConnectionID, err))
		return false
	}
	return true
}
...
func (c *Conn) handleComSetOption(data []byte) bool {
	...
	if ok {
		...
		case 1:
			c.Capabilities &^= CapabilityClientMultiStatements
		default:
			log.ErrorS(fmt.Sprintf("Got unhandled packet (ComSetOption default) from client %v, returning error: %v", c.ConnectionID, data))
			if !c.writeErrorAndLog(sqlerror.ERUnknownComError, sqlerror.SSNetError, "error handling packet: %v", data) {
				return false
			}
		}
		if err := c.writeEndResult(false, 0, 0, 0); err != nil {
			log.ErrorS(fmt.Sprintf("Error writeEndResult error %v ", err))
			return false
		}
	} else {
		log.ErrorS(fmt.Sprintf("Got unhandled packet (ComSetOption else) from client %v, returning error: %v", c.ConnectionID, data))
		if !c.writeErrorAndLog(sqlerror.ERUnknownComError, sqlerror.SSNetError, "error handling packet: %v", data) {
			return false
		}
	}
...
func (c *Conn) handleComPing() bool {
	...
	if c.listener.shutdown.Load() {
		...
		if err := c.writeOKPacket(&PacketOK{statusFlags: c.StatusFlags}); err != nil {
			log.ErrorS(fmt.Sprintf("Error writing ComPing result to %s: %v", c, err))
			return false
		}
	}
	return true
