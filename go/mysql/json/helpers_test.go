...
// and the two must sort every spelling into the same equality classes. Every
// value must fingerprint one way and every fingerprint must belong to one
// value, which together is pairwise agreement without the pairwise work.
func TestNumberHashMatchesDecimalComparison(t *testing.T) {
	var (
		// Decimal.String is canonical, so it names a value's equality class.
		hashByValue     = map[string]vthash.Hash{}
		spellingByValue = map[string]string{}
		valueByHash     = map[vthash.Hash]string{}
		compared        int
		skipped         int
	)
	for _, spelling := range numberSpellings() {
		...
		compared++
		class := value.String()
		hash := hashNumberText(t, spelling)

		if first, seen := hashByValue[class]; seen {
			require.Equalf(t, first, hash,
				"%q and %q are both %s but fingerprint apart", spellingByValue[class], spelling, class)
		} else {
			hashByValue[class] = hash
			spellingByValue[class] = spelling
		}
		if other, seen := valueByHash[hash]; seen {
			require.Equalf(t, other, class,
				"%q (%s) and %q (%s) fingerprint alike", spellingByValue[other], other, spelling, class)
		} else {
			valueByHash[hash] = class
		}
	}
	t.Logf("compared %d spellings, %d had no decimal value", compared, skipped)
}
...
// TestNumberHashSpellings checks that a parsed number fingerprints its value
// and not the way it was written, through the path a document takes (the
// parser, then hashNumber) rather than through hashDecimalText alone, which
// TestNumberHashMatchesDecimalComparison covers.
func TestNumberHashSpellings(t *testing.T) {
	...
	groups := [][]string{
		...
		// value: the first spelling is an integer, the next three are one
		// double with that integer's value, and 9007199254740993 stayed an
		// integer that no double can hold.
...
// TestNumericValueNonFinite pins that a double with no value reports itself
// out of range rather than panicking, and still fingerprints, apart from any
// number that has a value.
func TestNumericValueNonFinite(t *testing.T) {
	for _, text := range []string{"NaN", "+Inf", "-Inf"} {
		v := NewNumber(text, NumberTypeFloat)
		_, ok := v.NumericValue()
		require.False(t, ok, text)
		h := vthash.New()
		require.NotPanics(t, func() { v.Hash(&h) }, text)
		one := vthash.New()
		NewNumber("1", NumberTypeSigned).Hash(&one)
		require.NotEqual(t, one.Sum128(), h.Sum128(), text)
	}
}
