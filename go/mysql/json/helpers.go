...
// disagree by mistake.
//
// Recursion runs to the document's depth, as comparison's does: parsing caps
// it at MaxDepth, and a document built from an expression is as deep as the
// expression.
...
// unframed, `["", "X\x02\x00Y"]` and `["\x02\x00X", "Y"]` produce the same
// byte stream, because a payload can spell out the tags around it.
...
// read: an integer that fits stays exact, a SQL DECIMAL keeps its digits, and
// everything else becomes a double, losing precision there and then. Both
// comparison and hashing work from whatever was kept, which is why
// 9007199254740992.1 equals 9007199254740992 (the fraction went when it became
// a double, and that double's value is the integer) while 9007199254740993
// does not (it stayed an integer that no double can hold). Reading the decimal
// straight off the text would keep a precision MySQL has already discarded.
func (v *Value) NumericValue() (decimal.Decimal, bool) {
	...
	case NumberTypeFloat:
		...
		if !ok || math.IsNaN(f) || math.IsInf(f, 0) {
			// No value to compare by: report it out of range rather than
			// let decimal.NewFromFloat panic on it.
			return decimal.Zero, false
		}
		return decimal.NewFromFloat(f), true
	default:
		// A decimal, carried over from a SQL value or printed from a stored
		// document, keeps the digits it was written with, so it stands for
		// itself.
		return v.Decimal()
	}
}
...
func (v *Value) hashNumber(h *vthash.Hasher) {
	...
	// Text with no finite value to read. Numbers parsed from a document and
	// SQL numbers always have one, so this only keeps Hash total for a value
	// built by hand; the marker keeps such text clear of any number that does
	// have a value.
