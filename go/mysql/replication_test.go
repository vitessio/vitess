...
func TestWritePacketChunks(t *testing.T) {
	_ = utils.LeakCheckContext(t)
	listener, sConn, cConn := createSocketPair(t)
	defer func() {
		listener.Close()
	...
	t.Run("Single chunk", func(t *testing.T) {
		...
		err := cConn.WritePacketChunks([][]byte{payload})
		require.NoError(t, err)
	...
	t.Run("Multiple chunks", func(t *testing.T) {
		...
		err := cConn.WritePacketChunks(chunks)
		require.NoError(t, err)
	...
	t.Run("Interleaved with WritePacketDirect", func(t *testing.T) {
		...
		// Write packet 1 via WritePacketChunks
		err = cConn.WritePacketChunks([][]byte{{0xBB}, {0xCC}})
		require.NoError(t, err)
