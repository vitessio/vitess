...
func NewCollation(name string, weights WeightTable, weightPatches []WeightPatch, reorder []Reorder, contractions []Contraction, upperCaseFirst bool, levels int) *Collation900 {
	coll := &Collation900{
		table:        applyTailoring(weights, weightPatches, false),
...
func NewCollationLegacy(charset charset.UnicodeCharset, weights WeightTable, weightPatches []WeightPatch, contractions []Contraction, maxCodepoint rune) *CollationLegacy {
	coll := &CollationLegacy{
		next:         charset.Iterator(),
		table:        applyTailoring(weights, weightPatches, true),
