...
func doTestMultiResult(t *testing.T, disableClientDeprecateEOF bool) {
	...
	expectNoError(t, err)
	expectFlag(t, "ExecuteMultiFetch(multi result)", (status&mysql.ServerMoreResultsExists) != 0, true)
	assert.EqualValues(t, 1, len(qr.Rows))

	qr, status, _, err = conn.ReadQueryResult(10, true)
	expectNoError(t, err)
	expectFlag(t, "ReadQueryResult(1)", (status&mysql.ServerMoreResultsExists) != 0, true)
	assert.EqualValues(t, 0, len(qr.Rows))

	qr, status, _, err = conn.ReadQueryResult(10, true)
	expectNoError(t, err)
	expectFlag(t, "ReadQueryResult(2)", (status&mysql.ServerMoreResultsExists) != 0, false)
	assert.EqualValues(t, 1, len(qr.Rows))

	qr, status, err = conn.ExecuteFetchMulti("select 1 from dual", 10, true)
	expectNoError(t, err)
	expectFlag(t, "ExecuteMultiFetch(single result)", (status&mysql.ServerMoreResultsExists) != 0, false)
	assert.EqualValues(t, 1, len(qr.Rows))

	qr, status, err = conn.ExecuteFetchMulti("set autocommit=1", 10, true)
	expectNoError(t, err)
	expectFlag(t, "ExecuteMultiFetch(no result)", (status&mysql.ServerMoreResultsExists) != 0, false)
	assert.EqualValues(t, 0, len(qr.Rows))
