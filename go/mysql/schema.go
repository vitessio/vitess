...
order by table_name, ordinal_position`
	...
	DeleteFromViewsTable = `delete from %s.views where table_name in ::table_name`

	SelectFromViewsTable = `select table_name from %s.views where table_name in ::table_name`

	SelectAllViews = `select table_name, updated_at from %s.views`

	// FetchUpdatedViews queries fetches information about updated views
	FetchUpdatedViews = `select table_name, view_definition, create_statement from %s.views where table_name in ::viewnames`

	// FetchViews queries fetches all views
	FetchViews = `select table_name, view_definition, create_statement from %s.views`
)
