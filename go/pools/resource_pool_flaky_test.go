...
func TestOpen(t *testing.T) {
	...
	p := NewStaticResourcePool(PoolFactory, 6, 6, time.Second, 0, logWait)
	p.SetCapacity(5)
...
func TestPrefill(t *testing.T) {
	lastID.Set(0)
	count.Set(0)
	p := NewStaticResourcePool(PoolFactory, 5, 5, time.Second, 1, logWait)
	defer p.Close()
	if p.Active() != 5 {
		t.Errorf("p.Active(): %d, want 5", p.Active())
	}
	p = NewStaticResourcePool(FailFactory, 5, 5, time.Second, 1, logWait)
	defer p.Close()
	if p.Active() != 0 {
...
func TestPrefillTimeout(t *testing.T) {
	...
	start := time.Now()
	p := NewStaticResourcePool(SlowFailFactory, 5, 5, time.Second, 1, logWait)
	defer p.Close()
...
func TestShrinking(t *testing.T) {
	...
	p := NewStaticResourcePool(PoolFactory, 5, 5, time.Second, 0, logWait)
	var resources [10]Resource
...
func TestIdleTimeout(t *testing.T) {
	...
	lastID.Set(0)
	count.Set(0)
	p := NewStaticResourcePool(PoolFactory, 1, 1, 10*time.Millisecond, 0, logWait)
	defer p.Close()
...
func TestIdleTimeoutCreateFail(t *testing.T) {
	...
	// try to use it.
	p.(*StaticResourcePool).factory = FailFactory
	p.Put(r)
	time.Sleep(15 * time.Millisecond)
	if p.Active() != 0 {
		t.Errorf("p.Active(): %d, want 0", p.Active())
...
func TestCreateFail(t *testing.T) {
	...
	lastID.Set(0)
	count.Set(0)
	p := NewStaticResourcePool(FailFactory, 5, 5, time.Second, 0, logWait)
	defer p.Close()
...
func TestSlowCreateFail(t *testing.T) {
	...
	lastID.Set(0)
	count.Set(0)
	p := NewStaticResourcePool(SlowFailFactory, 2, 2, time.Second, 0, logWait)
	defer p.Close()
	ch := make(chan bool)
...
func TestTimeout(t *testing.T) {
	...
	lastID.Set(0)
	count.Set(0)
	p := NewStaticResourcePool(PoolFactory, 1, 1, time.Second, 0, logWait)
	defer p.Close()
	r, err := p.Get(ctx)
	if err != nil {
		t.Fatal(err)
