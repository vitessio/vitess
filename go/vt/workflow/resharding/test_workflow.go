...
func (tw *TestWorkflow) Run(ctx context.Context, manager *workflow.Manager, wi *topo.WorkflowInfo) error {
	...
	simpleTasks := tw.getTasks(phaseSimple, tw.checkpoint)
...
func (tw *TestWorkflow) getTasks(phaseName PhaseType, checkpoint *workflowpb.WorkflowCheckpoint) []*workflowpb.Task {
...
func (tw *TestWorkflow) runSimple(ctx context.Context, t *workflowpb.Task) error {
	...
	if tw.retryFlags[t.Id] {
		...
		fmt.Printf("I will fail at this time since retry flag is true.\n")

		// I cannot flip flag here, otherwise only one task will fail; create retry map to track tasks
