...
func (hw *HorizontalReshardingWorkflow) executeSplitClonePerShard(perhw *PerShardHorizontalResharding) error {
	...
	for _, d := range perhw.data.DestinationShards {
		if err := hw.wr.WaitForFilteredReplication(context.TODO(), perhw.data.Keyspace, d); err != nil {
			hw.logger.Infof(fmt.Sprintf("Horizontal Resharding: error in WaitForFilteredReplication: %v.", err))
			return err
		}
