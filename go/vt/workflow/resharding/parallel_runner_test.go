...
func TestParallelRunner(t *testing.T) {
	...
	if err := p.Run(make(map[string]*workflow.Node), tasks, executeLog, cp, PARALLEL); err != nil {
...
func TestParallelRunnerRetryAction(t *testing.T) {
	...
	if err := nodeManager.AddChildren(rootNode.Children, rootNode); err != nil {
		t.Errorf("adding children nodes failed: %v", err)
	}
	result, ok = <-notifications
	if !ok ||
	...
	executeLog := func(attr map[string]string) error {
		...
		if !retryFlag {
			if n == 3 {
				...
				return errors.New("Fake error for testing retry.")
			}
		}
		return nil
	}
			...
			case mornitor := <-notifications:
				if strings.Contains(string(mornitor), "Retry") {
					...
					if taskTarget.State != workflowpb.TaskState_TaskDone ||
						taskTarget.Error != "Fake error for testing retry." ||
	...
	go func() {
		...
		err := p.Run(taskNodeMap, tasks, executeLog, cp, PARALLEL)
		if err != nil {
