...
func TestGoRoutineLeakPrevention(t *testing.T) {
	...
	_ = qc.StreamExecute(context.Background(), nil, nil, "", nil, 0, 0, func(result *sqltypes.Result) error {
		return nil
	})
	require.Error(t, mqc.lastCallCtx.Err())

	_, _ = qc.BeginStreamExecute(context.Background(), nil, nil, nil, "", nil, 0, func(result *sqltypes.Result) error {
		return nil
	})
	require.Error(t, mqc.lastCallCtx.Err())
	...
	_, _ = qc.ReserveStreamExecute(context.Background(), nil, nil, nil, "", nil, 0, func(result *sqltypes.Result) error {
		return nil
	})
	require.Error(t, mqc.lastCallCtx.Err())
