...
func (sm *stateManager) unserveCommon() {
	...
	sm.tableGC.Close()
	log.Info("Finished table garbage collector close. Started system health monitor close")
	sm.systemHealth.Close()
	log.Info("Finished system health monitor close. Started lag throttler close")
	sm.throttler.Close()
