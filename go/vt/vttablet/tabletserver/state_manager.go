...
func (sm *stateManager) unserveCommon() {
	...
	sm.ddle.Close()
	log.Infof("Finished online ddl executor close")
	log.Infof("Started table garbage collector close")
	sm.tableGC.Close()
	log.Infof("Finished table garbage collector close")
	log.Infof("Started lag throttler close")
	sm.throttler.Close()
	log.Infof("Finished lag throttler close")
	log.Infof("Started messager close")
	sm.messager.Close()
	log.Infof("Finished messager close")
	log.Infof("Started txEngine close")
	sm.te.Close()
	log.Infof("Finished txEngine close")
	log.Info("Killing all OLAP queries.")
	sm.olapql.TerminateAll()
	log.Info("Finished Killing all OLAP queries.")
	log.Infof("Started tracker close")
	sm.tracker.Close()
	log.Infof("Finished tracker close")
	log.Infof("Started wait for requests")
	sm.requests.Wait()
	log.Infof("Finished wait for requests")
	log.Infof("Finished execution of unserveCommon")
}
