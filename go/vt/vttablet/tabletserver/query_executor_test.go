...
func TestQueryExecutorTableAclDualTableExempt(t *testing.T) {
	...
	// ignore acl
	query = "select 0 as x from dual where 1 + 1 union select 1 as x from dual where 1 + 1 limit 2"
	ctx = callerid.NewContext(context.Background(), nil, callerID)
	qre = newTestQueryExecutor(ctx, tsv, query, 0)
	_, err = qre.Execute()
	if err != nil {
...
func getQueryExecutorSupportedQueries(testTableHasMultipleUniqueKeys bool) map[string]*sqltypes.Result {
	return map[string]*sqltypes.Result{
		...
		"select 0 as x from dual where 1 + 1 union select 1 as x from dual where 1 + 1 limit 2": {
			Fields: []*querypb.Field{{
				Type: sqltypes.Uint64,
			}},
			Rows: [][]sqltypes.Value{
