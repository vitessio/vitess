...
func (tp *TxPool) transactionKiller() {
	...
	for _, conn := range tp.scp.GetElapsedTimeout(vterrors.TxKillerRollback) {
		log.WarnS(fmt.Sprintf("killing transaction (exceeded timeout: %v): %s", conn.timeout, conn.String(tp.env.Config().SanitizeLogMessages, tp.env.Environment().Parser())))
		switch {
		case conn.IsTainted():
			conn.Close()
...
func (tp *TxPool) RollbackAndRelease(ctx context.Context, txConn *StatefulConnection) {
	...
	if rollbackError != nil {
		log.ErrorS(fmt.Sprintf("tried to rollback, but failed with: %v", rollbackError.Error()))
	}
}
