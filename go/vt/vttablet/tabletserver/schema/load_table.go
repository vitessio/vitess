...
func getSpecifiedMessageFields(tableFields []*querypb.Field, specifiedCols []string, collationEnv *collations.Environment) []*querypb.Field {
	...
	for _, col := range specifiedCols {
		for _, field := range tableFields {
			if res, _ := evalengine.NullsafeCompare(collationEnv, sqltypes.NewVarChar(field.Name), sqltypes.NewVarChar(strings.TrimSpace(col)), collationEnv.DefaultConnectionCharset()); res == 0 {
				fields = append(fields, field)
				break
			}
		}
