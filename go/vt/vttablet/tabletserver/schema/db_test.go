...
func TestGetCreateStatement(t *testing.T) {
	db := fakesqldb.New(t)
	conn, err := connpool.NewConnectionNoPool(context.Background(), db.ConnParams(), nil, nil)
	require.NoError(t, err)
...
func collectGetViewDefinitions(conn *connpool.Connection, bv map[string]*querypb.BindVariable) (map[string]string, error) {
