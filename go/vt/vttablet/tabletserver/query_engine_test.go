...
func TestGetPlanPanicDuetoEmptyQuery(t *testing.T) {
	...
	_, err := qe.GetPlan(logStats, "", false, 0, nil)
...
func TestQueryPlanCache(t *testing.T) {
	...
	secondPlan, err := qe.GetPlan(logStats, secondQuery, false, 0, nil)
	if err != nil {
		t.Fatal(err)
	}
	if secondPlan == nil {
...
func TestNoQueryPlanCache(t *testing.T) {
	...
	logStats := tabletenv.NewLogStats(ctx, "GetPlanStats")
	qe.SetQueryPlanCacheCap(1024)
	firstPlan, err := qe.GetPlan(logStats, firstQuery, true, 0, nil)
	if err != nil {
		t.Fatal(err)
	}
	if firstPlan == nil {
...
func TestStatsURL(t *testing.T) {
	...
	ctx := context.Background()
	logStats := tabletenv.NewLogStats(ctx, "GetPlanStats")
	qe.GetPlan(logStats, query, false, 0, nil)
...
func BenchmarkPlanCacheThroughput(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		_, err := qe.GetPlan(logStats, query, false, 0, nil)
		if err != nil {
			b.Fatal(err)
		}
	}
