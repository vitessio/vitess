...
// The same SQL text can mean different statements under different lexer modes, so
// plans are built with the session's mode and cached separately per mode.
func TestQueryPlanCacheParseSQLMode(t *testing.T) {
	...
	cachedPipes, err := qe.GetPlan(ctx, logStats, query, sqlmode.PipesAsConcat, false, false)
	require.NoError(t, err)
