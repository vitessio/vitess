...
func TestAddQueryStats(t *testing.T) {
	...
	for _, testcase := range testcases {
		t.Run(testcase.name, func(t *testing.T) {
			...
			env := tabletenv.NewEnv(config, "TestAddQueryStats_"+testcase.name, collations.Local())
			se := schema.NewEngine(env)
			qe := NewQueryEngine(env, se)
...
func TestPlanPoolUnsafe(t *testing.T) {
	...
	for _, tcase := range tcases {
		t.Run(tcase.name, func(t *testing.T) {
			...
			require.NoError(t, err)
			plan, err := planbuilder.Build(statement, map[string]*schema.Table{}, "dbName", false, collations.Local())
