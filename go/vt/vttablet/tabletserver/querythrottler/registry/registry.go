...
func Register(name ThrottlingStrategy, factory StrategyFactory) {
	...
	log.InfoS(fmt.Sprintf("Registered throttling strategy: %s", name))
}
...
func CreateStrategy(cfg StrategyConfig, deps Deps) ThrottlingStrategyHandler {
	...
	if !ok {
		log.WarnS(fmt.Sprintf("Unknown strategy %s, using NoOp", cfg.GetStrategy()))
		return &NoOpStrategy{}
	}

	strategy, err := factory.New(deps, cfg)
	if err != nil {
		log.ErrorS(fmt.Sprintf("Strategy %s failed to init: %v, using NoOp", cfg.GetStrategy(), err))
		return &NoOpStrategy{}
	}
