...
func TestTxExecutorCommitRedoFail(t *testing.T) {
	...
	db.AddRejectedQuery("delete from _vt.redo_state where dtid = X'6262'", errors.New("delete redo log fail"))
	db.AddQuery("update _vt.redo_state set state = 0 where dtid = X'6262'", sqltypes.MakeTestResult(nil))
	err = txe.CommitPrepared("bb")
...
func newTestTxExecutor(t *testing.T, ctx context.Context) (txe *DTExecutor, tsv *TabletServer, db *fakesqldb.DB, closer func()) {
	...
	db.AddQueryPattern("insert into _vt\\.redo_state\\(dtid, state, time_created\\) values \\(X'6161', 1,.*", &sqltypes.Result{})
	db.AddQueryPattern("insert into _vt\\.redo_statement.*", &sqltypes.Result{})
	db.AddQuery("delete from _vt.redo_state where dtid = X'6161'", &sqltypes.Result{})
	db.AddQuery("delete from _vt.redo_statement where dtid = X'6161'", &sqltypes.Result{})
