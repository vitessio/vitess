...
// A regular expression to extract the savepoint name from savepoint related binlog events.
//
// We probably could use the sql parser instead to extract the savepoint name, but
// these statements are generated by MySQL in a very consistent fashion, so using
// a regular expression here should be fine.
var extractSavepointName = regexp.MustCompile("^(?:SAVEPOINT|ROLLBACK TO) `(.*)`$")
...
func (vs *vstreamer) parseEvents(ctx context.Context, events <-chan mysql.BinlogEvent) error {
	...
	// Due to filtering, we can encounter multiple savepoint events without any actual
	...
	bufferAndTransmit := func(vevent *binlogdatapb.VEvent) error {
		...
		case binlogdatapb.VEventType_SAVEPOINT:
			// Savepoint events in the MySQL binlog can either be `SAVEPOINT ...` or `ROLLBACK TO ...` events.
			// `RELEASE SAVEPOINT ...` is never written to the binlog.
			//
			// `ROLLBACK TO ...` can only be seen when MySQL is executed with GTID consistency checking disabled
			// and when a transaction creates a savepoint, modifies a non-transactional table, and then executes
