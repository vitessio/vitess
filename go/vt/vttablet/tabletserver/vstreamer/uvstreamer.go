...
func (uvs *uvstreamer) send2(evs []*binlogdatapb.VEvent) error {
	...
	uvs.setSecondsBehindPrimary(behind / 1e9)
	//log.Infof("sbm set to %d", uvs.secondsBehindPrimary)
	var evs2 []*binlogdatapb.VEvent
	if len(uvs.plans) > 0 {
...
func (uvs *uvstreamer) getSecondsBehindPrimary() int64 {
	uvs.mu.Lock()
	defer uvs.mu.Unlock()
	return uvs.secondsBehindPrimary
}

func (uvs *uvstreamer) setSecondsBehindPrimary(sbm int64) {
	uvs.mu.Lock()
	defer uvs.mu.Unlock()
	uvs.secondsBehindPrimary = sbm
}
