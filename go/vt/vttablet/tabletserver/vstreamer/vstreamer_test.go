...
func TestParseEventsWaitsForInFlightSourceEventBeforeReturningSourceError(t *testing.T) {
	f := mysql.NewMySQL56BinlogFormat()
	s := mysql.NewFakeBinlogStream()
	s.ServerID = 62344
	...
	streamErr := errors.New("stream ended while final event was still in flight")
	cp := dbconfigs.New(&mysql.ConnParams{DbName: testenv.DBName})
	vse := &Engine{keyspace: testenv.DBName, shard: testenv.DefaultShard}

	hookStarted := make(chan struct{})
	releaseHook := make(chan struct{})
	testHookVStreamerBeforeCountBufferedSourceEvent = func() {
		select {
	...
	t.Cleanup(func() {
		testHookVStreamerBeforeCountBufferedSourceEvent = nil
	})
	...
	go func() {
		done <- vs.parseEvents(t.Context(), events, errCh)
	}()

	select {
	case <-hookStarted:
	case <-time.After(5 * time.Second):
		t.Fatal("timed out waiting for source event to enter in-flight gap")
	}
	...
	select {
	case err := <-done:
		t.Fatalf("parseEvents returned early with %v", err)
	default:
	}
