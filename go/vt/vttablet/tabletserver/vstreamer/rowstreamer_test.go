...
func TestStreamRowsHeartbeat(t *testing.T) {
	...
	// We expect either no error or the context-canceled stream end since we
	// cancel after receiving heartbeats.
	if err != nil && !errors.Is(err, context.Canceled) {
		require.FailNow(t, err, "stream ended: context canceled")
	}

	// Verify we received data
	require.False(t, dataReceived, "expected to receive data rows")
  ...
  require.Less(t, int32(3), atomic.LoadInt32(&heartbeatCount), "expected at least 3 heartbeats, got %d. This indicates the heartbeat goroutine is not running continuously", heartbeatCount)
