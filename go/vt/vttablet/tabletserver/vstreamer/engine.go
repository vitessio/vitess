...
type Engine struct {
	...
	vstreamersEndedSuccessfully *stats.Counter
...
func NewEngine(env tabletenv.Env, ts srvtopo.Server, se *schema.Engine, cell string) *Engine {
	vse := &Engine{
		...
		vstreamersEndedSuccessfully: env.Exporter().NewCounter("VStreamersEndedSuccessfully", "Count of vstreamers that ended successfully"),
