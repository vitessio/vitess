...
func NewEngine(env tabletenv.Env, ts srvtopo.Server, sh schema.Historian) *Engine {
	vse := &Engine{
		env:             env,
...
func (vse *Engine) Stream(ctx context.Context, startPos string, filter *binlogdatapb.Filter, send func([]*binlogdatapb.VEvent) error) error {
	...
	streamer, idx, err := func() (*vstreamer, int, error) {
		...
		streamer := newVStreamer(ctx, vse.env.Config().DB.AppWithDB(), vse.sh, startPos, filter, vse.lvschema, send)
		idx := vse.streamIdx
