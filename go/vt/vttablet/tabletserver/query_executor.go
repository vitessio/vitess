...
func (qre *QueryExecutor) Execute() (reply *sqltypes.Result, err error) {
	...
	defer func(start time.Time) {
		...
		if reply == nil {
			qre.tsv.qe.AddStats(qre.plan.PlanID, tableName, qre.workload, 1, duration, mysqlTime, 0, 0, 1)
			qre.plan.AddStats(1, duration, mysqlTime, 0, 0, 1)
			return
		}
		qre.tsv.qe.AddStats(qre.plan.PlanID, tableName, qre.workload, 1, duration, mysqlTime, int64(reply.RowsAffected), int64(len(reply.Rows)), 0)
