...
func TestDBConnExec(t *testing.T) {
	...
	defer cancel()
	dbConn, err := newPooledConnection(context.Background(), connPool, db.ConnParams())
	if dbConn != nil {
		defer dbConn.Close()
	}
	if err != nil {
...
func TestDBNoPoolConnKill(t *testing.T) {
	...
	connPool.Open(db.ConnParams(), db.ConnParams(), db.ConnParams())
	defer connPool.Close()
	dbConn, err := NewConnectionNoPool(context.Background(), db.ConnParams(), connPool.dbaPool, nil)
	if dbConn != nil {
		defer dbConn.Close()
	}
	if err != nil {
...
func TestDBConnReApplySetting(t *testing.T) {
	...
	dbConn, err := newPooledConnection(ctx, connPool, db.ConnParams())
	require.NoError(t, err)
	defer dbConn.Close()
