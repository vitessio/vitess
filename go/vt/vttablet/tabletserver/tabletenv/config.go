...
func registerTabletEnvFlags(fs *pflag.FlagSet) {
	...
	fs.Bool("schema-user-tables-free-space-metrics-enabled", schemaUserTablesFreeSpaceMetricsEnabled.Default(), "publish per-table SchemaTableDataFreeBytes (DATA_FREE bytes reclaimable via OPTIMIZE) and SchemaTableAllocatedBytes (DATA_LENGTH + INDEX_LENGTH + DATA_FREE total tablespace) gauges for every user table on each schema reload. Dynamically reloadable via the viper config file.")
