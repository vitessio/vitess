...
func registerTabletEnvFlags(fs *pflag.FlagSet) {
	...
	SecondsVar(fs, &currentConfig.OltpReadPool.MaxLifetimeTimeoutSeconds, "queryserver-max-lifetime-timeout", defaultConfig.OltpReadPool.MaxLifetimeTimeoutSeconds, "query server refresh timeout (in seconds), vttablet manages various mysql connection pools. This config means if a connection has lived at least this long, it connection will be removed from pool upon the next time it is returned to the pool.")
...
func Init() {
	...
	currentConfig.OlapReadPool.MaxLifetimeTimeoutSeconds = currentConfig.OltpReadPool.MaxLifetimeTimeoutSeconds
	currentConfig.TxPool.MaxLifetimeTimeoutSeconds = currentConfig.OltpReadPool.MaxLifetimeTimeoutSeconds
