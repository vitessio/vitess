...
func TestBeginOnReplica(t *testing.T) {
	...
	response, err := tsv.Begin(ctx, &target, &options)
	...
	_, err = tsv.Rollback(ctx, &target, response.TransactionId)
...
func TestTabletServerPrimaryToReplica(t *testing.T) {
	...
	response1, err := tsv.Begin(ctx, &target, nil)
	require.NoError(t, err)

	_, err = tsv.Execute(ctx, &target, "update test_table set `name` = 2 where pk = 1", nil, response1.TransactionId, 0, nil)
	require.NoError(t, err)
	err = tsv.Prepare(ctx, &target, response1.TransactionId, "aa")
	require.NoError(t, err)
	response2, err := tsv.Begin(ctx, &target, nil)
	require.NoError(t, err)

	// This makes txid2 busy
	conn2, err := tsv.te.txPool.GetAndLock(response2.TransactionId, "for query")
	require.NoError(t, err)
	ch := make(chan bool)
	go func() {
...
func TestTabletServerCommitTransaction(t *testing.T) {
	...
	require.NoError(t, err)
	_, err = tsv.Execute(ctx, &target, executeSQL, nil, response.TransactionId, 0, nil)
	require.NoError(t, err)
	_, err = tsv.Commit(ctx, &target, response.TransactionId)
	require.NoError(t, err)
}
...
func TestTabletServerPrepare(t *testing.T) {
	...
	response, err := tsv.Begin(ctx, &target, nil)
	require.NoError(t, err)
	_, err = tsv.Execute(ctx, &target, "update test_table set `name` = 2 where pk = 1", nil, response.TransactionId, 0, nil)
	require.NoError(t, err)
	defer tsv.RollbackPrepared(ctx, &target, "aa", 0)
	err = tsv.Prepare(ctx, &target, response.TransactionId, "aa")
	require.NoError(t, err)
}
...
func TestTabletServerReserveConnection(t *testing.T) {
	...
	response, err := tsv.ReserveExecute(ctx, &target, nil, "select 42", nil, 0, options)
	require.NoError(t, err)

	// run a query in it
	_, err = tsv.Execute(ctx, &target, "select 42", nil, 0, response.ReservedId, options)
	require.NoError(t, err)

	// release the connection
	err = tsv.Release(ctx, &target, 0, response.ReservedId)
	require.NoError(t, err)
}
...
func TestMakeSureToCloseDbConnWhenBeginQueryFails(t *testing.T) {
	...
	// run a query with a non-existent reserved id
	_, err := tsv.ReserveBeginExecute(ctx, &target, []string{}, nil, "select 42", nil, options)
	require.Error(t, err)
}
...
func TestTabletServerReserveAndBeginCommit(t *testing.T) {
	...
	response, err := tsv.ReserveBeginExecute(ctx, &target, nil, nil, "select 42", nil, options)
	require.NoError(t, err)
	defer func() {
		// fallback so the test finishes quickly
		tsv.Release(ctx, &target, response.TransactionId, response.ReservedId)
	}()

	// run a query in it
	_, err = tsv.Execute(ctx, &target, "select 42", nil, response.TransactionId, response.ReservedId, options)
	require.NoError(t, err)

	// run a query in a non-existent connection
	_, err = tsv.Execute(ctx, &target, "select 42", nil, response.TransactionId, response.ReservedId+100, options)
	require.Error(t, err)
	_, err = tsv.Execute(ctx, &target, "select 42", nil, response.TransactionId+100, response.ReservedId, options)
	require.Error(t, err)

	// commit
	commitResponse, err := tsv.Commit(ctx, &target, response.TransactionId)
	require.NoError(t, err)
	assert.NotEqual(t, response.ReservedId, commitResponse.ReservedId)

	// begin and rollback
	beginResponse, err := tsv.BeginExecute(ctx, &target, nil, "select 42", nil, commitResponse.ReservedId, options)
	require.NoError(t, err)
	assert.Equal(t, commitResponse.ReservedId, beginResponse.TransactionId)

	rollbackResponse, err := tsv.Rollback(ctx, &target, beginResponse.TransactionId)
	require.NoError(t, err)
	assert.NotEqual(t, commitResponse.ReservedId, rollbackResponse.ReservedId)

	// release the connection
	err = tsv.Release(ctx, &target, 0, rollbackResponse.ReservedId)
	require.NoError(t, err)
...
func TestTabletServerRollbackPrepared(t *testing.T) {
	...
	err = tsv.Prepare(ctx, &target, response.TransactionId, "aa")
	require.NoError(t, err)
	err = tsv.RollbackPrepared(ctx, &target, "aa", response.TransactionId)
	require.NoError(t, err)
}
...
func TestSerializeTransactionsSameRow(t *testing.T) {
	...
	go func() {
		...
		response1, err := tsv.BeginExecute(ctx, &target, nil, q1, bvTx1, 0, nil)
		if err != nil {
			t.Errorf("failed to execute query: %s: %s", q1, err)
		}
		if _, err := tsv.Commit(ctx, &target, response1.TransactionId); err != nil {
	...
	go func() {
		...
		<-tx1Started
		response2, err := tsv.BeginExecute(ctx, &target, nil, q2, bvTx2, 0, nil)
		if err != nil {
		...
		// still pending.
		<-tx3Finished
		if _, err := tsv.Commit(ctx, &target, response2.TransactionId); err != nil {
			t.Errorf("call TabletServer.Commit failed: %v", err)
		}
	}()
	...
	go func() {
		...
		<-tx1Started
		response3, err := tsv.BeginExecute(ctx, &target, nil, q3, bvTx3, 0, nil)
		if err != nil {
			t.Errorf("failed to execute query: %s: %s", q3, err)
		}
		if _, err := tsv.Commit(ctx, &target, response3.TransactionId); err != nil {
			t.Errorf("call TabletServer.Commit failed: %v", err)
		}
		close(tx3Finished)
	}()
...
func TestDMLQueryWithoutWhereClause(t *testing.T) {
	...
	response, err := tsv.BeginExecute(ctx, &target, nil, q, nil, 0, nil)
	require.NoError(t, err)
	_, err = tsv.Commit(ctx, &target, response.TransactionId)
	require.NoError(t, err)
}
...
func TestSerializeTransactionsSameRow_TooManyPendingRequests(t *testing.T) {
	...
	go func() {
		...
		<-tx1Started
		_, err := tsv.BeginExecute(ctx, &target, nil, q2, bvTx2, 0, nil)
...
func TestSerializeTransactionsSameRow_RequestCanceled(t *testing.T) {
	...
	go func() {
		...
		_, err := tsv.BeginExecute(ctxTx2, &target, nil, q2, bvTx2, 0, nil)
...
func TestReserveBeginExecute(t *testing.T) {
	...
	response, err := tsv.ReserveBeginExecute(ctx, &target, []string{"select 43"}, nil, "select 42", nil, &querypb.ExecuteOptions{})
	require.NoError(t, err)

	assert.Greater(t, response.TransactionId, int64(0), "transactionID")
	assert.Equal(t, response.ReservedId, response.TransactionId, "reservedID should equal transactionID")
	expected := []string{
		"select 43",
		"begin",
	...
	err = tsv.Release(ctx, &target, response.TransactionId, response.ReservedId)
	require.NoError(t, err)
}
...
func TestReserveExecute_WithoutTx(t *testing.T) {
	...
	response, err := tsv.ReserveExecute(ctx, &target, []string{"select 43"}, "select 42", nil, 0, &querypb.ExecuteOptions{})
	require.NoError(t, err)
	assert.NotEqual(t, int64(0), response.ReservedId, "reservedID should not be zero")
	expected := []string{
		"select 43",
		"select 42 from dual limit 10001",
	}
...
func TestReserveExecute_WithTx(t *testing.T) {
	...
	beginResponse, err := tsv.Begin(ctx, &target, &querypb.ExecuteOptions{})
	require.NoError(t, err)
	require.NotEqual(t, int64(0), beginResponse.TransactionId)
	db.ResetQueryLog()

	reserveExecuteResponse, err := tsv.ReserveExecute(ctx, &target, []string{"select 43"}, "select 42", nil, beginResponse.TransactionId, &querypb.ExecuteOptions{})
	require.NoError(t, err)
	defer tsv.Release(ctx, &target, beginResponse.TransactionId, reserveExecuteResponse.ReservedId)
	assert.Equal(t, beginResponse.TransactionId, reserveExecuteResponse.ReservedId, "reservedID should be equal to transactionID")
	expected := []string{
		"select 43",
		"select 42 from dual limit 10001",
	}
	...
	err = tsv.Release(ctx, &target, beginResponse.TransactionId, reserveExecuteResponse.ReservedId)
	require.NoError(t, err)
}
	...
	for i, test := range tests {
		...
		t.Run(name, func(t *testing.T) {
			...
			case test.begin && test.reserve:
				response, err := tsv.ReserveBeginExecute(ctx, &target, []string{"select 1212"}, nil, "select 42", nil, &querypb.ExecuteOptions{})
				require.NoError(t, err)
				transactionID = response.TransactionId
				reservedID = response.ReservedId
				require.NotEqual(t, int64(0), transactionID)
				require.NotEqual(t, int64(0), reservedID)
			case test.begin:
				response, err := tsv.BeginExecute(ctx, &target, nil, "select 42", nil, 0, &querypb.ExecuteOptions{})
				require.NoError(t, err)
				transactionID = response.TransactionId
				require.NotEqual(t, int64(0), transactionID)
			case test.reserve:
				response, err := tsv.ReserveExecute(ctx, &target, nil, "select 42", nil, 0, &querypb.ExecuteOptions{})
				require.NoError(t, err)
				reservedID = response.ReservedId
				require.NotEqual(t, int64(0), reservedID)
			}
...
func TestReserveStats(t *testing.T) {
	...
	rbeResponse, err := tsv.ReserveBeginExecute(ctx, &target, nil, nil, "select 42", nil, &querypb.ExecuteOptions{})
	require.NoError(t, err)
	assert.EqualValues(t, 1, tsv.te.txPool.env.Stats().UserActiveReservedCount.Counts()["test"])

	// Starts reserved connection
	reResponse, err := tsv.ReserveExecute(ctx, &target, nil, "select 42", nil, 0, &querypb.ExecuteOptions{})
	require.NoError(t, err)
	...
	reBeResponse, err := tsv.BeginExecute(ctx, &target, nil, "select 42", nil, reResponse.ReservedId, &querypb.ExecuteOptions{})
	require.NoError(t, err)
	assert.EqualValues(t, 2, tsv.te.txPool.env.Stats().UserActiveReservedCount.Counts()["test"])

	// Starts transaction.
	beResponse, err := tsv.BeginExecute(ctx, &target, nil, "select 42", nil, 0, &querypb.ExecuteOptions{})
	require.NoError(t, err)
	assert.EqualValues(t, 2, tsv.te.txPool.env.Stats().UserActiveReservedCount.Counts()["test"])

	// Reserved the connection on previous transaction
	beReResponse, err := tsv.ReserveExecute(ctx, &target, nil, "select 42", nil, beResponse.TransactionId, &querypb.ExecuteOptions{})
	require.NoError(t, err)
	assert.EqualValues(t, 3, tsv.te.txPool.env.Stats().UserActiveReservedCount.Counts()["test"])

	err = tsv.Release(ctx, &target, rbeResponse.TransactionId, rbeResponse.ReservedId)
	require.NoError(t, err)
	assert.EqualValues(t, 2, tsv.te.txPool.env.Stats().UserActiveReservedCount.Counts()["test"])
	assert.EqualValues(t, 1, tsv.te.txPool.env.Stats().UserReservedCount.Counts()["test"])

	err = tsv.Release(ctx, &target, reBeResponse.TransactionId, reResponse.ReservedId)
	require.NoError(t, err)
	assert.EqualValues(t, 1, tsv.te.txPool.env.Stats().UserActiveReservedCount.Counts()["test"])
	assert.EqualValues(t, 2, tsv.te.txPool.env.Stats().UserReservedCount.Counts()["test"])

	err = tsv.Release(ctx, &target, beResponse.TransactionId, beReResponse.ReservedId)
	require.NoError(t, err)
...
func TestDatabaseNameReplaceByKeyspaceNameExecuteMethod(t *testing.T) {
	...
	response, err := tsv.Begin(ctx, target, nil)
	require.NoError(t, err)
	res, err := tsv.Execute(ctx, target, executeSQL, nil, response.TransactionId, 0, &querypb.ExecuteOptions{
		IncludedFields: querypb.ExecuteOptions_ALL,
	})
	require.NoError(t, err)
	for _, field := range res.Result.Fields {
...
func TestDatabaseNameReplaceByKeyspaceNameBeginExecuteMethod(t *testing.T) {
	...
	response, err := tsv.BeginExecute(ctx, target, nil, executeSQL, nil, 0, &querypb.ExecuteOptions{
		IncludedFields: querypb.ExecuteOptions_ALL,
	})
	require.NoError(t, err)
	for _, field := range response.Result.Fields {
		require.Equal(t, "keyspaceName", field.Database)
	}
	_, err = tsv.Commit(ctx, target, response.TransactionId)
	require.NoError(t, err)
...
func TestDatabaseNameReplaceByKeyspaceNameReserveExecuteMethod(t *testing.T) {
	...
	// Test ReserveExecute
	response, err := tsv.ReserveExecute(ctx, target, nil, executeSQL, nil, 0, &querypb.ExecuteOptions{
		IncludedFields: querypb.ExecuteOptions_ALL,
	})
	require.NoError(t, err)
	for _, field := range response.Result.Fields {
...
func TestDatabaseNameReplaceByKeyspaceNameReserveBeginExecuteMethod(t *testing.T) {
	...
	response, err := tsv.ReserveBeginExecute(ctx, target, nil, nil, executeSQL, nil, &querypb.ExecuteOptions{
		IncludedFields: querypb.ExecuteOptions_ALL,
	})
	require.NoError(t, err)
	for _, field := range response.Result.Fields {
