...
func TestBeginOnReplica(t *testing.T) {
	...
	state, err := tsv.Begin(ctx, &vtgatepb.Session{Options: &options}, &target)
...
func TestTabletServerPrimaryToReplica(t *testing.T) {
	...
	state1, err := tsv.Begin(ctx, &vtgatepb.Session{}, &target)
	require.NoError(t, err)

	_, err = tsv.Execute(ctx, &vtgatepb.Session{}, &target, "update test_table set `name` = 2 where pk = 1", nil, state1.TransactionID, 0)
	require.NoError(t, err)
	err = tsv.Prepare(ctx, &target, state1.TransactionID, "aa")
	require.NoError(t, err)
	state2, err := tsv.Begin(ctx, &vtgatepb.Session{}, &target)
	require.NoError(t, err)
...
func TestTabletServerBeginFail(t *testing.T) {
	...
	defer cancel()
	tsv.Begin(ctx, &vtgatepb.Session{}, &target)
...
func TestTabletServerCommitTransaction(t *testing.T) {
	...
	require.NoError(t, err)
	_, err = tsv.Execute(ctx, &vtgatepb.Session{}, &target, executeSQL, nil, state.TransactionID, 0)
	require.NoError(t, err)
...
func TestTabletServerPrepare(t *testing.T) {
	...
	state, err := tsv.Begin(ctx, &vtgatepb.Session{}, &target)
	require.NoError(t, err)
	_, err = tsv.Execute(ctx, &vtgatepb.Session{}, &target, "update test_table set `name` = 2 where pk = 1", nil, state.TransactionID, 0)
	require.NoError(t, err)
...
func TestTabletServerWithNilTarget(t *testing.T) {
	...
	_, err = tsv.Begin(nonLocalCtx, &vtgatepb.Session{}, target)
...
func TestTabletServerReserveConnection(t *testing.T) {
	...
	state, _, err := tsv.ReserveExecute(ctx, &vtgatepb.Session{Options: options}, &target, nil, "set sql_mode = ''", nil, 0)
	require.NoError(t, err)

	// run a query in it
	_, err = tsv.Execute(ctx, &vtgatepb.Session{Options: options}, &target, "select 42", nil, 0, state.ReservedID)
	require.NoError(t, err)
...
func TestTabletServerExecNonExistentConnection(t *testing.T) {
	...
	_, err := tsv.Execute(ctx, &vtgatepb.Session{Options: options}, &target, "select 42", nil, 0, 123456)
	require.Error(t, err)
}
...
func TestMakeSureToCloseDbConnWhenBeginQueryFails(t *testing.T) {
	...
	// run a query with a non-existent reserved id
	_, _, err := tsv.ReserveBeginExecute(ctx, &vtgatepb.Session{Options: options}, &target, []string{}, nil, "select 42", nil)
	require.Error(t, err)
}
...
func TestTabletServerReserveAndBeginCommit(t *testing.T) {
	...
	state, _, err := tsv.ReserveBeginExecute(ctx, &vtgatepb.Session{Options: options}, &target, nil, nil, "set sql_mode = ''", nil)
	require.NoError(t, err)
	defer func() {
	...
	// run a query in it
	_, err = tsv.Execute(ctx, &vtgatepb.Session{Options: options}, &target, "select 42", nil, state.TransactionID, state.ReservedID)
	require.NoError(t, err)

	// run a query in a non-existent connection
	_, err = tsv.Execute(ctx, &vtgatepb.Session{Options: options}, &target, "select 42", nil, state.TransactionID, state.ReservedID+100)
	require.Error(t, err)
	_, err = tsv.Execute(ctx, &vtgatepb.Session{Options: options}, &target, "select 42", nil, state.TransactionID+100, state.ReservedID)
	require.Error(t, err)
	...
	// begin and rollback
	beginState, _, err := tsv.BeginExecute(ctx, &vtgatepb.Session{Options: options}, &target, nil, "select 42", nil, rID)
	require.NoError(t, err)
...
func TestTabletServerStreamExecute(t *testing.T) {
	...
	if err := tsv.StreamExecute(ctx, &vtgatepb.Session{}, &target, executeSQL, nil, 0, 0, callback); err != nil {
...
func TestTabletServerBeginStreamExecute(t *testing.T) {
	...
	target := querypb.Target{TabletType: topodatapb.TabletType_PRIMARY}
	callback := func(*sqltypes.Result) error { return nil }
	state, err := tsv.BeginStreamExecute(ctx, &vtgatepb.Session{}, &target, nil, executeSQL, nil, 0, callback)
	if err != nil {
...
func TestSerializeTransactionsSameRow(t *testing.T) {
	...
	go func() {
		...
		state1, _, err := tsv.BeginExecute(ctx, &vtgatepb.Session{}, &target, nil, q1, bvTx1, 0)
		if err != nil {
	...
	go func() {
		...
		<-tx1Started
		state2, _, err := tsv.BeginExecute(ctx, &vtgatepb.Session{}, &target, nil, q2, bvTx2, 0)
		if err != nil {
	...
	go func() {
		...
		<-tx1Started
		state3, _, err := tsv.BeginExecute(ctx, &vtgatepb.Session{}, &target, nil, q3, bvTx3, 0)
		if err != nil {
...
func TestDMLQueryWithoutWhereClause(t *testing.T) {
	...
	state, _, err := tsv.BeginExecute(ctx, &vtgatepb.Session{}, &target, nil, q, nil, 0)
	require.NoError(t, err)
	_, err = tsv.Commit(ctx, &target, state.TransactionID)
	require.NoError(t, err)
}
...
func TestSerializeTransactionsSameRow_TooManyPendingRequests(t *testing.T) {
	...
	go func() {
		...
		<-tx1Started
		_, _, err := tsv.BeginExecute(ctx, &vtgatepb.Session{}, &target, nil, q2, bvTx2, 0)
...
func TestSerializeTransactionsSameRow_RequestCanceled(t *testing.T) {
	...
	go func() {
		...
		_, _, err := tsv.BeginExecute(ctxTx2, &vtgatepb.Session{}, &target, nil, q2, bvTx2, 0)
...
func TestReserveBeginExecute(t *testing.T) {
	...
	state, _, err := tsv.ReserveBeginExecute(ctx, &vtgatepb.Session{}, &target, nil, nil, "set @@sql_mode = ''", nil)
	require.NoError(t, err)
...
func TestReserveExecute_WithoutTx(t *testing.T) {
	...
	state, _, err := tsv.ReserveExecute(ctx, &vtgatepb.Session{}, &target, nil, "set sql_mode = ''", nil, 0)
	require.NoError(t, err)
...
func TestReserveExecute_WithTx(t *testing.T) {
	...
	beginState, err := tsv.Begin(ctx, &vtgatepb.Session{}, &target)
	require.NoError(t, err)
	require.NotEqual(t, int64(0), beginState.TransactionID)
	db.ResetQueryLog()

	reserveState, _, err := tsv.ReserveExecute(ctx, &vtgatepb.Session{}, &target, nil, "set sql_mode = ''", nil, beginState.TransactionID)
	require.NoError(t, err)
...
func TestRelease(t *testing.T) {
	...
	for i, test := range tests {
		...
		t.Run(name, func(t *testing.T) {
			...
			case test.begin:
				state, _, err := tsv.BeginExecute(ctx, &vtgatepb.Session{}, &target, nil, "select 42", nil, 0)
				require.NoError(t, err)
				transactionID = state.TransactionID
				require.NotEqual(t, int64(0), transactionID)
			case test.reserve:
...
func TestReserveStats(t *testing.T) {
	...
	rbeState, _, err := tsv.ReserveBeginExecute(ctx, &vtgatepb.Session{}, &target, nil, nil, "set sql_mode = ''", nil)
	require.NoError(t, err)
	assert.EqualValues(t, 1, tsv.te.txPool.env.Stats().UserActiveReservedCount.Counts()["test"])

	// Starts reserved connection
	reState, _, err := tsv.ReserveExecute(ctx, &vtgatepb.Session{}, &target, nil, "set sql_mode = ''", nil, 0)
	require.NoError(t, err)
	...
	reBeState, _, err := tsv.BeginExecute(ctx, &vtgatepb.Session{}, &target, nil, "select 42", nil, reState.ReservedID)
	require.NoError(t, err)
	assert.EqualValues(t, 2, tsv.te.txPool.env.Stats().UserActiveReservedCount.Counts()["test"])

	// Starts transaction.
	beState, _, err := tsv.BeginExecute(ctx, &vtgatepb.Session{}, &target, nil, "select 42", nil, 0)
	require.NoError(t, err)
	assert.EqualValues(t, 2, tsv.te.txPool.env.Stats().UserActiveReservedCount.Counts()["test"])

	// Reserved the connection on previous transaction
	beReState, _, err := tsv.ReserveExecute(ctx, &vtgatepb.Session{}, &target, nil, "set sql_mode = ''", nil, beState.TransactionID)
	require.NoError(t, err)
...
func TestDatabaseNameReplaceByKeyspaceNameExecuteMethod(t *testing.T) {
	...
	state, err := tsv.Begin(ctx, &vtgatepb.Session{}, target)
	require.NoError(t, err)
	res, err := tsv.Execute(ctx, &vtgatepb.Session{Options: &querypb.ExecuteOptions{
		IncludedFields: querypb.ExecuteOptions_ALL,
	}}, target, executeSQL, nil, state.TransactionID, 0)
	require.NoError(t, err)
...
func TestDatabaseNameReplaceByKeyspaceNameStreamExecuteMethod(t *testing.T) {
	...
	err := tsv.StreamExecute(ctx, &vtgatepb.Session{Options: &querypb.ExecuteOptions{
		IncludedFields: querypb.ExecuteOptions_ALL,
	}}, target, executeSQL, nil, 0, 0, callback)
	require.NoError(t, err)
}
...
func TestDatabaseNameReplaceByKeyspaceNameBeginExecuteMethod(t *testing.T) {
	...
	state, res, err := tsv.BeginExecute(ctx, &vtgatepb.Session{Options: &querypb.ExecuteOptions{
		IncludedFields: querypb.ExecuteOptions_ALL,
...
func TestDatabaseNameReplaceByKeyspaceNameReserveExecuteMethod(t *testing.T) {
	...
	// Test ReserveExecute
	_, res, err := tsv.ReserveExecute(ctx, &vtgatepb.Session{Options: &querypb.ExecuteOptions{
		IncludedFields: querypb.ExecuteOptions_ALL,
	}}, target, nil, executeSQL, nil, 0)
	require.NoError(t, err)
	for _, field := range res.Fields {
...
func TestDatabaseNameReplaceByKeyspaceNameReserveBeginExecuteMethod(t *testing.T) {
	...
	state, res, err := tsv.ReserveBeginExecute(ctx, &vtgatepb.Session{Options: &querypb.ExecuteOptions{
		IncludedFields: querypb.ExecuteOptions_ALL,
	}}, target, nil, nil, executeSQL, nil)
	require.NoError(t, err)
	for _, field := range res.Fields {
		require.Equal(t, "keyspaceName", field.Database)
	}
