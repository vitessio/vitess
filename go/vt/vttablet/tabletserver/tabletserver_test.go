...
func TestTabletServerCreateTransaction(t *testing.T) {
	...
	db.AddQueryPattern(fmt.Sprintf("insert into _vt\\.dt_state\\(dtid, state, time_created\\) values \\(X'6161', %d,.*", int(querypb.TransactionState_PREPARE)), &sqltypes.Result{})
	db.AddQueryPattern("insert into _vt\\.dt_participant\\(dtid, id, keyspace, shard\\) values \\(X'6161', 1,.*", &sqltypes.Result{})
...
func TestTabletServerStartCommit(t *testing.T) {
	...
	commitTransition := fmt.Sprintf("update _vt.dt_state set state = %d where dtid = X'6161' and state = %d", int(querypb.TransactionState_COMMIT), int(querypb.TransactionState_PREPARE))
...
func TestTabletserverSetRollback(t *testing.T) {
	...
	rollbackTransition := fmt.Sprintf("update _vt.dt_state set state = %d where dtid = X'6161' and state = %d", int(querypb.TransactionState_ROLLBACK), int(querypb.TransactionState_PREPARE))
...
func TestTabletServerReadTransaction(t *testing.T) {
	...
	db.AddQuery("select dtid, state, time_created from _vt.dt_state where dtid = X'6161'", &sqltypes.Result{})
	...
	db.AddQuery("select dtid, state, time_created from _vt.dt_state where dtid = X'6161'", txResult)
	db.AddQuery("select keyspace, shard from _vt.dt_participant where dtid = X'6161'", &sqltypes.Result{
		Fields: []*querypb.Field{
			{Type: sqltypes.VarBinary},
			{Type: sqltypes.VarBinary},
		},
...
func TestTabletServerConcludeTransaction(t *testing.T) {
	...
	db.AddQuery("delete from _vt.dt_state where dtid = X'6161'", &sqltypes.Result{})
	db.AddQuery("delete from _vt.dt_participant where dtid = X'6161'", &sqltypes.Result{})
