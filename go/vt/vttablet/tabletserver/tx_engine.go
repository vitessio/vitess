...
func (te *TxEngine) ReserveBegin(ctx context.Context, options *querypb.ExecuteOptions, preQueries []string) (int64, string, error) {
	...
	parseMode, setsSQLMode, err := planbuilder.ValidateReservedSettings(preQueries, te.env.Environment().Parser())
	if err != nil {
		return 0, "", err
	}
	err = te.isTxPoolAvailable(te.beginRequests.Add)
	...
	conn, err := te.reserve(ctx, options, preQueries, parseMode, setsSQLMode)
	if err != nil {
		return 0, "", err
	}
	defer conn.UnlockUpdateTime()
...
func (te *TxEngine) Reserve(ctx context.Context, options *querypb.ExecuteOptions, txID int64, preQueries []string) (int64, error) {
	...
	err = te.taintConn(ctx, conn, preQueries, parseMode, setsSQLMode)
	if err != nil {
		return 0, err
	}
	return conn.ReservedID(), nil
...
// Reserve creates a reserved connection and returns the id to it
func (te *TxEngine) reserve(ctx context.Context, options *querypb.ExecuteOptions, preQueries []string, parseMode sqlmode.Mode, setsSQLMode bool) (*StatefulConnection, error) {
...
// taintConn taints the connection and runs the pre-queries on it. The lexer modes are
// recorded only when the pre-queries assign sql_mode: an existing transaction's
// connection may already be in a mode a SET inside the transaction put it in, which
// pre-queries that leave sql_mode alone must not reset.
func (te *TxEngine) taintConn(ctx context.Context, conn *StatefulConnection, preQueries []string, parseMode sqlmode.Mode, setsSQLMode bool) error {
...
// ConnParseSQLMode returns the lexer modes recorded on the given stateful
// connection, or 0 when the connection is unknown.
func (te *TxEngine) ConnParseSQLMode(connID int64) sqlmode.Mode {
