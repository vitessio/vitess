...
func TestMaxRows(t *testing.T) {
	...
	_, err := client.Execute(`delete from vitess_a`, nil)
	require.NoError(t, err)
	...
	_, err = client.Execute(`select * from vitess_a`, nil)
	...
	_, err = client.ExecuteWithOptions(`select * from vitess_a`, nil, inDMLExecOption)
