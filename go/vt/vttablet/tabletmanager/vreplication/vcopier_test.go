...
func testPlayerCopyCharPK(t *testing.T) {
	...
	expectNontxQueries(t, qh.Expect(
		...
		"/delete cs, ctp from _vt.copy_state as cs left join _vt.copy_table_post as ctp on cs.vrepl_id=ctp.vrepl_id and cs.table_name=ctp.table_name.*dst",
...
func testPlayerCopyTablesWithFK(t *testing.T) {
	...
	expectDBClientQueries(t, qh.Expect(
		...
		"/delete cs, ctp from _vt.copy_state as cs left join _vt.copy_table_post as ctp on cs.vrepl_id=ctp.vrepl_id and cs.table_name=ctp.table_name.*dst1",
		...
		// copy of dst1 is done: delete from copy_state.
		"/delete cs, ctp from _vt.copy_state as cs left join _vt.copy_table_post as ctp on cs.vrepl_id=ctp.vrepl_id and cs.table_name=ctp.table_name.*dst2",
...
func testPlayerCopyTables(t *testing.T) {
	...
	expectDBClientQueries(t, qh.Expect(
		...
		"/delete cs, ctp from _vt.copy_state as cs left join _vt.copy_table_post as ctp on cs.vrepl_id=ctp.vrepl_id and cs.table_name=ctp.table_name.*yes",
		// All tables copied. Final catch up followed by Running state.
		"/update _vt.vreplication set state='Running'",
	))
	expectData(t, "dst1", [][]string{
...
func testPlayerCopyWildcardRule(t *testing.T) {
	...
	expectNontxQueries(t, qh.Expect(
		...
		// Wrap-up.
		"/delete cs, ctp from _vt.copy_state as cs left join _vt.copy_table_post as ctp on cs.vrepl_id=ctp.vrepl_id and cs.table_name=ctp.table_name.*src",
		// Copy is done. Go into running state.
		"/update _vt.vreplication set state='Running'",
	)))
...
func testPlayerCopyTableContinuation(t *testing.T) {
	...
	expectNontxQueries(t, qh.Expect(
		...
		"/delete cs, ctp from _vt.copy_state as cs left join _vt.copy_table_post as ctp on cs.vrepl_id=ctp.vrepl_id and cs.table_name=ctp.table_name.*not_copied",
		"/update _vt.vreplication set state='Running'",
	)))
