...
func newController(ctx context.Context, params map[string]string, dbClientFactory func() binlogplayer.DBClient, mysqld mysqlctl.MysqlDaemon, ts *topo.Server, cell string, blpStats *binlogplayer.Stats, vre *Engine, tpo discovery.TabletPickerOptions) (*controller, error) {
	...
	log.InfoS(fmt.Sprintf("creating controller with id: %v, name: %v, cell: %v, tabletTypes: %v", ct.id, ct.workflow, cell, tabletTypesStr))
	...
	if ct.source.GetExternalMysql() == "" {
		...
		log.InfoS(fmt.Sprintf("creating tablet picker for source keyspace/shard %v/%v with cell: %v and tabletTypes: %v", ct.source.Keyspace, ct.source.Shard, cell, tabletTypesStr))
...
func (ct *controller) run(ctx context.Context) {
	defer func() {
		log.InfoS(fmt.Sprintf("stream %v: stopped", ct.id))
		close(ct.done)
	}()
	...
	for {
		...
		select {
		case <-ctx.Done():
			log.WarnS("context canceled: " + err.Error())
			return
		default:
		}
...
func (ct *controller) runBlp(ctx context.Context) (err error) {
	defer func() {
		ct.sourceTablet.Store(&topodatapb.TabletAlias{})
		if x := recover(); x != nil {
			log.ErrorS(fmt.Sprintf("stream %v: caught panic: %v\n%s", ct.id, x, tb.Stack(4)))
	...
	case ct.source.Filter != nil:
		...
		if (err != nil && vr.WorkflowSubType == int32(binlogdatapb.VReplicationWorkflowSubType_AtomicCopy) && vr.state == binlogdatapb.VReplicationWorkflowState_Copying) ||
			...
			if errSetState := vr.setState(binlogdatapb.VReplicationWorkflowState_Error, err.Error()); errSetState != nil {
				log.ErrorS(fmt.Sprintf("INTERNAL: unable to setState() in controller: %v. Could not set error text to: %v.", errSetState, err))
				return err // yes, err and not errSetState.
			}
			log.ErrorS(fmt.Sprintf("vreplication stream %d going into error state due to %+v", ct.id, err))
...
func (ct *controller) pickSourceTablet(ctx context.Context, dbClient binlogplayer.DBClient) (*topodatapb.Tablet, error) {
	...
	log.InfoS(fmt.Sprintf("Trying to find an eligible source tablet for vreplication stream id %d for workflow: %s", ct.id, ct.workflow))
	...
	log.InfoS(fmt.Sprintf("Found eligible source tablet %s for vreplication stream id %d for workflow %s", tablet.Alias.String(), ct.id, ct.workflow))
