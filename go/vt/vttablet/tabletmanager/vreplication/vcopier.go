...
func (vc *vcopier) initTablesForCopy(ctx context.Context) error {
	...
	if len(plan.TargetTables) != 0 {
		...
		buf.WriteString(fmt.Sprintf("insert into %s.copy_state(vrepl_id, table_name) values ", sidecardb.GetIdentifier()))
		prefix := ""
...
func (vc *vcopier) copyNext(ctx context.Context, settings binlogplayer.VRSettings) error {
	qr, err := vc.vr.dbClient.Execute(fmt.Sprintf("select table_name, lastpk from %s.copy_state where vrepl_id = %d and id in (select max(id) from %s.copy_state group by vrepl_id, table_name)",
		sidecardb.GetIdentifier(), vc.vr.id, sidecardb.GetIdentifier()))
	if err != nil {
		return err
	}
...
func (vc *vcopier) copyTable(ctx context.Context, tableName string, copyState map[string]*sqltypes.Result) error {
			...
			case <-copyStateGCTicker.C:
				...
				go func() {
					gcQuery := fmt.Sprintf("delete from %s.copy_state where vrepl_id = %d and table_name = %s and id < (select maxid from (select max(id) as maxid from %s.copy_state where vrepl_id = %d and table_name = %s) as depsel)",
						sidecardb.GetIdentifier(), vc.vr.id, encodeString(tableName), sidecardb.GetIdentifier(), vc.vr.id, encodeString(tableName))
	...
	buf.Myprintf(
		...
		sidecardb.GetIdentifier(), copyStateTableName,
		sidecardb.GetIdentifier(), postCopyActionTableName,
		...
		case vcopierCopyTaskInsertCopyState:
			advanceFn = func(ctx context.Context, args *vcopierCopyTaskArgs) error {
				if err := vbc.insertCopyState(ctx, args.lastpk); err != nil {
					return vterrors.Wrapf(err, "error updating copy_state")
				}
				return nil
			}
		case vcopierCopyTaskCommit:
