...
type tableCopyInfo struct {
	...
	updateCopyState *sqlparser.ParsedQuery
...
func (vc *parallelVcopier) copyNext(ctx context.Context, settings binlogplayer.VRSettings) error {
	...
	return vc.copyTables(ctx, tablesToCopy, copyState)
}
...
func (vc *parallelVcopier) catchup(ctx context.Context, copyState map[string]*sqltypes.Result) error {
...
func (vc *parallelVcopier) copyTables(ctx context.Context, tableNames []string, copyState map[string]*sqltypes.Result) error {
	defer vc.vr.dbClient.Rollback()
	...
	err = vc.vr.sourceVStreamer.VStreamRowsParallel(ctx, queries, lastpkpbs, func(rows *binlogdatapb.VStreamRowsResponse) error {
		...
		_, err = tc.tablePlan.applyBulkInsert(&tc.sqlbuffer, rows, func(sql string) (*sqltypes.Result, error) {
			start := time.Now()
		...
		updateState, err := tc.updateCopyState.GenerateQuery(tc.bv, nil)
		if err != nil {
			return err
		}
	...
	case <-ctx.Done():
		for _, tc := range tableCopyInfoMap {
			log.Infof("Copy of %v stopped at lastpk: %v", tc.tableName, tc.bv)
		}
		return nil
	default:
	}
	...
	for _, tc := range tableCopyInfoMap {
		...
		buf := sqlparser.NewTrackedBuffer(nil)
		buf.Myprintf("delete from _vt.copy_state where vrepl_id=%s and table_name=%s", strconv.Itoa(int(vc.vr.id)), encodeString(tc.tableName))
		if _, err := vc.vr.dbClient.Execute(buf.String()); err != nil {
			return err
		}
	}
