...
func TestJournalOneToOne(t *testing.T) {
	...
	expectDBClientQueries(t, qh.Expect(
		...
		"/update _vt.vreplication set state='Running', message=left\\('', 1000\\), time_updated=\\d+ where id.*",
	))
...
func TestJournalTableMixed(t *testing.T) {
	...
	expectDBClientQueries(t, qh.Expect(
		"/update _vt.vreplication set state='Stopped', message=left\\('unable to handle journal event: tables were partially matched', 1000\\), time_updated=\\d+ where id",
	))
