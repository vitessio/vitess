...
func registerVReplicationFlags(fs *pflag.FlagSet) {
	...
	fs.DurationVar(&vplayerProgressDeadline, "vplayer-progress-deadline", vplayerProgressDeadline, "At what point, without having been able successfully replicate a pending batch of events, should we consider the vplayer stalled; then producing an error and log message and restarting the player.")
}
