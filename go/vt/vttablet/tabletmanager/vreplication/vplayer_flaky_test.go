...
// images are applied correctly when the target table has columns that are not
// sourced from a streamed column, e.g. a Materialize filter that adds a
// constant. The DataColumns bitmap sent by the vstreamer describes the streamed
// columns, so the vplayer has to map it onto its target column expressions when
// generating partial queries. The source JSON column is not part of the filter,
// but a partial JSON update to it still marks the row event as partial.
func TestPlayerPartialImagesWithTargetOnlyExpressions(t *testing.T) {
	...
	execStatements(t, []string{
		...
		fmt.Sprintf("create table %s.dst (id int, val varbinary(128), c int, primary key(id))", vrepldb),
	})
	defer execStatements(t, []string{
		"drop table src",
		fmt.Sprintf("drop table %s.dst", vrepldb),
	...
	filter := &binlogdatapb.Filter{
		Rules: []*binlogdatapb.Rule{{
			Match:  "dst",
			Filter: "select id, val, 1 as c from src",
		}},
	}
	bls := &binlogdatapb.BinlogSource{
		Keyspace: env.KeyspaceName,
	...
	testCases := []struct {
	...
	}{
		{
			...
			output: []string{
				"insert into dst(id,val,c) values (1,_binary'aaa',1)",
			},
			data: [][]string{
				{"1", "aaa", "1"},
			},
		},
		{
			...
			// still a partial one; the constant target column must still be set.
			input: `update src set jd=JSON_SET(jd, '$.color', 'red') where id = 1`,
			output: []string{
				"update dst set val=_binary'aaa', c=1 where id=1",
			},
			data: [][]string{
				{"1", "aaa", "1"},
			},
		},
		{
			...
			output: []string{
				"update dst set val=_binary'bbb', c=1 where id=1",
			},
			data: [][]string{
				{"1", "bbb", "1"},
			},
...
func TestPlayerDDL(t *testing.T) {
	...
	expectDBClientQueries(t, qh.Expect(
		...
		"/update _vt.vreplication set state='Error', message=left\\('terminal error: unrecoverable: error applying event: Duplicate",
	))
	cancel()
