...
func newUnstartedReplicationTM(t *testing.T, mysqlMode topodatapb.TabletMySQLMode) *TabletManager {
	...
	fakeDb := newTestMysqlDaemon(t, 1)
	tm := newTestReplicationTM(&topodatapb.Tablet{
		Alias: &topodatapb.TabletAlias{Cell: "cell1", Uid: 100},
	}, fakeDb, nil)
	tm._waitForGrantsComplete = make(chan struct{})
	tm.mysqlMode = mysqlMode
	tm.QueryServiceControl = tabletservermock.NewController()
	tm.SemiSyncMonitor = semisyncmonitor.CreateTestSemiSyncMonitor(fakeDb.DB(), exporter)
	return tm
}
...
func TestReparentingDisabled(t *testing.T) {
	old := mysqlctl.DisableActiveReparents
	t.Cleanup(func() { mysqlctl.DisableActiveReparents = old })
	require.False(t, old, "test assumes the global is unset")
	...
	tests := []struct {
		...
		{"unmanaged tablet leaves replication alone", topodatapb.TabletMySQLMode_UNMANAGED, false, true},
		{"managed tablet still manages replication", topodatapb.TabletMySQLMode_MANAGED, false, false},
		{"managed tablet honours global disable", topodatapb.TabletMySQLMode_MANAGED, true, true},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			mysqlctl.DisableActiveReparents = tt.disabled
			tm := &TabletManager{mysqlMode: tt.mode}
