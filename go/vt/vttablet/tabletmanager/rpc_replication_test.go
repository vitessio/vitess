...
func TestSetReplicationSourceConfiguration(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		{name: "read_error", heartbeat: 30, readErr: errors.New("heartbeat unavailable"), calls: []string{config}, wantError: "read replication heartbeat: heartbeat unavailable"},
		{name: "not_a_replica", heartbeat: 30, readErr: mysql.ErrNotReplica, calls: []string{config}, wantError: "read replication heartbeat: no replication status"},
	} {
		t.Run(tt.name, func(t *testing.T) {
			...
			for _, call := range tt.calls {
				switch call {
				case config:
					calls = append(calls, daemon.EXPECT().ReplicationHeartbeat(ctx).Return(tt.configured, int32(60), tt.readErr))
				case status:
