...
func (ts *tmState) RefreshFromTopo(ctx context.Context) error {
	...
	defer span.Finish()
	log.InfoS("Refreshing from Topo")
...
func (ts *tmState) ChangeTabletType(ctx context.Context, tabletType topodatapb.TabletType, action DBAction) error {
	ts.mu.Lock()
	defer ts.mu.Unlock()
	log.InfoS(fmt.Sprintf("Changing Tablet Type: %v for %s", tabletType, ts.tablet.Alias.String()))
		...
		if err != nil {
			...
			for {
				...
				if ti.Type == tabletType && proto.Equal(ti.PrimaryTermStartTime, primaryTermStartTime) {
					log.InfoS("Tablet record in toposerver matches, continuing operation")
					break
				}
				log.ErrorS("Tablet record read from toposerver does not match what we attempted to write, canceling operation")
				return err
			}
		}
	}
...
func (ts *tmState) ChangeTabletTags(ctx context.Context, tabletTags map[string]string) {
	ts.mu.Lock()
	defer ts.mu.Unlock()
	log.InfoS(fmt.Sprintf("Changing Tablet Tags: %v for %s", tabletTags, ts.tablet.Alias.String()))
...
func (ts *tmState) updateLocked(ctx context.Context) error {
	...
	if reason != "" {
		log.InfoS(fmt.Sprintf("Disabling query service: %v", reason))
...
func (ts *tmState) applyDenyList(ctx context.Context) (err error) {
	...
	if len(deniedTables) > 0 {
		...
		if len(tables) > 0 {
			log.InfoS(fmt.Sprintf("Denying tables %v", strings.Join(tables, ", ")))
	...
	if loadRuleErr != nil {
		log.WarnS(fmt.Sprintf("Fail to load query rule set %s: %s", denyListQueryList, loadRuleErr))
	}
	return nil
}

func (ts *tmState) publishStateLocked(ctx context.Context) {
	log.InfoS(fmt.Sprintf("Publishing state: %v", ts.tablet))
	...
	if err != nil {
		if topo.IsErrType(err, topo.NoNode) { // Someone deleted the tablet record under us. Shut down gracefully.
			log.ErrorS("Tablet record has disappeared, shutting down")
			servenv.ExitChan <- syscall.SIGTERM
			return
		}
		log.ErrorS(fmt.Sprintf("Unable to publish state to topo, will keep retrying: %v", err))
...
func (ts *tmState) retryPublish() {
	...
	for {
		...
		log.InfoS(fmt.Sprintf("Published state: %v", ts.tablet))
		return
	}
}
