...
func TestStartBuildTabletFromInput(t *testing.T) {
	...
	wantTablet := &topodatapb.Tablet{
		...
		DbNameOverride:       "aa",
		DefaultConnCollation: uint32(collations.Local().DefaultConnectionCharset()),
	}

	gotTablet, err := BuildTabletFromInput(alias, port, grpcport, nil, collations.Local())
	...
	initKeyspace = ""
	initShard = "0"
	_, err = BuildTabletFromInput(alias, port, grpcport, nil, collations.Local())
