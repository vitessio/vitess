...
func newMergeSorter(participants map[string]*shardStreamer, comparePKs []compareColInfo) *engine.MergeSort {
	...
	for i, cpk := range comparePKs {
		...
		if cpk.collation == nil {
			ob[i] = engine.OrderByParams{Col: cpk.colIndex, WeightStringCol: weightStringCol, Type: -1, CollationID: collations.CollationBinaryID}
		} else {
			ob[i] = engine.OrderByParams{Col: cpk.colIndex, WeightStringCol: weightStringCol, Type: -1, CollationID: cpk.collation.ID()}
		}
	}
