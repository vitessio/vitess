	...
	tests := []struct {
		...
		},
		{
			...
			args: args{
				summary: &Summary{
					RowsCompared: 33,
					StartedAt:    now.Add(-10 * time.Second).UTC().Format(TimestampFormat),
				},
				rowsToCompare: 100,
			},
			want: &ProgressReport{
				Percentage: 33,
				ETA:        now.Add(20 * time.Second).UTC().Format(TimestampFormat),
			},
		},
		{
			name: "half way",
			args: args{
				summary: &Summary{
					...
					StartedAt:    now.Add(-10 * time.Hour).UTC().Format(TimestampFormat),
				},
				rowsToCompare: 10000000000,
			},
			want: &ProgressReport{
				Percentage: 50,
				ETA:        now.Add(10 * time.Hour).UTC().Format(TimestampFormat),
			},
		},
		{
			name: "full progress",
			args: args{
				summary: &Summary{
					RowsCompared: 100,
					CompletedAt:  now.UTC().Format(TimestampFormat),
				},
				rowsToCompare: 100,
			},
			want: &ProgressReport{
				Percentage: 100,
				ETA:        now.UTC().Format(TimestampFormat),
			},
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.want.ETA != "" {
				...
				wantTime, err := time.Parse(TimestampFormat, tt.want.ETA)
				require.NoError(t, err)
				var timeDiff float64
				if tt.want.Percentage == 100 {
					completedTime, err := time.Parse(TimestampFormat, tt.args.summary.CompletedAt)
					require.NoError(t, err)
					timeDiff = math.Abs(completedTime.Sub(wantTime).Seconds())
				} else {
					startTime, err := time.Parse(TimestampFormat, tt.args.summary.StartedAt)
					require.NoError(t, err)
					completedTimeUnix := float64(now.UTC().Unix()-startTime.UTC().Unix()) * (100 / tt.want.Percentage)
					estimatedTime, err := time.Parse(TimestampFormat, tt.want.ETA)
					require.NoError(t, err)
