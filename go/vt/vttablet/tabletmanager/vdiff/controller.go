...
// markStoppedByRequest records the fact that this VDiff was stopped via user request
...
func (ct *controller) markStoppedByRequest() {
	dbClient := ct.vde.dbClientFactoryFiltered()
	if err := dbClient.Connect(); err != nil {
		log.Errorf("Encountered an error marking vdiff %s as stopped by request: %v", ct.uuid, err)
		return
	}
	defer dbClient.Close()
