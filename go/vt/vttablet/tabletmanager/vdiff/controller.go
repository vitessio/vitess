...
// recordStoppedByRequest records the fact that this VDiff was stopped via user request
// and resets the error generated by cancelling the context to stop it:
//   "vttablet: rpc error: code = Canceled desc = context canceled"
// This differentiates non-user requested stops that would occur e.g. during PlannedReparentShard
// or tablet restart, in those cases the error will be saved and will cause the VDiff to be
// retried ASAP -- which is NOT what we want here.
func (ct *controller) recordStoppedByRequest() {
	dbClient := ct.vde.dbClientFactoryFiltered()
	if err := dbClient.Connect(); err != nil {
		log.Errorf("Encountered an error marking vdiff %s as stopped: %v", ct.uuid, err)
		return
	}
	defer dbClient.Close()
	...
	insertVDiffLog(ct.vde.ctx, dbClient, ct.id, fmt.Sprintf("State changed to: %s", StoppedState))
}
