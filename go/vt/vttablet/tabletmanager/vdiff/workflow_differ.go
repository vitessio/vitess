...
func (wd *workflowDiffer) reconcileExtraRows(dr *DiffReport, maxExtraRowsToCompare int64, maxVdiffReportRows uint64) error {
	...
	// We can now trim the extra rows diffs on both sides to the maxVdiffReportRows value
	if uint64(len(dr.ExtraRowsSourceDiffs)) > maxVdiffReportRows && maxVdiffReportRows > 0 {
		dr.ExtraRowsSourceDiffs = dr.ExtraRowsSourceDiffs[:maxVdiffReportRows-1]
	}
	if uint64(len(dr.ExtraRowsTargetDiffs)) > maxVdiffReportRows && maxVdiffReportRows > 0 {
		dr.ExtraRowsTargetDiffs = dr.ExtraRowsTargetDiffs[:maxVdiffReportRows-1]
	}
...
func (wd *workflowDiffer) diffTable(ctx context.Context, dbClient binlogplayer.DBClient, td *tableDiffer) error {
	...
	dr, err := td.diff(ctx, wd.opts.CoreOptions.MaxRows, wd.opts.ReportOptions.DebugQuery, wd.opts.ReportOptions.OnlyPks, wd.opts.CoreOptions.MaxExtraRowsToCompare, wd.opts.ReportOptions.MaxVdiffReportRows)
	if err != nil {
	...
	if dr.ExtraRowsSource > 0 || dr.ExtraRowsTarget > 0 {
		if err := wd.reconcileExtraRows(dr, wd.opts.CoreOptions.MaxExtraRowsToCompare, wd.opts.ReportOptions.MaxVdiffReportRows); err != nil {
