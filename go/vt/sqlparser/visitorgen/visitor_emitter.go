...
// EmitTypeRewriteSwitches is an anti-parser (a.k.a prettifier) - it takes a struct that is much like an AST,
// and produces a string from it. This method will produce the switch cases needed to cover the Vitess AST.
func EmitTypeRewriteSwitches(vd *VisitorPlan) string {
	var sb builder
	for _, s := range vd.Switches {
		sb.newLine()
...
func EmitTypeVisitSwitches(vd *VisitorPlan) string {
	var sb builder
	for _, s := range vd.Switches {
		sb.newLine()
		sb.appendF("\t\tcase %s:", s.Type.toTypString())
		for _, k := range s.Fields {
			sb.appendF(k.asWalkerVisit())
		}
	}
