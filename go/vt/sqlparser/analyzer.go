...
func NewPlanValue(node Expr) (sqltypes.PlanValue, error) {
	...
	case *SQLVal:
		...
		case IntVal:
			...
			if err != nil {
				return sqltypes.PlanValue{}, vterrors.Errorf(vtrpc.Code_INVALID_ARGUMENT, "could not create integral from val %v: %v", string(node.Val), err)
			}
		...
		case HexVal:
			...
			if err != nil {
				return sqltypes.PlanValue{}, vterrors.Errorf(vtrpc.Code_INVALID_ARGUMENT, "could not hex decode: %v", err)
			}
	...
	case ValTuple:
		...
		for _, val := range node {
			...
			if innerpv.ListKey != "" || innerpv.Values != nil {
				return sqltypes.PlanValue{}, vterrors.New(vtrpc.Code_UNIMPLEMENTED, "unsupported: nested lists")
			}
	...
	return sqltypes.PlanValue{}, vterrors.Errorf(vtrpc.Code_INVALID_ARGUMENT, "expression is too complex '%v'", String(node))
}
