...
func TestFindColNames(t *testing.T) {
	...
	require.NoError(t, err)
	RewriteWithPath(ast, nil, func(cursor *Cursor) bool {
		if _, isColName := cursor.Node().(*ColName); isColName {
			// TODO: actually assert something here
			fmt.Println(cursor.CurrentPath().DebugString())
		}
		return true
	})
}
