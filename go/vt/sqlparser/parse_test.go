...
// TestFailsMySQLReserved ensures that all MySQL's reserved keywords should not correctly parse (without quotes)
func TestFailsMySQLReserved(t *testing.T) {
	aliasTest := "SELECT 1 as %s"
	iTest := "INSERT INTO t (%s) VALUES (1)"
	dTest := "DELETE FROM t where %s=1"
	uTest := "UPDATE t SET %s=1"
...
// TestPassesMySQLNonReserved ensures that all MySQL's non-reserved keywords should correctly parse (without quotes)
func TestPassesMySQLNonReserved(t *testing.T) {
	aliasTest := "SELECT 1 as %s"
	iTest := "INSERT INTO t (%s) VALUES (1)"
	dTest := "DELETE FROM t where %s=1"
	uTest := "UPDATE t SET %s=1"
	...
	for _, kw := range mysqlNonReserved {
		for _, query := range tests {
			test := fmt.Sprintf(query, kw)
			t.Run(test, func(t *testing.T) {
				_, err := Parse(test)
