...
type VSchemaViews interface {
	FindView(name TableName) TableSubquery
}
...
func (er *astRewriter) rewriteAliasedTable(cursor *Cursor, node *AliasedTableExpr) {
	...
	node.Expr = &DerivedTable{Select: CloneTableSubquery(view)} // TODO: this is a bit hacky. We want to update the schema def so it contains new types
	if node.As.IsEmpty() {
