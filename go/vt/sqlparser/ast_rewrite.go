...
func (a *application) rewriteRefOfJSONAggregateExpr(parent SQLNode, node *JSONAggregateExpr, replacer replacerFunc) bool {
	if node == nil {
		return true
	}
	if a.pre != nil {
	...
	if !a.rewriteColIdent(node, node.Name, func(newNode, parent SQLNode) {
		parent.(*JSONAggregateExpr).Name = newNode.(ColIdent)
	}) {
		return false
	}
	for x, el := range node.Columns {
		if !a.rewriteRefOfColName(node, el, func(idx int) replacerFunc {
			return func(newNode, parent SQLNode) {
				parent.(*JSONAggregateExpr).Columns[idx] = newNode.(*ColName)
			}
		}(x)) {
			return false
		}
