...
yydefault:
	...
	case 154:
		...
		//line sql.y:851
		{
			if num, ok := yyDollar[2].valExpr.(IntVal); ok {
				yyVAL.valExpr = num
			} else {
		...
		//line sql.y:859
		{
			if num, ok := yyDollar[2].valExpr.(IntVal); ok {
				...
				if num[0] == '-' {
					...
					yyVAL.valExpr = append(IntVal("-"), num...)
				}
			} else {
	...
	case 181:
		...
		//line sql.y:990
		{
			yyVAL.valExpr = IntVal(yyDollar[1].bytes)
		}
	case 182:
		yyDollar = yyS[yypt-1 : yypt+1]
		//line sql.y:994
		{
			yyVAL.valExpr = FloatVal(yyDollar[1].bytes)
		}
	case 183:
		yyDollar = yyS[yypt-1 : yypt+1]
		//line sql.y:998
