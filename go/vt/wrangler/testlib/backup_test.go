...
func TestDoNotDemotePrimaryAfterBackup(t *testing.T) {
	delay := discovery.GetTabletPickerRetryDelay()
	defer func() {
		discovery.SetTabletPickerRetryDelay(delay)
	}()
	...
	// run the backup
	require.NoError(t, vp.Run([]string{"Backup", "--allow_primary", topoproto.TabletAliasString(primary.Tablet.Alias)}))

	assert.Equal(t, topodatapb.TabletType_PRIMARY, primary.Tablet.Type)
	require.NoError(t, primary.FakeMysqlDaemon.CheckSuperQueryList())
}
