...
func pkColsToGroupByParams(pkCols []int) []*engine.GroupByParams {
	...
	for _, col := range pkCols {
		res = append(res, &engine.GroupByParams{KeyCol: col, WeightStringCol: -1, Type: -1})
	}
	return res
}
...
func newMergeSorter(participants map[string]*shardStreamer, comparePKs []compareColInfo) *engine.MergeSort {
	...
	for _, cpk := range comparePKs {
		...
		if cpk.collation == nil {
			ob = append(ob, engine.OrderByParams{Col: cpk.colIndex, WeightStringCol: weightStringCol, Type: -1, CollationID: collations.CollationBinaryID})
		} else {
			ob = append(ob, engine.OrderByParams{Col: cpk.colIndex, WeightStringCol: weightStringCol, Type: -1, CollationID: cpk.collation.ID()})
		}
	}
