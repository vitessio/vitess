...
func (wr *Wrangler) NewVReplicationWorkflow(ctx context.Context, workflowType VReplicationWorkflowType,
	...
	log.InfoS(fmt.Sprintf("NewVReplicationWorkflow with params %+v", params))
	...
	log.InfoS(fmt.Sprintf("Workflow state is %+v", ws))
...
func (vrw *VReplicationWorkflow) initMoveTables() error {
	log.InfoS(fmt.Sprintf("In VReplicationWorkflow.initMoveTables() for %+v", vrw))
...
func (vrw *VReplicationWorkflow) initReshard() error {
	log.InfoS(fmt.Sprintf("In VReplicationWorkflow.initReshard() for %+v", vrw))
...
func (vrw *VReplicationWorkflow) switchReads() (*[]string, error) {
	log.InfoS(fmt.Sprintf("In VReplicationWorkflow.switchReads() for %+v", vrw))
...
func (vrw *VReplicationWorkflow) switchWrites() (*[]string, error) {
	...
	var dryRunResults *[]string
	var err error
	log.InfoS(fmt.Sprintf("In VReplicationWorkflow.switchWrites() for %+v", vrw))
	if vrw.params.Direction == workflow.DirectionBackward {
		keyspace := vrw.params.SourceKeyspace
		...
		log.InfoS(fmt.Sprintf("In VReplicationWorkflow.switchWrites(reverse) for %+v", vrw))
	}
	...
	log.InfoS(fmt.Sprintf("switchWrites succeeded with journal id %d", journalID))
	return dryRunResults, nil
}
...
func (vrw *VReplicationWorkflow) canSwitch(keyspace, workflowName string) (reason string, err error) {
	...
	if vrw.params.Direction == workflow.DirectionForward && ws.WritesSwitched ||
		...
		log.InfoS("writes already switched no need to check lag")
		return "", nil
	}
	log.InfoS(fmt.Sprintf("state:%s, direction %d, switched %t", vrw.CachedState(), vrw.params.Direction, ws.WritesSwitched))
...
func (wr *Wrangler) deleteWorkflowVDiffData(ctx context.Context, tablet *topodatapb.Tablet, workflow string) {
	if _, err := wr.tmc.VDiff(ctx, tablet, &tabletmanagerdatapb.VDiffRequest{
		...
		log.ErrorS(fmt.Sprintf("Error deleting vdiff data for %s.%s workflow: %v", tablet.Keyspace, workflow, err))
	}
}
...
func (wr *Wrangler) optimizeCopyStateTable(tablet *topodatapb.Tablet) {
	if wr.sem != nil {
		if !wr.sem.TryAcquire(1) {
			log.WarnS(fmt.Sprintf("Deferring work to optimize the copy_state table on %q due to hitting the maximum concurrent background job limit.", tablet.Alias.String()))
			return
		}
	}
	go func() {
		...
		if _, err := wr.tmc.ExecuteFetchAsAllPrivs(ctx, tablet, &tabletmanagerdatapb.ExecuteFetchAsAllPrivsRequest{
			...
			log.WarnS(fmt.Sprintf("Failed to optimize the copy_state table on %q: %v", tablet.Alias.String(), err))
		}
		...
		if _, err := wr.tmc.ExecuteFetchAsAllPrivs(ctx, tablet, &tabletmanagerdatapb.ExecuteFetchAsAllPrivsRequest{
			...
			log.WarnS(fmt.Sprintf("Failed to reset the auto_increment value for the copy_state table on %q: %v", tablet.Alias.String(), err))
		}
	}()
}
