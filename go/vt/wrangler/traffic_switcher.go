	...
	for _, servedType := range servedTypes {
		...
		for _, table := range ts.Tables() {
			if direction == workflow.DirectionForward {
				...
				toTarget := []string{sqlescape.EscapeID(ts.TargetKeyspaceName()) + "." + sqlescape.EscapeID(table)}
				rules[sqlescape.EscapeID(table)+"@"+tt] = toTarget
				rules[sqlescape.EscapeID(ts.TargetKeyspaceName())+"."+sqlescape.EscapeID(table)+"@"+tt] = toTarget
				rules[sqlescape.EscapeID(ts.SourceKeyspaceName())+"."+sqlescape.EscapeID(table)+"@"+tt] = toTarget
			} else {
				...
				rules[sqlescape.EscapeID(ts.TargetKeyspaceName())+"."+sqlescape.EscapeID(table)+"@"+tt] = toSource
				rules[sqlescape.EscapeID(ts.SourceKeyspaceName())+"."+sqlescape.EscapeID(table)+"@"+tt] = toSource
			}
		}
	}
...
func (ts *trafficSwitcher) changeWriteRoute(ctx context.Context) error {
	...
	for _, table := range ts.tables {
		...
		ts.wr.Logger().Infof("Delete routing: %v", ts.TargetKeyspaceName()+"."+table)
		rules[sqlescape.EscapeID(table)] = []string{sqlescape.EscapeID(ts.TargetKeyspaceName()) + "." + sqlescape.EscapeID(table)}
		rules[sqlescape.EscapeID(ts.SourceKeyspaceName())+"."+sqlescape.EscapeID(table)] = []string{sqlescape.EscapeID(ts.TargetKeyspaceName()) + "." + sqlescape.EscapeID(table)}
		ts.wr.Logger().Infof("Add routing: %v %v", sqlescape.EscapeID(table), sqlescape.EscapeID(ts.SourceKeyspaceName())+"."+sqlescape.EscapeID(table))
	}
	if err := topotools.SaveRoutingRules(ctx, ts.TopoServer(), rules); err != nil {
		return err
	}
