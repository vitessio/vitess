...
func (wr *Wrangler) SwitchReads(ctx context.Context, targetKeyspace, workflowName string, servedTypes []topodatapb.TabletType,
	...
	log.InfoS(fmt.Sprintf("Switching reads: %s.%s tt %+v, cells %+v, workflow state: %+v", targetKeyspace, workflowName, servedTypes, cells, ws))
	...
	if journalsExist {
		log.InfoS(fmt.Sprintf("Found a previous journal entry for %d", ts.id))
	}
	var sw iswitcher
	if dryRun {
...
func (wr *Wrangler) areTabletsAvailableToStreamFrom(ctx context.Context, ts *trafficSwitcher, keyspace string, shards []*topo.ShardInfo) error {
	...
	if allErrors.HasErrors() {
		log.ErrorS(fmt.Sprintf("%s", allErrors.Error()))
...
func (wr *Wrangler) DropTargets(ctx context.Context, targetKeyspace, workflow string, keepData, keepRoutingRules, dryRun bool) (*[]string, error) {
	...
	if !keepData {
		...
		case binlogdatapb.MigrationType_TABLES:
			log.InfoS("Deleting target tables")
		...
		case binlogdatapb.MigrationType_SHARDS:
			log.InfoS("Removing target shards")
...
func (wr *Wrangler) finalizeMigrateWorkflow(ctx context.Context, targetKeyspace, workflow, tableSpecs string,
	...
	log.InfoS(fmt.Sprintf("cancel is %t, keepData %t", cancel, keepData))
	if cancel && !keepData {
		if err := sw.removeTargetTables(ctx); err != nil {
			return nil, err
		}
...
func (wr *Wrangler) getShardSubset(ctx context.Context, keyspace string, shardSubset []string) ([]string, error) {
	...
	log.InfoS(fmt.Sprintf("Selecting subset of shards in keyspace %s: %d from %d :: %+v", keyspace, len(shardSubset), len(allShards), shardSubset))
...
func (wr *Wrangler) buildTrafficSwitcher(ctx context.Context, targetKeyspace, workflowName string) (*trafficSwitcher, error) {
	...
	if err != nil {
		log.InfoS(fmt.Sprintf("Error building targets: %s", err))
		return nil, err
	}
	...
	log.InfoS(fmt.Sprintf("Migration ID for workflow %s: %d", workflowName, ts.id))
	sourceTopo := wr.ts
	...
	if ts.isPartialMigration {
		log.InfoS(fmt.Sprintf("Migration is partial, for shards %+v", sourceShards))
	}
	return ts, nil
}
...
func (ts *trafficSwitcher) switchTableReads(ctx context.Context, cells []string, servedTypes []topodatapb.TabletType, direction workflow.TrafficSwitchDirection) error {
	log.InfoS(fmt.Sprintf("switchTableReads: servedTypes: %+v, direction %v", servedTypes, direction))
	...
	for _, servedType := range servedTypes {
		...
		for _, table := range ts.Tables() {
			if direction == workflow.DirectionForward {
				log.InfoS("Route direction forward")
				...
				log.InfoS("Route direction backwards")
...
func (ts *trafficSwitcher) stopSourceWrites(ctx context.Context) error {
	...
	if err != nil {
		log.WarnS(fmt.Sprintf("Error: %s", err))
		return err
	}
...
func (ts *trafficSwitcher) changeTableSourceWrites(ctx context.Context, access accessType) error {
	...
	if err != nil {
		log.WarnS(fmt.Sprintf("Error in changeTableSourceWrites: %s", err))
		return err
	}
...
func (ts *trafficSwitcher) waitForCatchup(ctx context.Context, filteredReplicationWaitTime time.Duration) error {
	...
	if err := ts.ForAllUIDs(func(target *workflow.MigrationTarget, uid int32) error {
		...
		log.InfoS(fmt.Sprintf("After catchup: target keyspace:shard: %v:%v, source position %v, uid %d", ts.TargetKeyspaceName(), target.GetShard().ShardName(), source.Position, uid))
		...
		if _, err := ts.TabletManagerClient().VReplicationExec(ctx, target.GetPrimary().Tablet, binlogplayer.StopVReplication(uid, "stopped for cutover")); err != nil {
			log.InfoS(fmt.Sprintf("error marking stopped for cutover on %s, uid %d", target.GetPrimary().AliasString(), uid))
			return err
		}
		return nil
	}); err != nil {
...
func (ts *trafficSwitcher) createReverseVReplication(ctx context.Context) error {
	...
	err := ts.ForAllUIDs(func(target *workflow.MigrationTarget, uid int32) error {
		...
		log.InfoS(fmt.Sprintf("Creating reverse workflow vreplication stream on tablet %s: workflow %s, startPos %s for target %s:%s, uid %d", source.GetPrimary().Alias, ts.ReverseWorkflowName(), target.Position, ts.TargetKeyspaceName(), target.GetShard().ShardName(), uid))
		...
		if updateQuery != "" {
			log.InfoS(fmt.Sprintf("Updating vreplication stream entry on %s with: %s", source.GetPrimary().Alias, updateQuery))
...
func (ts *trafficSwitcher) createJournals(ctx context.Context, sourceWorkflows []string) error {
	log.InfoS(fmt.Sprintf("In createJournals for source workflows %+v", sourceWorkflows))
	return ts.ForAllSources(func(source *workflow.MigrationSource) error {
		if source.Journaled {
			return nil
		}
		...
		log.InfoS(fmt.Sprintf("Creating journal %v", journal))
