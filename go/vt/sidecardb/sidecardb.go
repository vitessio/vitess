...
// PrintCallerDetails is a temp helper for dev debugging
...
// hack to remove an extra DEFAULT CHARSET which we get from "show create table"
...
// finds diff that needs to be applied to current schema to get the desired one. Will be an empty string if they match
