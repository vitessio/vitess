...
func loadSchemaDefinitions(parser *sqlparser.Parser) {
	...
	err := fs.WalkDir(schemaLocation, ".", func(path string, entry fs.DirEntry, err error) error {
		...
		if !entry.IsDir() {
			...
			if !strings.HasSuffix(strings.ToLower(fname), sqlFileExtension) {
				log.InfoS(fmt.Sprintf("Ignoring non-SQL file: %s, found during sidecar database initialization", path))
				return nil
			}
	...
	if err != nil {
		log.ErrorS(fmt.Sprintf("error loading schema files: %+v", err))
	}
}
...
func printCallerDetails() {
	...
	if ok && details != nil {
		log.InfoS(fmt.Sprintf("%s schema init called from %s:%d\n", sidecar.GetName(), details.Name(), line))
	}
}
...
func Init(ctx context.Context, env *vtenv.Environment, exec Exec) error {
	...
	log.InfoS("Starting sidecardb.Init()")
...
func (si *schemaInit) doesSidecarDBExist() (bool, error) {
	...
	switch len(rs.Rows) {
	case 0:
		log.InfoS(fmt.Sprintf("doesSidecarDBExist: %s not found", sidecar.GetName()))
		return false, nil
	case 1:
		log.InfoS("doesSidecarDBExist: found " + sidecar.GetName())
		return true, nil
	default:
...
func (si *schemaInit) createSidecarDB() error {
	...
	log.InfoS("createSidecarDB: " + sidecar.GetName())
	return nil
}
...
func (si *schemaInit) getCurrentSchema(tableName string) (string, error) {
	...
	if err != nil {
		...
		log.ErrorS(fmt.Sprintf("Error getting table schema for %s: %+v", tableName, err))
		return "", err
	}
	if len(rs.Rows) > 0 {
...
func (si *schemaInit) findTableSchemaDiff(tableName, current, desired string) (string, error) {
	...
	if diff != nil {
		...
		if ddl == "" {
			log.InfoS("No changes needed for table " + tableName)
		} else {
			log.InfoS(fmt.Sprintf("Applying DDL for table %s:\n%s", tableName, ddl))
		}
	}
...
func (si *schemaInit) ensureSchema(table *sidecarTable) error {
	...
	if ddl != "" {
		...
		log.InfoS(fmt.Sprintf("Applied DDL %s for table %s during sidecar database initialization", ddl, table))
		ddlCount.Add(1)
		return nil
	}
	log.InfoS(fmt.Sprintf("Table schema was already up to date for the %s table in the %s sidecar database", table.name, sidecar.GetName()))
	return nil
}
