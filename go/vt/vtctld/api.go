...
func initAPI(ctx context.Context, ts topo.Server, actions *ActionRepository, realtimeStats *realtimeStats) {
	...
	handleCollection("tablet_statuses", func(r *http.Request) (interface{}, error) {
		...
		if targetPath == "" {
			...
			// Getting the aggregated data for the highest level view (keyspace=all, cell=all, types=all).
			if keyspace == "all" && cell == "all" && tabletType == "all" {
				heatmap, err := realtimeStats.aggregatedHeatmapData(metric)
				if err != nil {
	...
	handleCollection("tablet_health", func(r *http.Request) (interface{}, error) {
		...
		// Request was incorrectly formatted.
		if len(parts) != 2 {
			return nil, fmt.Errorf("invalid tablet_info path: %q  expected path: /tablet_info/<cell>/<uid>", tabletPath)
		}
