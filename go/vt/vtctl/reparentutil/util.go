	...
	for candidate, position := range validCandidates {
		...
		if !candidateInfo.IsGTIDBased {
			if candidateInfo.IsSemiSyncReplica {
				return nil, vterrors.Errorf(vtrpc.Code_FAILED_PRECONDITION, "candidate %v does not report MySQL GTID-based positions and has semi-sync enabled. Replicas without MySQL GTID-based positions are unsupported in a MySQL GTID-based shard", candidate)
			}
