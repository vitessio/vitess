...
func TestEmergencyReparenterLegacyFindErrantGTIDs(t *testing.T) {
	u1 := "00000000-0000-0000-0000-000000000001"
	u2 := "00000000-0000-0000-0000-000000000002"
	tests := []struct {
		name                     string
		...
		},
		{
			...
			wantedCandidates:         []string{"zone1-0000000102", "zone1-0000000104"},
			wantMostAdvancedPossible: []string{"zone1-0000000102"},
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			require.NoError(t, err)
			require.True(t, isGtid)
			candidates, err := erp.legacyFindErrantGTIDS(validCandidates, tt.statusMap)
			require.NoError(t, err)
			keys := make([]string, 0, len(candidates))
			for key := range candidates {
				keys = append(keys, key)
			...
			ers := EmergencyReparenter{logger: logutil.NewCallbackLogger(func(*logutilpb.Event) {})}
			winningPrimary, _, err := ers.findMostAdvanced(candidates, tt.tabletMap, EmergencyReparentOptions{durability: dp})
			if tt.wantMostAdvancedPossible == nil {
				require.Error(t, err)
				return
			}
			require.NoError(t, err)
