...
func (erp *EmergencyReparenter) findErrantGTIDs(
	...
	for _, candidate := range maxLenCandidates {
		...
		if errantGTIDs != nil {
			log.ErrorS(fmt.Sprintf("skipping %v with GTIDSet:%v because we detected errant GTIDs - %v", candidate, afterStatus.RelayLogPosition.GTIDSet, errantGTIDs))
			continue
		}
		maxLenPositions = append(maxLenPositions, candidatePositions.Combined)
		updatedValidCandidates[candidate] = validCandidates[candidate]
	...
	for alias, length := range reparentJournalLen {
		...
		if errantGTIDs != nil {
			log.ErrorS(fmt.Sprintf("skipping %v with GTIDSet:%v because we detected errant GTIDs - %v", alias, validCandidates[alias], errantGTIDs))
			continue
		}
