...
- ERS runs while the shard cannot accept writes, and its operation is time-bound (`--wait-replicas-timeout`): it must resolve the availability loss without open-ended waits, and new steps must not extend the total time envelope. Changes should prioritize reducing points of failure - avoid new RPCs or work that may delay or make ERS more brittle
