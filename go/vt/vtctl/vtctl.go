...
func commandCreateKeyspace(ctx context.Context, wr *wrangler.Wrangler, subFlags *pflag.FlagSet, args []string) error {
	...
	durabilityPolicy := subFlags.String("durability-policy", reparentutil.DurabilityNone, "Type of durability to enforce for this keyspace. Default is none. Possible values include 'semi_sync' and others as dictated by registered plugins.")
	...
	if ktype == topodatapb.KeyspaceType_SNAPSHOT {
		if *durabilityPolicy != reparentutil.DurabilityNone {
