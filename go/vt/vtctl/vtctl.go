...
func parseListFromString(listString string) []string {
	var list []string
	if listString != "" {
...
func commandWaitForDrain(ctx context.Context, wr *wrangler.Wrangler, subFlags *flag.FlagSet, args []string) error {
	cellsStr := subFlags.String("cells", "", "Specifies a comma-separated list of cells to look for tablets")
	...
	cells := parseListFromString(*cellsStr)
