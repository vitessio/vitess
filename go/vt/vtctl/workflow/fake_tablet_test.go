...
// The tests in this directory call workflow functions directly. So, there's
...
func newFakeTablet(t *testing.T, s *Server, cell string, uid uint32, tabletType topodatapb.TabletType, db *fakesqldb.DB, options ...TabletOption) *fakeTablet {
	if uid > 99 {
	...
	if err := s.ts.InitTablet(context.Background(), tablet, false /* allowPrimaryOverride */, true /* createShardAndKeyspace */, false /* allowUpdate */); err != nil {
...
func (ft *fakeTablet) StartActionLoop(t *testing.T, s *Server) {
	if ft.TM != nil {
...
func init() {
	...
	tabletconntest.SetProtocol("go.vt.vtctl.workflow.fake_tablet_test", "grpc")
	tmclienttest.SetProtocol("go.vt.vtctl.workflow.fake_tablet_test", "grpc")
}
