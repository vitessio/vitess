...
func TestTableMigrateMainflow(t *testing.T) {
	...
	checkCellRouting(t, s, "cell1", map[string][]string{
		"t1":     {"ks1.t1"},
		"ks2.t1": {"ks1.t1"},
		"t2":     {"ks1.t2"},
		"ks2.t2": {"ks1.t2"},
	...
	_, err := tme.s.switchReads(ctx, tme.targetKeyspace, "test", []topodatapb.TabletType{topodatapb.TabletType_RDONLY}, []string{"cell1"}, DirectionForward, false)
	if err != nil {
		t.Fatal(err)
	}
	checkCellRouting(t, tme.s, "cell1", map[string][]string{
		"t1":            {"ks1.t1"},
		"ks2.t1":        {"ks1.t1"},
		"t2":            {"ks1.t2"},
		"ks2.t2":        {"ks1.t2"},
	...
	checkCellRouting(t, tme.s, "cell2", map[string][]string{
		"t1":     {"ks1.t1"},
		"ks2.t1": {"ks1.t1"},
		"t2":     {"ks1.t2"},
		"ks2.t2": {"ks1.t2"},
	...
	_, err = tme.s.switchReads(ctx, tme.targetKeyspace, "test", []topodatapb.TabletType{topodatapb.TabletType_REPLICA}, []string{"cell2"}, DirectionForward, false)
	if err != nil {
		t.Fatal(err)
	}
	checkCellRouting(t, tme.s, "cell1", map[string][]string{
	...
	_, err = tme.s.switchReads(ctx, tme.targetKeyspace, "test", []topodatapb.TabletType{topodatapb.TabletType_REPLICA}, []string{"cell2"}, DirectionBackward, false)
	if err != nil {
		t.Fatal(err)
	}
	checkCellRouting(t, tme.s, "cell1", map[string][]string{
	...
	//-------------------------------------------------------------------------------------------------------------------
	// Switch all REPLICA.
	_, err = tme.s.switchReads(ctx, tme.targetKeyspace, "test", []topodatapb.TabletType{topodatapb.TabletType_REPLICA}, nil, DirectionForward, false)
	if err != nil {
		t.Fatal(err)
	}
	checkRouting(t, tme.s, map[string][]string{
		"t1":             {"ks1.t1"},
		"ks2.t1":         {"ks1.t1"},
		"t2":             {"ks1.t2"},
		"ks2.t2":         {"ks1.t2"},
	...
	_, err = tme.s.switchReads(ctx, tme.targetKeyspace, "test", []topodatapb.TabletType{topodatapb.TabletType_RDONLY}, nil, DirectionBackward, false)
	if err != nil {
		t.Fatal(err)
	}
	checkRouting(t, tme.s, map[string][]string{
	...
	//-------------------------------------------------------------------------------------------------------------------
	// All cells RDONLY backward migration.
	_, err = tme.s.switchReads(ctx, tme.targetKeyspace, "test", []topodatapb.TabletType{topodatapb.TabletType_REPLICA}, nil, DirectionBackward, false)
	if err != nil {
		t.Fatal(err)
	}
	checkRouting(t, tme.s, map[string][]string{
	...
	_, err = tme.s.switchReads(ctx, tme.targetKeyspace, "test", []topodatapb.TabletType{topodatapb.TabletType_PRIMARY}, nil, DirectionForward, false)
	...
	_, err = tme.s.switchReads(ctx, tme.targetKeyspace, "test", []topodatapb.TabletType{topodatapb.TabletType_RDONLY}, nil, DirectionForward, false)
	if err != nil {
		t.Fatal(err)
	}
	tme.expectNoPreviousJournals()
	...
	switchWrites(tme)
	_, _, err = tme.s.switchWrites(ctx, tme.targetKeyspace, "test", 0*time.Second, false, false, true, false)
	...
	waitForCatchup := func() {
		// mi.waitForCatchup-> mi.s.tmc.VReplicationWaitForPos
		...
		// mi.waitForCatchup-> mi.s.tmc.VReplicationExec('Stopped')
	...
	journalID, _, err := tme.s.switchWrites(ctx, tme.targetKeyspace, "test", 1*time.Second, false, false, true, false)
	if err != nil {
		t.Fatal(err)
	}
...
func TestShardMigrateMainflow(t *testing.T) {
	...
	waitForCatchup := func() {
		...
		// mi.waitForCatchup-> mi.s.tmc.VReplicationExec('stopped for cutover')
		tme.dbTargetClients[0].addQuery("select id from _vt.vreplication where id = 1", resultid1, nil)
		tme.dbTargetClients[0].addQuery("update _vt.vreplication set state = 'Stopped', message = 'stopped for cutover' where id in (1)", &sqltypes.Result{}, nil)
		tme.dbTargetClients[0].addQuery("select id from _vt.vreplication where id = 2", resultid2, nil)
		tme.dbTargetClients[0].addQuery("update _vt.vreplication set state = 'Stopped', message = 'stopped for cutover' where id in (2)", &sqltypes.Result{}, nil)
...
func testTableMigrateOneToMany(t *testing.T, keepData, keepRoutingRules bool) {
	...
	dropSourcesInvalid()
	_, err = tme.s.dropSources(ctx, tme.targetKeyspace, "test", DropTable, keepData, keepRoutingRules, false, false)
	...
	switchWrites(tme)
	_, _, err = tme.s.switchWrites(ctx, tme.targetKeyspace, "test", 1*time.Second, false, false, false, false)
	if err != nil {
		t.Fatal(err)
	}
	...
	results, err := tme.s.dropSources(ctx, tme.targetKeyspace, "test", DropTable, keepData, keepRoutingRules, false, true)
	...
	results, err = tme.s.dropSources(ctx, tme.targetKeyspace, "test", RenameTable, keepData, keepRoutingRules, false, true)
	require.NoError(t, err)
	...
	checkRouting(t, tme.s, wantRouting)
	_, err = tme.s.dropSources(ctx, tme.targetKeyspace, "test", RenameTable, keepData, keepRoutingRules, false, false)
	require.NoError(t, err)
...
func TestTableMigrateOneToManyDryRun(t *testing.T) {
	...
	tme.expectNoPreviousJournals()
	dryRunResults, err := tme.s.switchReads(ctx, tme.targetKeyspace, "test", []topodatapb.TabletType{topodatapb.TabletType_RDONLY}, nil, DirectionForward, true)
	require.NoError(t, err)
	...
	switchWrites(tme)
	_, results, err := tme.s.switchWrites(ctx, tme.targetKeyspace, "test", 1*time.Second, false, false, false, true)
	require.NoError(t, err)
...
func TestMigrateFailJournal(t *testing.T) {
	...
	switchWrites(tme)
	_, _, err = tme.s.switchWrites(ctx, tme.targetKeyspace, "test", 1*time.Second, false, false, true, false)
...
func TestTableMigrateCancel(t *testing.T) {
	...
	switchWrites(tme)
	_, _, err = tme.s.switchWrites(ctx, tme.targetKeyspace, "test", 1*time.Second, true, false, false, false)
	if err != nil {
		t.Fatal(err)
	}
	verifyQueries(t, tme.allDBClients)
...
func TestTableMigrateCancelDryRun(t *testing.T) {
	...
	switchWrites(tme)
	_, dryRunResults, err := tme.s.switchWrites(ctx, tme.targetKeyspace, "test", 1*time.Second, true, false, false, true)
	require.NoError(t, err)
...
func TestShardMigrateNoAvailableTabletsForReverseReplication(t *testing.T) {
	...
	ts, state, err := tme.s.getWorkflowState(ctx, tme.targetKeyspace, "test")
	...
	//-------------------------------------------------------------------------------------------------------------------
	// Single cell RDONLY migration.
	ts.optTabletTypes = topodatapb.TabletType_RDONLY.String()
	ts.optCells = "cell1"
	_, err = tme.s.switchReads(ctx, ts, state, timeout, false, DirectionForward, false)
	if err != nil {
		t.Fatal(err)
	}
	checkCellServedTypes(t, tme.ts, "ks:-40", "cell1", 2)
	...
	//-------------------------------------------------------------------------------------------------------------------
	// Other cell REPLICA migration.
	ts.optTabletTypes = topodatapb.TabletType_REPLICA.String()
	ts.optCells = "cell1"
	_, err = tme.s.switchReads(ctx, ts, state, timeout, false, DirectionForward, false)
	if err != nil {
		t.Fatal(err)
	...
	ts.optTabletTypes = topodatapb.TabletType_REPLICA.String()
	ts.optCells = "cell2"
	_, err = tme.s.switchReads(ctx, ts, state, timeout, false, DirectionBackward, false)
	if err != nil {
		t.Fatal(err)
	}
	checkCellServedTypes(t, tme.ts, "ks:-40", "cell1", 2)
	...
	//-------------------------------------------------------------------------------------------------------------------
	// Can't switch primary with SwitchReads.
	ts.optTabletTypes = topodatapb.TabletType_PRIMARY.String()
	ts.optCells = "cell1,cell2"
	_, err = tme.s.switchReads(ctx, ts, state, timeout, false, DirectionForward, false)
	want := "tablet type must be REPLICA or RDONLY: PRIMARY"
	if err == nil || err.Error() != want {
...
func checkRouting(t *testing.T, s *Server, want map[string][]string) {
	t.Helper()
	ctx := context.Background()
	got, err := topotools.GetRoutingRules(ctx, s.ts)
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, cell := range cells {
		checkCellRouting(t, s, cell, want)
	}
}

func checkCellRouting(t *testing.T, s *Server, cell string, want map[string][]string) {
	t.Helper()
	ctx := context.Background()
	svs, err := s.ts.GetSrvVSchema(ctx, cell)
	if err != nil {
		t.Fatal(err)
	}
