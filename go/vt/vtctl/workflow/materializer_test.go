...
func TestMissingTables(t *testing.T) {
	...
	_, err = env.ws.MoveTablesCreate(ctx, &vtctldatapb.MoveTablesCreateRequest{
		...
		SourceKeyspace: ms.SourceKeyspace,
		TargetKeyspace: ms.TargetKeyspace,
		IncludeTables:  []string{"t1", "tyt", "t2", "txt"},
		AutoStart:      true,
		OnDdl:          defaultOnDDL,
	})
...
func TestMoveTablesAllAndExclude(t *testing.T) {
	...
	targetTables := func(env *testMaterializerEnv) []string {
		vschema, err := env.ws.ts.GetSrvVSchema(ctx, env.cell)
		require.NoError(t, err)
	...
	testCases := []*testCase{
		{allTables: true, excludeTables: nil, want: allTables},
		{allTables: true, excludeTables: []string{"t2", "t3"}, want: []string{"t1"}},
		{allTables: true, excludeTables: []string{"t1"}, want: []string{"t2", "t3"}},
	}
	for _, tcase := range testCases {
		t.Run("", func(t *testing.T) {
			...
			_, err = env.ws.MoveTablesCreate(ctx, &vtctldatapb.MoveTablesCreateRequest{
				...
				SourceKeyspace: ms.SourceKeyspace,
				TargetKeyspace: ms.TargetKeyspace,
				ExcludeTables:  tcase.excludeTables,
				AllTables:      tcase.allTables,
				AutoStart:      true,
				OnDdl:          defaultOnDDL,
			})
			require.NoError(t, err)
...
func TestCreateLookupVindexFailures(t *testing.T) {
	...
	for _, tcase := range testcases {
		err := ws.CreateLookupVindex(context.Background(), "sourceks", tcase.input, "", "", false)
...
func TestExternalizeVindex(t *testing.T) {
	...
	for _, tcase := range testcases {
		...
		err := env.ws.ExternalizeVindex(context.Background(), tcase.input)
		if tcase.err != "" {
			if err == nil || !strings.Contains(err.Error(), tcase.err) {
...
func TestMaterializerOneToOne(t *testing.T) {
	...
	err := env.ws.Materialize(context.Background(), ms)
	require.NoError(t, err)
...
func TestMoveTablesStopFlags(t *testing.T) {
	...
	t.Run("StopStartedAndStopAfterCopyFlags", func(t *testing.T) {
		...
		env.tmc.expectVRQuery(100, mzCheckJournal, &sqltypes.Result{})
		env.tmc.expectVRQuery(200, mzSelectFrozenQuery, &sqltypes.Result{})
		// insert expects stop_after_copy to be true
		insert := `/insert into _vt.vreplication\(.*stop_after_copy:true.*`

		env.tmc.expectVRQuery(200, getWorkflowQuery, getWorkflowRes)
		env.tmc.expectVRQuery(200, insert, &sqltypes.Result{})
		//env.tmc.expectVRQuery(200, mzSelectIDQuery, &sqltypes.Result{})
		// --auto-start=false is tested by NOT expecting the update query which sets state to RUNNING
		_, err := env.ws.MoveTablesCreate(ctx, &vtctldatapb.MoveTablesCreateRequest{
			Workflow:       ms.Workflow,
			SourceKeyspace: ms.SourceKeyspace,
			TargetKeyspace: ms.TargetKeyspace,
