...
func (lv *lookupVindex) prepareMultipleCreate(ctx context.Context, workflow, keyspace string, specs *vschemapb.Keyspace, continueAfterCopyWithOwner bool) (
	...
	var (
		...
		// sourceColVindex is the ColumnVindex for the source table obtained from `specs`.
		sourceColVindex *vschemapb.ColumnVindex
	...
	for _, vInfo := range vInfos {
		...
		targetTable, ok := specs.Tables[vInfo.targetTableName]
		if !ok {
			return nil, nil, nil, nil, vterrors.Errorf(vtrpcpb.Code_INVALID_ARGUMENT, "target table not found for vindex %s", vInfo.name)
		}

		sourceColVindex, sourceVindexColumns, err = validateSourceTableAndGetVindexColumnsMultiple(vInfo, vindex)
		if err != nil {
			return nil, nil, nil, nil, err
		}
		...
		// Update sourceVSchema
		sourceVSchema.Vindexes[vInfo.name] = vindex
		sourceVSchemaTable.ColumnVindexes = append(sourceVSchemaTable.ColumnVindexes, sourceColVindex)
	}
...
func validateSourceTableAndGetVindexColumnsMultiple(vInfo *vindexInfo, vindex *vschemapb.Vindex) (colVindex *vschemapb.ColumnVindex, sourceVindexColumns []string, err error) {
	colVindexIndex := slices.IndexFunc(vInfo.sourceTable.ColumnVindexes, func(colVindex *vschemapb.ColumnVindex) bool {
		return colVindex.Name == vInfo.name
	})
	if colVindexIndex < 0 {
		return nil, nil, vterrors.Errorf(vtrpcpb.Code_INVALID_ARGUMENT, "ColumnVindex name (%s) not found in table %s",
			vInfo.name, vInfo.sourceTableName)
	}
	colVindex = vInfo.sourceTable.ColumnVindexes[colVindexIndex]

	if vindex.Owner != "" && vindex.Owner != vInfo.sourceTableName {
		return nil, nil, vterrors.Errorf(vtrpcpb.Code_INVALID_ARGUMENT, "vindex owner (%s) must match table name (%s)",
			vindex.Owner, vInfo.sourceTableName)
	}
	...
	if len(colVindex.Columns) != 0 {
		...
		if colVindex.Column == "" {
			return nil, nil, vterrors.Errorf(vtrpcpb.Code_INVALID_ARGUMENT, "at least one column must be specified in ColumnVindexes for the %s table",
				vInfo.sourceTableName)
		}
		sourceVindexColumns = []string{colVindex.Column}
	}
	if len(sourceVindexColumns) != len(vInfo.fromCols) {
		return nil, nil, vterrors.Errorf(vtrpcpb.Code_INVALID_ARGUMENT, "length of table columns (%d) differs from length of vindex columns (%d)",
			len(sourceVindexColumns), len(vInfo.fromCols))
	}
	return colVindex, sourceVindexColumns, nil
}
