...
func newTestTableMigraterCustom(ctx context.Context, t *testing.T, sourceShards, targetShards []string, fmtQuery string) *testMigraterEnv {
	...
	tme.s = NewServer(tme.ts, tmclient.NewTabletManagerClient())
	tme.s.sem = semaphore.NewWeighted(1)
	...
	for _, shard := range sourceShards {
		tme.sourcePrimaries = append(tme.sourcePrimaries, newFakeTablet(t, tme.s, "cell1", uint32(tabletID), topodatapb.TabletType_PRIMARY, tme.tmeDB, TabletKeyspaceShard(t, "ks1", shard)))
		tabletID += 10
	...
	for _, shard := range targetShards {
		tme.targetPrimaries = append(tme.targetPrimaries, newFakeTablet(t, tme.s, "cell1", uint32(tabletID), topodatapb.TabletType_PRIMARY, tme.tmeDB, TabletKeyspaceShard(t, "ks2", shard)))
		tabletID += 10
	...
	if err := topotools.SaveRoutingRules(ctx, tme.s.ts, map[string][]string{
		"t1":     {"ks1.t1"},
		"ks2.t1": {"ks1.t1"},
		"t2":     {"ks1.t2"},
		"ks2.t2": {"ks1.t2"},
...
func newTestShardMigrater(ctx context.Context, t *testing.T, sourceShards, targetShards []string) *testShardMigraterEnv {
	...
	tabletID := 10
	for _, shard := range sourceShards {
		tme.sourcePrimaries = append(tme.sourcePrimaries, newFakeTablet(t, tme.s, "cell1", uint32(tabletID), topodatapb.TabletType_PRIMARY, tme.tmeDB, TabletKeyspaceShard(t, "ks", shard)))
		tabletID += 10
	...
	for _, shard := range targetShards {
		tme.targetPrimaries = append(tme.targetPrimaries, newFakeTablet(t, tme.s, "cell1", uint32(tabletID), topodatapb.TabletType_PRIMARY, tme.tmeDB, TabletKeyspaceShard(t, "ks", shard)))
		tabletID += 10
...
func (tme *testMigraterEnv) startTablets(t *testing.T) {
	...
	for _, primary := range allPrimarys {
		primary.StartActionLoop(t, tme.s)
	}
	// Wait for the shard record primaries to be set.
	for _, primary := range allPrimarys {
		primaryFound := false
		for i := 0; i < 10; i++ {
			si, err := tme.s.ts.GetShard(context.Background(), primary.Tablet.Keyspace, primary.Tablet.Shard)
			if err != nil {
				t.Fatal(err)
			}
