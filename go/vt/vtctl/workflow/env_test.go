...
func newTestEnv(t *testing.T, ctx context.Context, ms *vtctldatapb.MaterializeSettings, sourceShards, targetShards []string) *testEnv {
	t.Helper()

	tmc := newTestTMClient(ms.SourceKeyspace, sourceShards, ms.TableSettings)
...
func (env *testEnv) addTablet(t *testing.T, id int, keyspace, shard string, tabletType topodatapb.TabletType) *topodatapb.Tablet {
...
func newTestTMClient(keyspace string, sourceShards []string, tableSettings []*vtctldatapb.TableMaterializeSettings) *testTMClient {
	return &testTMClient{
		keyspace:                           keyspace,
