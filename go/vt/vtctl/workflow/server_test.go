...
func getMoveTablesWorkflow(t *testing.T, cells, tabletTypes string) *vtctldata.GetWorkflowResponse {
...
func TestReshardingWorkflowErrorsAndMisc(t *testing.T) {
	...
	require.ElementsMatch(t, tabletTypes, []topodatapb.TabletType{topodatapb.TabletType_REPLICA, topodatapb.TabletType_RDONLY})
	...
	require.ElementsMatch(t, tabletTypes,
		[]topodatapb.TabletType{topodatapb.TabletType_REPLICA, topodatapb.TabletType_RDONLY, topodatapb.TabletType_PRIMARY})
...
func TestMoveTablesV2(t *testing.T) {
	...
	tme := newTestTableMigrater(ctx, t)
	defer tme.stopTablets(t)
	wf, err := tme.s.NewVReplicationWorkflow(ctx, MoveTablesWorkflow, p)
	require.NoError(t, err)
	require.NotNil(t, wf)
	require.Equal(t, WorkflowStateNotSwitched, wf.CurrentState())
