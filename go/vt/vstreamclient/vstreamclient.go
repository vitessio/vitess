...
type VStreamClient struct {
	...
	// lastEventReceivedAtUnixNano is the time the last event was received, which is used in the heartbeat monitor
	// to let us know if we've been disconnected from the stream.
	lastEventReceivedAtUnixNano atomic.Int64
...
//
// After shutdown, the client is considered closed and Run will return an error if called again.
