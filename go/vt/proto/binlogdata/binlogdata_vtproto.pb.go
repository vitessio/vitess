...
func (m *OnlineDDLEvent) MarshalVT() (dAtA []byte, err error) {
	if m == nil {
		return nil, nil
	}
	size := m.SizeVT()
...
func (m *OnlineDDLEvent) MarshalToVT(dAtA []byte) (int, error) {
	size := m.SizeVT()
	return m.MarshalToSizedBufferVT(dAtA[:size])
}

func (m *OnlineDDLEvent) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	if m == nil {
		return 0, nil
	}
	i := len(dAtA)
	...
	if len(m.Ddl) > 0 {
		i -= len(m.Ddl)
		copy(dAtA[i:], m.Ddl)
		i = encodeVarint(dAtA, i, uint64(len(m.Ddl)))
		i--
		dAtA[i] = 0x2a
	}
	if len(m.MysqlTableName) > 0 {
		i -= len(m.MysqlTableName)
		copy(dAtA[i:], m.MysqlTableName)
		i = encodeVarint(dAtA, i, uint64(len(m.MysqlTableName)))
		i--
		dAtA[i] = 0x22
	}
	if len(m.MaterializedTableName) > 0 {
		i -= len(m.MaterializedTableName)
		copy(dAtA[i:], m.MaterializedTableName)
		i = encodeVarint(dAtA, i, uint64(len(m.MaterializedTableName)))
		i--
		dAtA[i] = 0x1a
	}
	if len(m.Uuid) > 0 {
...
func (m *VEvent) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if m.OnlineDdlEvent != nil {
		size, err := m.OnlineDdlEvent.MarshalToSizedBufferVT(dAtA[:i])
		if err != nil {
			return 0, err
		}
		i -= size
...
func (m *OnlineDDLEvent) SizeVT() (n int) {
	if m == nil {
		return 0
	}
	var l int
	...
	l = len(m.MaterializedTableName)
	if l > 0 {
		n += 1 + l + sov(uint64(l))
	}
	l = len(m.MysqlTableName)
	if l > 0 {
		n += 1 + l + sov(uint64(l))
	}
	l = len(m.Ddl)
...
func (m *OnlineDDLEvent) UnmarshalVT(dAtA []byte) error {
	l := len(dAtA)
	iNdEx := 0
	for iNdEx < l {
		preIndex := iNdEx
		...
		wireType := int(wire & 0x7)
		if wireType == 4 {
			return fmt.Errorf("proto: OnlineDDLEvent: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return fmt.Errorf("proto: OnlineDDLEvent: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 0 {
			...
			for shift := uint(0); ; shift += 7 {
				...
				b := dAtA[iNdEx]
				iNdEx++
				m.EventType |= OnlineDDLEventType(b&0x7F) << shift
				if b < 0x80 {
					break
				}
			}
		...
		case 3:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field MaterializedTableName", wireType)
			}
			var stringLen uint64
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			m.MaterializedTableName = string(dAtA[iNdEx:postIndex])
			iNdEx = postIndex
		case 4:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field MysqlTableName", wireType)
			}
			var stringLen uint64
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			m.MysqlTableName = string(dAtA[iNdEx:postIndex])
			iNdEx = postIndex
		case 5:
			if wireType != 2 {
...
func (m *VEvent) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 24:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field OnlineDdlEvent", wireType)
			}
			var msglen int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			if m.OnlineDdlEvent == nil {
				m.OnlineDdlEvent = &OnlineDDLEvent{}
			}
			if err := m.OnlineDdlEvent.UnmarshalVT(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		default:
