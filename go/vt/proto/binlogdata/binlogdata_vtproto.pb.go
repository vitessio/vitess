...
func (m *MinimalTable) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if len(m.PKIndex) > 0 {
		i -= len(m.PKIndex)
		copy(dAtA[i:], m.PKIndex)
		i = encodeVarint(dAtA, i, uint64(len(m.PKIndex)))
		i--
		dAtA[i] = 0x22
	}
...
func (m *MinimalTable) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 4:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field PKIndex", wireType)
			}
			var stringLen uint64
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			m.PKIndex = string(dAtA[iNdEx:postIndex])
			iNdEx = postIndex
		default:
			iNdEx = preIndex
			skippy, err := skip(dAtA[iNdEx:])
