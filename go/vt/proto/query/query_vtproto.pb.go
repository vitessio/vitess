...
func (m *Target) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if len(m.DbServerVersion) > 0 {
		i -= len(m.DbServerVersion)
		copy(dAtA[i:], m.DbServerVersion)
		i = encodeVarint(dAtA, i, uint64(len(m.DbServerVersion)))
		i--
		dAtA[i] = 0x2a
	}
	if len(m.Cell) > 0 {
...
func (m *Target) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 5:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field DbServerVersion", wireType)
			}
			var stringLen uint64
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			m.DbServerVersion = string(dAtA[iNdEx:postIndex])
			iNdEx = postIndex
		default:
			iNdEx = preIndex
