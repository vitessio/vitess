...
type SplitQueryRequest struct {
	...
	//  EQUAL_SPLITS
	//    If this algorithm is selected then only the first split_column given is
	//    used (or the first primary key column if no split_column's are
	//    specified). In the rest of this algorithm's description, we refer to
	//    this column as "the split column".
	//    The split_column must have numeric type (integral or floating point).
	//    The algorithm works by taking the interval [min, max], where min and
	//    max are the minimum and maximum values of of the split_column in
	...
	//    restricts that part to rows whose value in the split_column belongs to
	//    a particular sub-interval. This is fast, but requires that the
	//    distribution of values of the split_column be uniform in [min, max],
