...
func (m *SchemaMigration) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if m.UserThrottleRatio != 0 {
		i -= 4
		binary.LittleEndian.PutUint32(dAtA[i:], uint32(math.Float32bits(float32(m.UserThrottleRatio))))
		i--
		dAtA[i] = 0x2
		i--
		dAtA[i] = 0xdd
...
func (m *WorkflowSwitchTrafficResponse) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		wireType := int(wire & 0x7)
		if wireType == 4 {
			return fmt.Errorf("proto: WorkflowSwitchTrafficResponse: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return fmt.Errorf("proto: WorkflowSwitchTrafficResponse: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 2 {
