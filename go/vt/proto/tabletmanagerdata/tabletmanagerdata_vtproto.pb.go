...
func (m *DeleteTableDataResponse) CloneVT() *DeleteTableDataResponse {
	...
	if rhs := m.TableResults; rhs != nil {
		tmpContainer := make(map[string]uint64, len(rhs))
		for k, v := range rhs {
			tmpContainer[k] = v
		}
		r.TableResults = tmpContainer
	}
	if len(m.unknownFields) > 0 {
		r.unknownFields = make([]byte, len(m.unknownFields))
		copy(r.unknownFields, m.unknownFields)
...
func (m *DeleteTableDataResponse) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if len(m.TableResults) > 0 {
		for k := range m.TableResults {
			v := m.TableResults[k]
			baseI := i
...
func (m *DeleteTableDataResponse) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 1:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field TableResults", wireType)
			}
			var msglen int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			if m.TableResults == nil {
				m.TableResults = make(map[string]uint64)
			}
			var mapkey string
			var mapvalue uint64
			for iNdEx < postIndex {
			...
			m.TableResults[mapkey] = mapvalue
			iNdEx = postIndex
		default:
			iNdEx = preIndex
			skippy, err := protohelpers.Skip(dAtA[iNdEx:])
