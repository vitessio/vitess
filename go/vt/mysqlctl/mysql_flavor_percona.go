...
// percona is the implementation of PerconaFlavor for Percona 5.6.
type percona struct {
}
...
func (*percona) VersionMatch(version string) bool {
	return strings.HasPrefix(version, "5.6")
}

// MasterPosition implements MysqlFlavor.MasterPosition().
func (flavor *percona) MasterPosition(mysqld *Mysqld) (rp replication.Position, err error) {
...
func (flavor *percona) SlaveStatus(mysqld *Mysqld) (replication.Status, error) {
	...
	if err != nil {
		return replication.Status{}, fmt.Errorf("SlaveStatus can't parse Percona 5.6 GTID (Executed_Gtid_Set: %#v): %v", fields["Executed_Gtid_Set"], err)
	}
	return status, nil
}

// WaitMasterPos implements MysqlFlavor.WaitMasterPos().
func (*percona) WaitMasterPos(mysqld *Mysqld, targetPos replication.Position, waitTimeout time.Duration) error {
	var query string
...
func (*percona) ResetReplicationCommands() []string {
	return []string{
		"STOP SLAVE",
		"RESET SLAVE",
...
func (*percona) PromoteSlaveCommands() []string {
	return []string{
		"RESET SLAVE",
	}
}
...
func (*percona) SetSlavePositionCommands(pos replication.Position) ([]string, error) {
	return []string{
...
func (*percona) SetMasterCommands(params *sqldb.ConnParams, masterHost string, masterPort int, masterConnectRetry int) ([]string, error) {
...
func (*percona) ParseGTID(s string) (replication.GTID, error) {
	return replication.ParseGTID(perconaFlavorID, s)
}

// ParseReplicationPosition implements MysqlFlavor.ParseReplicationPosition().
func (*percona) ParseReplicationPosition(s string) (replication.Position, error) {
	return replication.ParsePosition(perconaFlavorID, s)
}

// SendBinlogDumpCommand implements MysqlFlavor.SendBinlogDumpCommand().
func (flavor *percona) SendBinlogDumpCommand(conn *SlaveConnection, startPos replication.Position) error {
	const ComBinlogDumpGTID = 0x1E // COM_BINLOG_DUMP_GTID

	gtidSet, ok := startPos.GTIDSet.(replication.PerconaGTIDSet)
	if !ok {
		return fmt.Errorf("startPos.GTIDSet is wrong type - expected PerconaGTIDSet, got: %#v", startPos.GTIDSet)
	}
	...
	// Build the command.
	buf := makePerconaBinlogDumpGTIDCommand(0, conn.slaveID, gtidSet)
	return conn.SendCommand(ComBinlogDumpGTID, buf)
}

// MakeBinlogEvent implements MysqlFlavor.MakeBinlogEvent().
func (*percona) MakeBinlogEvent(buf []byte) replication.BinlogEvent {
	return NewPerconaBinlogEvent(buf)
}

// EnableBinlogPlayback implements MysqlFlavor.EnableBinlogPlayback().
func (*percona) EnableBinlogPlayback(mysqld *Mysqld) error {
	return nil
}

// DisableBinlogPlayback implements MysqlFlavor.DisableBinlogPlayback().
func (*percona) DisableBinlogPlayback(mysqld *Mysqld) error {
	return nil
}

// perconaBinlogEvent wraps a raw packet buffer and provides methods to examine
// it by implementing replication.BinlogEvent. Some methods are pulled in from
// binlogEvent.
type perconaBinlogEvent struct {
	binlogEvent
}

// NewPerconaBinlogEvent creates a BinlogEvent from given byte array
func NewPerconaBinlogEvent(buf []byte) replication.BinlogEvent {
	return perconaBinlogEvent{binlogEvent: binlogEvent(buf)}
}

// IsGTID implements BinlogEvent.IsGTID().
func (ev perconaBinlogEvent) IsGTID() bool {
	return ev.Type() == 33 // GTID_LOG_EVENT
}

// HasGTID implements BinlogEvent.HasGTID().
func (ev perconaBinlogEvent) HasGTID(f replication.BinlogFormat) bool {
...
func (ev perconaBinlogEvent) GTID(f replication.BinlogFormat) (replication.GTID, error) {
	data := ev.Bytes()[f.HeaderLength:]
	var sid replication.PSID
	copy(sid[:], data[1:1+16])
	gno := int64(binary.LittleEndian.Uint64(data[1+16 : 1+16+8]))
	return replication.PerconaGTID{Server: sid, Sequence: gno}, nil
}

// StripChecksum implements BinlogEvent.StripChecksum().
func (ev perconaBinlogEvent) StripChecksum(f replication.BinlogFormat) (replication.BinlogEvent, []byte, error) {
	...
	case BinlogChecksumAlgCRC32:
		...
		data = data[:length-4]
		return perconaBinlogEvent{binlogEvent: binlogEvent(data)}, checksum, nil
	default:
...
// makePerconaBinlogDumpGTIDCommand builds a buffer containing the data for a MySQL 5.6
// COM_BINLOG_DUMP_GTID command. Only the GTID form is supported.
//
// https://dev.mysql.com/doc/internals/en/com-binlog-dump-gtid.html
func makePerconaBinlogDumpGTIDCommand(flags uint16, serverID uint32, gtidSet replication.PerconaGTIDSet) []byte {
...
func init() {
	registerFlavorBuiltin(perconaFlavorID, &percona{})
}
