...
func NewMysqld(dbcfgs *dbconfigs.DBConfigs) *Mysqld {
	...
	result.dbaPool = dbconnpool.NewConnectionPool("DbaConnPool", pools.ResourceImpl, *dbaPoolSize, *dbaIdleTimeout, 0)
	result.dbaPool.Open(dbcfgs.Dba(), dbaMysqlStats)

	// Create and open the connection pool for app access.
	result.appPool = dbconnpool.NewConnectionPool("AppConnPool", pools.ResourceImpl, *appPoolSize, *appIdleTimeout, 0)
