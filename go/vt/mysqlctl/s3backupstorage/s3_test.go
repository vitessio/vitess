...
func (sfc *s3FakeClient) PutObjectRequest(in *s3.PutObjectInput) (*request.Request, *s3.PutObjectOutput) {
	...
	req := request.Request{
		...
		Error: sfc.err, // if not nil, this forces req.Send() (which is called by the uploader) to always return non-nil error
	}
...
func TestAddFileErrorStats(t *testing.T) {
	...
	require.Equal(t, scopedStats.ScopeV[stats.ScopeOperation], "S3:Upload")
