...
func registerXtraBackupEngineFlags(fs *pflag.FlagSet) {
	...
	fs.StringVar(&xtrabackupStreamMode, "xtrabackup_stream_mode", xtrabackupStreamMode, "Which mode to use if streaming, valid values are tar and xbstream. Please note that `tar` is not supported in XtraBackup 8.0 (and XtraBackup 8.0.X does not support MySQL 8.0.X+1)")
