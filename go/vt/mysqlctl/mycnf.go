...
// merge will create a new Mycnf produced from merging provided Mycnfs from
// right-to-left, skipping non-zero values.
func merge(cnfs ...*Mycnf) *Mycnf {
	cnf := new(Mycnf)

	for i, c := range cnfs {
		if i == 0 || c.ServerID != 0 {
			cnf.ServerID = c.ServerID
		}
		if i == 0 || c.MysqlPort != 0 {
			cnf.MysqlPort = c.MysqlPort
		}
		if i == 0 || c.DataDir != "" {
			cnf.DataDir = c.DataDir
		}
		if i == 0 || c.InnodbDataHomeDir != "" {
			cnf.InnodbDataHomeDir = c.InnodbDataHomeDir
		}
		if i == 0 || c.InnodbLogGroupHomeDir != "" {
			cnf.InnodbLogGroupHomeDir = c.InnodbLogGroupHomeDir
		}
		if i == 0 || c.SocketFile != "" {
			cnf.SocketFile = c.SocketFile
		}
		if i == 0 || c.GeneralLogPath != "" {
			cnf.GeneralLogPath = c.GeneralLogPath
		}
		if i == 0 || c.ErrorLogPath != "" {
			cnf.ErrorLogPath = c.ErrorLogPath
		}
		if i == 0 || c.SlowLogPath != "" {
			cnf.SlowLogPath = c.SlowLogPath
		}
		if i == 0 || c.RelayLogPath != "" {
			cnf.RelayLogPath = c.RelayLogPath
		}
		if i == 0 || c.RelayLogIndexPath != "" {
			cnf.RelayLogIndexPath = c.RelayLogIndexPath
		}
		if i == 0 || c.RelayLogInfoPath != "" {
			cnf.RelayLogInfoPath = c.RelayLogInfoPath
		}
		if i == 0 || c.BinLogPath != "" {
			cnf.BinLogPath = c.BinLogPath
		}
		if i == 0 || c.MasterInfoFile != "" {
			cnf.MasterInfoFile = c.MasterInfoFile
		}
		if i == 0 || c.PidFile != "" {
			cnf.PidFile = c.PidFile
		}
		if i == 0 || c.TmpDir != "" {
			cnf.TmpDir = c.TmpDir
		}
		if i == 0 || c.SecureFilePriv != "" {
			cnf.SecureFilePriv = c.SecureFilePriv
		}
		if i == 0 || c.ServerID != 0 {
			cnf.ServerID = c.ServerID
		}
