...
func (r *resolver) watch() {
	...
	for {
		...
		case nil:
			...
			case 0:
				log.WarnS(fmt.Sprintf("%s: found no %ss (cluster %s); updating grpc clientconn state anyway", logPrefix, r.component, r.cluster))
			default:
				log.InfoS(fmt.Sprintf("%s: found %d %ss (cluster %s)", logPrefix, len(state.Addresses), r.component, r.cluster))
			}

			if updateErr := r.cc.UpdateState(*state); updateErr != nil {
				log.ErrorS(fmt.Sprintf("%s: failed to update %ss addresses for %s (cluster %s): %s", logPrefix, r.component, r.component, r.cluster, updateErr))
				err = updateErr
			}
		default:
			log.ErrorS(fmt.Sprintf("%s: failed to resolve new addresses for %s (cluster %s): %s", logPrefix, r.component, r.cluster, err))
...
func (r *resolver) resolve() (*grpcresolver.State, error) {
	...
	log.InfoS(fmt.Sprintf("%s: resolving %ss (cluster %s)", logPrefix, r.component, r.cluster))
