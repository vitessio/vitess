...
func NewBinlogConnection(cp dbconfigs.Connector) (*BinlogConnection, error) {
	...
	log.InfoS(fmt.Sprintf("new binlog connection: serverID=%d", bc.serverID))
	return bc, nil
}
...
func (bc *BinlogConnection) StartBinlogDumpFromPosition(ctx context.Context, binlogFilename string, startPos replication.Position) (<-chan mysql.BinlogEvent, <-chan error, error) {
	...
	log.InfoS(fmt.Sprintf("sending binlog dump command: startPos=%v, serverID=%v", startPos, bc.serverID))
	if err := bc.SendBinlogDumpCommand(bc.serverID, binlogFilename, startPos); err != nil {
		log.ErrorS(fmt.Sprintf("couldn't send binlog dump command: %v", err))
		return nil, nil, err
	}
		...
		for {
			...
			if err != nil {
				...
				if sqlErr, ok := err.(*sqlerror.SQLError); ok && sqlErr.Number() == sqlerror.CRServerLost {
					...
					log.InfoS(fmt.Sprintf("connection closed during binlog stream (possibly intentional): %v", err))
					return
				}
				log.ErrorS(fmt.Sprintf("read error while streaming binlog events: %v", err))
				return
			}
...
func (bc *BinlogConnection) findFileBeforeTimestamp(ctx context.Context, timestamp int64) (filename string, err error) {
	...
	log.ErrorS(fmt.Sprintf("couldn't find an old enough binlog to match timestamp >= %v (looked at %v files)", timestamp, len(binlogs.Rows)))
...
func (bc *BinlogConnection) Close() {
	if bc.Conn != nil {
		log.InfoS("closing binlog socket to unblock reads")
		bc.Conn.Close()
		...
		if bc.cancel != nil {
			log.InfoS("waiting for binlog dump thread to end")
			bc.cancel()
			bc.wg.Wait()
			bc.cancel = nil
		}

		log.InfoS(fmt.Sprintf("closing binlog MySQL client with serverID %v. Will recycle ID.", bc.serverID))
		bc.Conn = nil
