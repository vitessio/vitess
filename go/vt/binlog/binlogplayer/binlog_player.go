...
var AlterVReplicationTable = []string{
	...
	"ALTER TABLE _vt.vreplication ADD COLUMN rows_estimate BIGINT(20) NOT NULL DEFAULT 0",
	"ALTER TABLE _vt.vreplication ADD COLUMN rows_copied BIGINT(20) NOT NULL DEFAULT 0",
	"ALTER TABLE _vt.vreplication ADD COLUMN time_started BIGINT(20) NOT NULL DEFAULT 0",
	"ALTER TABLE _vt.vreplication ADD COLUMN eta_seconds BIGINT(20) NOT NULL DEFAULT 0",
}
...
func GenerateUpdateTime(uid uint32, timeUpdated int64) (string, error) {
	...
	return fmt.Sprintf("update _vt.vreplication set time_updated=%v, time_started=IF(time_started=0, %v, time_started) where id=%v", timeUpdated, timeUpdated, uid), nil
}

// GenerateUpdateRowsEstimate returns a statement to update rows_estimate in the _vt.vreplication table.
func GenerateUpdateRowsEstimate(uid uint32, rowsEstimate int64) string {
	return fmt.Sprintf("update _vt.vreplication set rows_estimate=%d where id=%v", rowsEstimate, uid)
}
