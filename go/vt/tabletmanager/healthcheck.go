...
func (agent *ActionAgent) runHealthCheck(targetTabletType topo.TabletType) {
	...
	if err := topotools.ChangeType(agent.ctx, agent.TopoServer, tablet.Alias, newTabletType, health, true /*runHooks*/); err != nil {
	...
	if err := agent.refreshTablet(agent.ctx, "healthcheck"); err != nil {
...
func (agent *ActionAgent) terminateHealthChecks(targetTabletType topo.TabletType) {
	...
	if err := topotools.ChangeType(agent.ctx, agent.TopoServer, tablet.Alias, topo.TYPE_SPARE, make(map[string]string), true /*runHooks*/); err != nil {
		log.Infof("Error updating tablet record: %v", err)
		return
	}
	...
	go func() {
		...
		if err := agent.refreshTablet(agent.ctx, "terminatehealthcheck"); err != nil {
			log.Warningf("refreshTablet failed: %v", err)
		}
	}()
}
...
func (agent *ActionAgent) rebuildShardIfNeeded(tablet *topo.TabletInfo, targetTabletType topo.TabletType) error {
	if topo.IsInServingGraph(targetTabletType) {
		...
		if _, err := topotools.RebuildShard(agent.ctx, logutil.NewConsoleLogger(), agent.TopoServer, tablet.Keyspace, tablet.Shard, []string{tablet.Alias.Cell}, agent.LockTimeout); err != nil {
