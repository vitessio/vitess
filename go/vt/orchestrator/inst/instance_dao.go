	...
	instance.Key = *instanceKey
	{
		...
		if (instance.IsOracleMySQL() || instance.IsPercona()) && !instance.IsSmallerMajorVersionByString("5.6") {
			...
			go func() {
				...
				} else if primaryInfoRepositoryOnTable {
...
func ReadInstanceClusterAttributes(instance *Instance) (err error) {
	...
	err = db.QueryOrchestrator(query, args, func(m sqlutils.RowMap) error {
		...
		primaryOrGroupPrimaryInstanceKey.Hostname = m.GetString("primary_host")
		primaryOrGroupPrimaryInstanceKey.Port = m.GetInt("primary_port")
...
// ReadAllInstanceKeysPrimaryKeys
func ReadAllMinimalInstances() ([]MinimalInstance, error) {
	res := []MinimalInstance{}
	query := `
		select
			hostname, port, primary_host, primary_port, cluster_name
		from
			database_instance
			`
	err := db.QueryOrchestrator(query, sqlutils.Args(), func(m sqlutils.RowMap) error {
		...
		minimalInstance.PrimaryKey = InstanceKey{
			Hostname: m.GetString("primary_host"),
			Port:     m.GetInt("primary_port"),
		}
		...
		_, err := db.ExecOrchestrator(`
        	insert ignore into
        		database_instance_topology_history (snapshot_unix_timestamp,
        			hostname, port, primary_host, primary_port, cluster_name, version)
        	select
        		UNIX_TIMESTAMP(NOW()),
        		hostname, port, primary_host, primary_port, cluster_name, version
			from
