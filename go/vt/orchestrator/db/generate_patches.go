...
var generateSQLPatches = []string{
	...
	`,
	`
		DROP INDEX primary_host_port_idx ON database_instance
	`,
	`
		ALTER TABLE
			database_instance
			ADD INDEX primary_host_port_idx_database_instance (primary_host, primary_port)
	`,
	`
		ALTER TABLE
			database_instance
	...
	`
		ALTER TABLE
			database_instance
			ADD COLUMN replica_uuid varchar(64) CHARACTER SET ascii NOT NULL AFTER oracle_gtid
	`,
	`
		ALTER TABLE
			database_instance
	...
	`
		ALTER TABLE
			database_instance
			ADD COLUMN ancestry_uuid text CHARACTER SET ascii NOT NULL AFTER replica_uuid
	`,
	`
		ALTER TABLE
			database_instance
