...
func (thc *tabletHealthCheck) checkConn(hc *HealthCheckImpl) {
	...
	for {
		...
		if err != nil {
			...
			// We have reason to suspect the tablet healthcheck record is corrupted or invalid so let's remove from the healtcheck
			// and it will get re-added again if healthy
