...
func NewKeyspaceEventWatcher(ctx context.Context, topoServer srvtopo.Server, hc HealthCheck, localCell string) *KeyspaceEventWatcher {
	...
	kew.run(ctx)
	log.InfoS(fmt.Sprintf("started watching keyspace events in %q", localCell))
	return kew
}
...
func (kew *KeyspaceEventWatcher) run(ctx context.Context) {
	...
	go func() {
		...
		if err != nil {
			log.ErrorS(fmt.Sprintf("CEM: initialize failed for cell %q: %v", kew.localCell, err))
			return
		}
...
func (kss *keyspaceState) ensureConsistentLocked() {
	...
	kss.consistent = true
	log.InfoS(fmt.Sprintf("keyspace %s is now consistent", kss.keyspace))
	...
	for shard, sstate := range kss.shards {
		...
		log.DebugS(fmt.Sprintf("keyspace event resolved: %s is now consistent (serving: %t)",
...
func (kss *keyspaceState) getMoveTablesStatus(vs *vschemapb.SrvVSchema) (*MoveTablesState, error) {
	...
	if isPartialTables {
		...
		log.InfoS(fmt.Sprintf("getMoveTablesStatus: keyspace %s declaring partial move tables %s", kss.keyspace, mtState.String()))
		return mtState, nil
	}
	...
	if rr != nil {
		...
		if ok && len(r) > 0 && r[0] != fmt.Sprintf("%s.%s", kss.keyspace, oneDeniedTable) {
			mtState.State = MoveTablesSwitched
			log.InfoS(fmt.Sprintf("onSrvKeyspace::  keyspace %s writes have been switched for table %s, rule %v", kss.keyspace, oneDeniedTable, r[0]))
		}
	}
	log.InfoS(fmt.Sprintf("getMoveTablesStatus: keyspace %s declaring regular move tables %s", kss.keyspace, mtState.String()))
...
func (kss *keyspaceState) onSrvKeyspace(newKeyspace *topodatapb.SrvKeyspace, newError error) bool {
	...
	if topo.IsErrType(newError, topo.NoNode) {
		kss.deleted = true
		log.InfoS(fmt.Sprintf("keyspace %q deleted", kss.keyspace))
		return false
	}
	...
	if newError != nil {
		...
		log.ErrorS(fmt.Sprintf("error while watching keyspace %q: %v", kss.keyspace, newError))
		return true
	}
...
func (kss *keyspaceState) onSrvVSchema(vs *vschemapb.SrvVSchema, err error) bool {
	...
	if kss.moveTablesState, kerr = kss.getMoveTablesStatus(vs); err != nil {
		log.ErrorS(fmt.Sprintf("onSrvVSchema: keyspace %s failed to get move tables status: %v", kss.keyspace, kerr))
	}
...
func newKeyspaceState(ctx context.Context, kew *KeyspaceEventWatcher, cell, keyspace string) *keyspaceState {
	log.InfoS(fmt.Sprintf("created dedicated watcher for keyspace %s/%s", cell, keyspace))
	kss := &keyspaceState{
		kew:      kew,
		keyspace: keyspace,
...
func (kew *KeyspaceEventWatcher) WaitForConsistentKeyspaces(ctx context.Context, ksList []string) error {
	...
	for {
		...
		case <-ctx.Done():
			for _, ks := range keyspaces {
				if ks != "" {
					log.InfoS(fmt.Sprintf("keyspace %v didn't become consistent", ks))
				}
			}
			return ctx.Err()
