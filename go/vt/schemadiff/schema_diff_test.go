...
func TestSchemaDiff(t *testing.T) {
	...
	tt := []struct {
		...
		// FKs
		{
			...
			instantCapability:      InstantDDLCapabilityIrrelevant,
			expectAffectedFKTables: []string{"t1", "t3"},
		},
		{
			...
			sequential:             true,
			instantCapability:      InstantDDLCapabilityIrrelevant,
			expectAffectedFKTables: []string{"tp", "t3"},
		},
		{
			...
			expectAffectedFKTables: []string{"t11", "t12"},
		},
		{
		...
		},
		{
			...
			entityOrder:            []string{"t2"},
			instantCapability:      InstantDDLCapabilityImpossible,
			expectAffectedFKTables: []string{"t1", "t2"},
		},
		{
			...
			instantCapability:      InstantDDLCapabilityImpossible,
			expectAffectedFKTables: []string{"tp", "t2"},
		},
		{
		...
		},
		{
			...
			instantCapability:      InstantDDLCapabilityImpossible,
			expectAffectedFKTables: []string{"t0", "t2"},
		},
		{
			...
			instantCapability:      InstantDDLCapabilityImpossible,
			expectAffectedFKTables: []string{"t2", "t9"},
		},
		{
		...
		},
		{
			...
			instantCapability:      InstantDDLCapabilityImpossible,
			expectAffectedFKTables: []string{"t1", "t2", "t3", "t4", "t5"},
		},
		{
			name: "drop fk",
			fromQueries: []string{
		...
		},
		{
			...
			instantCapability:      InstantDDLCapabilityImpossible,
			fkStrategy:             ForeignKeyCheckStrategyStrict,
			expectAffectedFKTables: []string{"t1", "t2", "t3"},
		},
		{
		...
		},
		{
			...
			entityOrder:            []string{"t1"},
			instantCapability:      InstantDDLCapabilityImpossible,
			expectAffectedFKTables: []string{"t1", "parent"},
		},
		{
			name: "add two foreign key references",
			fromQueries: []string{
		...
		{
			name: "modify parent of fk table",
			fromQueries: []string{
				"CREATE TABLE t1 (id bigint NOT NULL, name varchar(255), PRIMARY KEY (id))",
				"CREATE TABLE t3 (id bigint NOT NULL, name varchar(255), t1_id bigint, PRIMARY KEY (id), KEY t1_id (t1_id), KEY nameidx (name), CONSTRAINT t3_ibfk_1 FOREIGN KEY (t1_id) REFERENCES t1 (id) ON DELETE CASCADE ON UPDATE CASCADE)",
			},
			toQueries: []string{
				"CREATE TABLE t1 (id bigint NOT NULL, name varchar(255), PRIMARY KEY (id), KEY idx_name (name))",
				"CREATE TABLE t3 (id bigint NOT NULL, name varchar(255), t1_id bigint, PRIMARY KEY (id), KEY t1_id (t1_id), KEY nameidx (name), CONSTRAINT t3_ibfk_1 FOREIGN KEY (t1_id) REFERENCES t1 (id) ON DELETE CASCADE ON UPDATE CASCADE)",
	...
	for _, tc := range tt {
		t.Run(tc.name, func(t *testing.T) {
			...
			affectedFKTables := schemaDiff.AffectedForeignKeyTables()
			for name, e := range affectedFKTables {
				require.NotNil(t, e)
				assert.Equal(t, name, e.Name())
			}
			assert.ElementsMatch(t, tc.expectAffectedFKTables, maps.Keys(affectedFKTables))
