...
type InfoForRecoveryAnalysis struct {
	...
	ReadOnly                                  uint
	IsStalledDisk                             uint
	InnoDBLongSemaphoreWaitSeen               uint
}
...
func (info *InfoForRecoveryAnalysis) ConvertToRowMap() sqlutils.RowMap {
	...
	rowMap["innodb_long_semaphore_wait_seen"] = sqlutils.CellData{String: strconv.FormatUint(uint64(info.InnoDBLongSemaphoreWaitSeen), 10), Valid: true}
	return rowMap
}
