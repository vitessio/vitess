...
func RegisterFlags(fs *pflag.FlagSet) {
	...
	utils.SetFlagStringSliceVar(fs, &cellsNoRecovery, "cells-no-recovery", cellsNoRecovery, "Comma-separated list of cells in which VTOrc skips recovery actions when the analyzed tablet is in one of these cells. This includes suppressing an initial primary election (ClusterHasNoPrimary) when the analyzed tablet is in a listed cell. Detection still happens and discovery still spans all cells. Cells are validated against the topology at startup. Example: \"cell1,cell2\"")
