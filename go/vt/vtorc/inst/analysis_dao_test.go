...
// TestSameTabletShardWidePreservesSuppression verifies that when a primary
// matches both a non-shard-wide problem with BeforeAnalyses (PrimaryIsReadOnly)
// and a shard-wide problem (PrimarySemiSyncBlocked), the shard-wide problem
// is still recorded on the cluster's analysis state so other tablets'
// non-dependent analyses are suppressed. The chosen analysis for the primary
// is the BeforeAnalyses-promoted PrimaryIsReadOnly so its recovery proceeds.
func TestSameTabletShardWidePreservesSuppression(t *testing.T) {
	...
	got, err := GetDetectionAnalysis("", "", &DetectionAnalysisHints{})
	require.NoError(t, err)
	require.Len(t, got, 1, "replica's ReplicaIsWritable should be suppressed by recorded shard-wide PrimarySemiSyncBlocked")
	assert.Equal(t, PrimaryIsReadOnly, got[0].Analysis)
	assert.Equal(t, uint32(101), got[0].AnalyzedInstanceAlias.Uid)
}
