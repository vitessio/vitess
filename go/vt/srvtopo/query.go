		...
		go func() {
			defer func() {
				if err := recover(); err != nil {
					log.ErrorS(fmt.Sprintf("ResilientQuery uncaught panic, cell :%v, err :%v)", key, err))
				}
			}()
			...
			if err == nil {
				...
				if entry.insertionTime.IsZero() {
					log.ErrorS(fmt.Sprintf("ResilientQuery(%v, %v) failed: %v (no cached value, caching and returning error)", ctx, wkey, err))
				} else if newCtx.Err() == context.DeadlineExceeded {
					log.ErrorS(fmt.Sprintf("ResilientQuery(%v, %v) failed: %v (request timeout), (keeping cached value: %v)", ctx, wkey, err, entry.value))
				} else if entry.value != nil && time.Since(entry.insertionTime) < q.cacheTTL {
					q.counts.Add(cachedCategory, 1)
					log.WarnS(fmt.Sprintf("ResilientQuery(%v, %v) failed: %v (cached value still considered valid: %v)", ctx, wkey, err, entry.value))
				} else {
					log.ErrorS(fmt.Sprintf("ResilientQuery(%v, %v) failed: %v (cached value expired, keeping cached value)", ctx, wkey, err))
				}
			}
