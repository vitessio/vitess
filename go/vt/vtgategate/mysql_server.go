...
// connHandler implements the Listener interface.
...
func newConnHandler(vtg *VTGategate) *connHandler {
	return &connHandler{
		vtg:         vtg,
...
func (vh *connHandler) NewConnection(c *mysql.Conn) {
	vh.mu.Lock()
	defer vh.mu.Unlock()
	vh.connections[c] = true
}

func (vh *connHandler) numConnections() int {
	vh.mu.Lock()
	defer vh.mu.Unlock()
	return len(vh.connections)
}

func (vh *connHandler) ComResetConnection(c *mysql.Conn) {

	// XXX/demmer how to cancel outstanding txns??
...
func (vh *connHandler) ConnectionClosed(c *mysql.Conn) {
...
func (vh *connHandler) ComQuery(c *mysql.Conn, query string, callback func(*sqltypes.Result) error) error {
	ctx := context.Background()
	var cancel context.CancelFunc
	if *mysqlQueryTimeout != 0 {
	...
	span, ctx, err := startSpan(ctx, query, "connHandle.ComQuery")
	if err != nil {
	...
	ef := callerid.NewEffectiveCallerID(
		...
		"VTGategate MySQL Connector" /* subcomponent: part of the client */)
...
func (vh *connHandler) ComPrepare(c *mysql.Conn, query string, bindVars map[string]*querypb.BindVariable) ([]*querypb.Field, error) {
	var ctx context.Context
	var cancel context.CancelFunc
	if *mysqlQueryTimeout != 0 {
		ctx, cancel = context.WithTimeout(context.Background(), *mysqlQueryTimeout)
...
func (vh *connHandler) ComStmtExecute(c *mysql.Conn, prepare *mysql.PrepareData, callback func(*sqltypes.Result) error) error {
	var ctx context.Context
	var cancel context.CancelFunc
	if *mysqlQueryTimeout != 0 {
		ctx, cancel = context.WithTimeout(context.Background(), *mysqlQueryTimeout)
...
func (vh *connHandler) WarningCount(c *mysql.Conn) uint16 {
	return uint16(len(vh.session(c).GetWarnings()))
}

func (vh *connHandler) session(c *mysql.Conn) *vtgatepb.Session {
...
func initMySQLProtocol() {
	...
	if vtGate == nil {
		log.Errorf("vtgate not initialized when mysql protocol initializing")
		return
	}
	...
	// Create a Listener.
	var err error
	connHandler := newConnHandler(vtGate)
	if *mysqlServerPort >= 0 {
		mysqlListener, err = mysql.NewListener(*mysqlTCPVersion, net.JoinHostPort(*mysqlServerBindAddress, fmt.Sprintf("%v", *mysqlServerPort)), authServer, connHandler, *mysqlConnReadTimeout, *mysqlConnWriteTimeout, *mysqlProxyProtocol)
		if err != nil {
	...
	if *mysqlServerSocketPath != "" {
		...
		// clients can connect to VTGategate mysql server without being VTGategate user
		oldMask := syscall.Umask(000)
		mysqlUnixListener, err = newMysqlUnixSocket(*mysqlServerSocketPath, authServer, connHandler)
...
func rollbackAtShutdown() {
	...
	func() {
		connHandle.mu.Lock()
		defer connHandle.mu.Unlock()
		for c := range connHandle.connections {
			log.Infof("Rolling back transactions associated with connection ID: %v", c.ConnectionID)
			c.Close()
		}
	}()

	// If VTGategate is instead busy executing a query, the number of open conns
	// will be non-zero. Give another second for those queries to finish.
	for i := 0; i < 100; i++ {
		if connHandle.numConnections() == 0 {
