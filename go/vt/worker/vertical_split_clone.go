...
func (vscw *VerticalSplitCloneWorker) findTargets(ctx context.Context) error {
	...
	var err error
	vscw.sourceAlias, err = FindWorkerTablet(ctx, vscw.wr, vscw.cleaner, nil /* healthCheck */, nil /* tsc */, vscw.cell, vscw.sourceKeyspace, "0", vscw.minHealthyRdonlyTablets)
	if err != nil {
	...
	defer waitCancel()
	if err := discovery.WaitForTablets(waitCtx, vscw.tsc,
