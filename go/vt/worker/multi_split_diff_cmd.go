...
func commandMultiSplitDiff(wi *Instance, wr *wrangler.Wrangler, subFlags *flag.FlagSet, args []string) (Worker, error) {
	...
	return NewMultiSplitDiffWorker(wr, wi.cell, keyspace, shard, excludeTableArray, excludeShardsArray, *minHealthyRdonlyTablets, *parallelDiffsCount), nil
}
...
func interactiveMultiSplitDiff(ctx context.Context, wi *Instance, wr *wrangler.Wrangler, w http.ResponseWriter, r *http.Request) (Worker, *template.Template, map[string]interface{}, error) {
	...
	// start the diff job
	wrk := NewMultiSplitDiffWorker(wr, wi.cell, keyspace, shard, excludeTableArray, excludeShardsArray, int(minHealthyRdonlyTablets), int(parallelDiffsCount))
