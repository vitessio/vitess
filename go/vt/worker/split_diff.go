...
func (sdw *SplitDiffWorker) findTargets(ctx context.Context) error {
	...
	var err error
	sdw.destinationAlias, err = FindWorkerTablet(ctx, sdw.wr, sdw.cleaner, nil /* healthCheck */, nil /* tsc */, sdw.cell, sdw.keyspace, sdw.shard, sdw.minHealthyRdonlyTablets)
	if err != nil {
	...
	for _, ss := range sdw.shardInfo.SourceShards {
		if ss.Uid == sdw.sourceUID {
			sdw.sourceAlias, err = FindWorkerTablet(ctx, sdw.wr, sdw.cleaner, nil /* healthCheck */, nil /* tsc */, sdw.cell, sdw.keyspace, ss.Shard, sdw.minHealthyRdonlyTablets)
			if err != nil {
