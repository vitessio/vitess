...
func NewMultiSplitDiffWorker(wr *wrangler.Wrangler, cell, keyspace, shard string, excludeTables []string, excludeShards []string, minHealthyRdonlyTablets, parallelDiffsCount int) Worker {
...
func (msdw *MultiSplitDiffWorker) findTargets(ctx context.Context) error {
	...
	msdw.sourceAlias, err = FindWorkerTablet(ctx, msdw.wr, msdw.cleaner, nil /* tsc */, msdw.cell, msdw.keyspace, msdw.shard, msdw.minHealthyRdonlyTablets)
	if err != nil {
	...
	for i, destinationShard := range msdw.destinationShards {
		...
		destinationAlias, err := FindWorkerTablet(ctx, msdw.wr, msdw.cleaner, nil /* tsc */, msdw.cell, keyspace, shard, msdw.minHealthyRdonlyTablets)
		if err != nil {
...
func (msdw *MultiSplitDiffWorker) synchronizeReplication(ctx context.Context) error {
	...
	destBlpPositions := make([]*tabletmanagerdatapb.BlpPosition, len(msdw.destinationShards))
	for i, shardInfo := range msdw.destinationShards {
		masterInfo := masterInfos[i]

		msdw.wr.Logger().Infof("Stopping master binlog replication on %v", shardInfo.MasterAlias)
		shortCtx, cancel := context.WithTimeout(ctx, *remoteActionsTimeout)
		blpPositionList, err := msdw.wr.TabletManagerClient().StopBlp(shortCtx, masterInfo.Tablet)
		destBlpPositions[i] = blpPositionList[msdw.sourceUID]
		cancel()
		if err != nil {
			return fmt.Errorf("StopBlp for %v failed: %v", msdw.shardInfo.MasterAlias, err)
		}
		wrangler.RecordStartBlpAction(msdw.cleaner, masterInfo.Tablet)
	}
	...
	var stoppedAt string // will be the last GTID that we stopped at
	for _, blpPos := range destBlpPositions {
		...
		cancel()
		if err != nil {
			return fmt.Errorf("cannot stop slave %v at right binlog position %v: %v", msdw.sourceAlias, blpPos.Position, err)
		}

		shortCtx, cancel = context.WithTimeout(ctx, *remoteActionsTimeout)
		stoppedAt, err = msdw.wr.TabletManagerClient().StopSlaveMinimum(shortCtx, sourceTablet.Tablet, blpPos.Position, *remoteActionsTimeout)
		cancel()
		if err != nil {
			return fmt.Errorf("cannot stop slave %v at right binlog position %v: %v", msdw.sourceAlias, blpPos.Position, err)
		}
	...
	for i, shardInfo := range msdw.destinationShards {
		...
		msdw.wr.Logger().Infof("Restarting master %v until it catches up to %v", shardInfo.MasterAlias, stopPositionList)
		shortCtx, cancel = context.WithTimeout(ctx, *remoteActionsTimeout)
		masterPos, err := msdw.wr.TabletManagerClient().RunBlpUntil(shortCtx, masterInfo.Tablet, stopPositionList, *remoteActionsTimeout)
		cancel()
		if err != nil {
			return fmt.Errorf("RunBlpUntil for %v until %v failed: %v", shardInfo.MasterAlias, stopPositionList, err)
		}
		...
		shortCtx, cancel = context.WithTimeout(ctx, *remoteActionsTimeout)
		err = msdw.wr.TabletManagerClient().StartBlp(shortCtx, masterInfo.Tablet)
		if err := msdw.cleaner.RemoveActionByName(wrangler.StartBlpActionName, topoproto.TabletAliasString(shardInfo.MasterAlias)); err != nil {
			msdw.wr.Logger().Warningf("Cannot find cleaning action %v/%v: %v", wrangler.StartBlpActionName, topoproto.TabletAliasString(shardInfo.MasterAlias), err)
		}
		cancel()
		if err != nil {
