...
func (scw *SplitCloneWorker) run(ctx context.Context) error {
	// First phase: read what we need to do.
	...
	// Second phase: Find destination master tablets.
	...
	//	// Third phase: (optional) online clone.
	//	if scw.online {
	...
	// Fourth phase: offline clone.
	if scw.offline {
		// Take source tablets out of serving for an exact snapshot.
