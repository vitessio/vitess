...
func (blw *BackfillLookupWorker) init(ctx context.Context) error {
	...
	blw.healthCheck = discovery.NewLegacyHealthCheck(*healthcheckRetryDelay, *healthCheckTimeout)
	...
	for _, si := range allShards {
		watcher := discovery.NewLegacyShardReplicationWatcher(ctx, blw.wr.TopoServer(), blw.healthCheck,
...
func (blw *BackfillLookupWorker) loadVindex(ctx context.Context) error {
	...
	sourceTableDef := kschema.Tables[blw.sourceTable]
...
func (blw *BackfillLookupWorker) findDestinationMasters(ctx context.Context) error {
	...
	for _, si := range blw.destinationShards {
		...
		defer waitCancel()
		if err := blw.tsc.WaitForTablets(waitCtx, si.Keyspace(), si.ShardName(), topodatapb.TabletType_MASTER); err != nil {
		...
		go func(chunk chunk) {
			...
			transform := func(row []sqltypes.Value) ([]sqltypes.Value, bool, error) {
				...
				if blw.destinationAutoIncrement != nil {
					...
					results, err := seqQueryService.Execute(ctx, target, fmt.Sprintf("select next :n values from %s", stripKeyspaceFromSequence(blw.destinationAutoIncrement.Sequence)), bindVars, 0, 0, nil)
					if err != nil {
						return nil, false, err
					}
...
func (blw *BackfillLookupWorker) StatsUpdate(ts *discovery.LegacyTabletStats) {
