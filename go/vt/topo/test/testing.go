...
func CheckLocks(t *testing.T, ts topo.Server) {
	//
	// keyspace locks
	//
	if err := ts.CreateKeyspace("test_keyspace"); err != nil {
	...
	if _, err := ts.LockKeyspaceForAction("test_keyspace", "unused-fake-content", time.Second/2, interrupted); err != topo.ErrTimeout {
	...
	// test we can't lock a non-existint shard
	interrupted = make(chan struct{}, 1)
