	...
	for _, sql := range []string{"show session status", "show session status like 'Ssl_cipher'"} {
		...
		wantqr = &sqltypes.Result{
			Fields: []*querypb.Field{
				...
				{Name: "value", Type: sqltypes.VarChar, Charset: uint32(collations.Local().DefaultConnectionCharset())},
			},
			Rows: [][]sqltypes.Value{
...
func TestGetPlanUnnormalized(t *testing.T) {
	...
	emptyvc, _ := newVCursorImpl(NewSafeSession(&vtgatepb.Session{TargetString: "@unknown"}), makeComments(""), r, nil, r.vm, r.VSchema(), r.resolver.resolver, nil, false, pv, collations.Local())
	unshardedvc, _ := newVCursorImpl(NewSafeSession(&vtgatepb.Session{TargetString: KsTestUnsharded + "@unknown"}), makeComments(""), r, nil, r.vm, r.VSchema(), r.resolver.resolver, nil, false, pv, collations.Local())
...
func TestGetPlanCacheUnnormalized(t *testing.T) {
	...
	t.Run("Skip Cache", func(t *testing.T) {
		...
		ksIDVc1, _ := newVCursorImpl(NewSafeSession(&vtgatepb.Session{TargetString: KsTestUnsharded + "[deadbeef]"}), makeComments(""), r, nil, r.vm, r.VSchema(), r.resolver.resolver, nil, false, pv, collations.Local())
		...
		ksIDVc2, _ := newVCursorImpl(NewSafeSession(&vtgatepb.Session{TargetString: KsTestUnsharded + "[beefdead]"}), makeComments(""), r, nil, r.vm, r.VSchema(), r.resolver.resolver, nil, false, pv, collations.Local())
...
func TestGetPlanPriority(t *testing.T) {
	...
	for _, aTestCase := range testCases {
		...
		t.Run(testCase.name, func(t *testing.T) {
			...
			r.normalize = true
			logStats := logstats.NewLogStats(ctx, "Test", "", "", nil)
			vCursor, err := newVCursorImpl(session, makeComments(""), r, nil, r.vm, r.VSchema(), r.resolver.resolver, nil, false, pv, collations.Local())
			assert.NoError(t, err)
