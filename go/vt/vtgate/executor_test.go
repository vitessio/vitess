...
func TestExecutorTransactionsAutoCommitStreaming(t *testing.T) {
	...
	require.NoError(t, err)
	assert.NotNil(t, result.QueryStats)
	assert.NotZero(t, result.QueryStats.ExecuteDuration)
	assert.NotZero(t, result.QueryStats.PlanDuration)
	assert.Zero(t, result.QueryStats.CommitDuration)
	assert.NotZero(t, result.QueryStats.TotalDuration)
	result, err = executor.Execute(ctx, "TestExecute", session, "commit", nil)
	assert.NotNil(t, result.QueryStats)
	assert.Zero(t, result.QueryStats.ExecuteDuration)
	assert.NotZero(t, result.QueryStats.PlanDuration)
	assert.NotZero(t, result.QueryStats.CommitDuration)
	assert.NotZero(t, result.QueryStats.TotalDuration)
