...
func TestExecutorAddSequenceDDL(t *testing.T) {
	*vschemaacl.AuthorizedDDLUsers = "%"
	...
	if _, err = executor.Execute(ctx, "TestExecute", session, stmt, nil); err != nil {
		t.Error(err)
	}
...
func TestExecutorAddDropVindexDDL(t *testing.T) {
	*vschemaacl.AuthorizedDDLUsers = "%"
	...
	qr, err := executor.Execute(ctx, "TestExecute", session, "show vschema vindexes on TestExecutor.test", nil)
	if err != nil {
	...
	_, err = executor.Execute(ctx, "TestExecute", session, "show vschema vindexes on TestExecutor.test", nil)
	...
	qr, err = executor.Execute(ctx, "TestExecute", session, "show vschema vindexes on TestExecutor.test2", nil)
	if err != nil {
	...
	stmt = "alter vschema on nonexistent drop vindex test_lookup"
	_, err = executor.Execute(ctx, "TestExecute", NewSafeSession(&vtgatepb.Session{TargetString: "InvalidKeyspace"}), stmt, nil)
...
func TestGetPlanUnnormalized(t *testing.T) {
	r, _, _, _ := createExecutorEnv()
	emptyvc, _ := newVCursorImpl(ctx, NewSafeSession(&vtgatepb.Session{TargetString: "@unknown"}), makeComments(""), r, nil, r.vm, r.resolver.resolver)
	unshardedvc, _ := newVCursorImpl(ctx, NewSafeSession(&vtgatepb.Session{TargetString: KsTestUnsharded + "@unknown"}), makeComments(""), r, nil, r.vm, r.resolver.resolver)
