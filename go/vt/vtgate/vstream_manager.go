...
func (vsm *vstreamManager) VStream(ctx context.Context, tabletType topodatapb.TabletType, vgtid *binlogdatapb.VGtid,
	...
	log.InfoS(fmt.Sprintf("VStream flags: minimize_skew=%v, heartbeat_interval=%v, stop_on_reshard=%v, cells=%v, cell_preference=%v, tablet_order=%v, stream_keyspace_heartbeats=%v, include_reshard_journal_events=%v, tables_to_copy=%v, exclude_keyspace_from_table_name=%v, transaction_chunk_size=%v", flags.GetMinimizeSkew(), flags.GetHeartbeatInterval(), flags.GetStopOnReshard(), flags.Cells, flags.CellPreference, flags.TabletOrder,
	...
	if ts == nil {
		log.ErrorS("unable to get topo server in VStream()")
	...
	if flags.TransactionChunkSize > 0 && flags.GetMinimizeSkew() {
		log.WarnS("Minimize skew cannot be set with transaction chunk size (can cause deadlock), ignoring transaction chunk size.")
...
func (vs *vstream) sendEvents(ctx context.Context) {
	...
	send := func(evs []*binlogdatapb.VEvent) error {
		if err := vs.send(evs); err != nil {
			log.InfoS(fmt.Sprintf("Error in vstream send (wrapper) to client: %v", err))
			vs.once.Do(func() {
	...
	for {
		select {
		case <-ctx.Done():
			log.InfoS("vstream context canceled")
			vs.once.Do(func() {
		...
		case evs := <-vs.eventCh:
			if err := send(evs); err != nil {
				log.InfoS(fmt.Sprintf("Error in vstream send events to client: %v", err))
				vs.once.Do(func() {
					vs.setError(err, "error sending events")
				})
				return
		...
		case t := <-heartbeat:
			...
			if err := send(evs); err != nil {
				log.InfoS(fmt.Sprintf("Error in vstream sending heartbeat to client: %v", err))
				vs.once.Do(func() {
		...
		case <-vs.streamLivenessTimer.C:
			...
			log.InfoS("Error in vstream: " + msg)
			vs.once.Do(func() {
...
func (vs *vstream) startOneStream(ctx context.Context, sgtid *binlogdatapb.ShardGtid) {
	...
	go func() {
		...
		if err != nil {
			log.ErrorS(fmt.Sprintf("Error in vstream for %+v: %v", sgtid, err))
...
func (vs *vstream) computeSkew(streamID string, event *binlogdatapb.VEvent) bool {
	...
	if vs.laggard != "" { // we are skewed, check if this event has fixed the skew
		...
		if (maxTs - minTs) > MaxSkew { // check if we are skewed due to this event
			log.InfoS(fmt.Sprintf("Skew found, laggard is %s, %+v", laggardStream, vs.timestamps))
...
func (vs *vstream) alignStreams(ctx context.Context, event *binlogdatapb.VEvent, keyspace, shard string) error {
	...
	for {
		...
		case <-time.After(time.Duration(vs.skewTimeoutSeconds) * time.Second):
			log.ErrorS("timed out while waiting for skew to reduce: " + streamID)
...
func (vs *vstream) streamFromTablet(ctx context.Context, sgtid *binlogdatapb.ShardGtid) error {
	...
	for {
		...
		tabletPickerErr := func(err error) error {
			...
			log.ErrorS(fmt.Sprintf("%v", tperr))
			return tperr
		}
		...
		log.InfoS(fmt.Sprintf("Picked %s tablet %s for VStream in %s/%s within the %s cell(s)", vs.tabletType.String(), tabletAliasString, sgtid.GetKeyspace(), sgtid.GetShard(), strings.Join(cells, ",")))
		...
		go func() {
			_ = tabletConn.StreamHealth(ctx, func(shr *querypb.StreamHealthResponse) error {
				...
				if err != nil {
					log.WarnS(fmt.Sprintf("Tablet state changed: %s, attempting to restart", err))
		...
		log.InfoS(fmt.Sprintf("Starting to vstream from %s, with req %+v", tabletAliasString, req))
		var txLockHeld bool
		...
		err = tabletConn.VStream(ctx, req, func(events []*binlogdatapb.VEvent) error {
			...
			case streamErr := <-errCh:
				log.InfoS(fmt.Sprintf("vstream for %s/%s ended due to health check, should retry: %v", sgtid.Keyspace, sgtid.Shard, streamErr))
			...
			case <-journalDone:
				...
				// This can happen if a server misbehaves and does not end
				// the stream after we return an error.
				log.InfoS(fmt.Sprintf("vstream for %s/%s ended due to journal event, returning io.EOF", sgtid.Keyspace, sgtid.Shard))
				return io.EOF
			default:
			}
			...
			for i, event := range events {
				...
				case binlogdatapb.VEventType_COMMIT, binlogdatapb.VEventType_DDL, binlogdatapb.VEventType_OTHER, binlogdatapb.VEventType_ROLLBACK:
					...
					if sendErr != nil {
						log.InfoS(fmt.Sprintf("vstream for %s/%s, error in sendAll: %v", sgtid.Keyspace, sgtid.Shard, sendErr))
				...
				case binlogdatapb.VEventType_COPY_COMPLETED:
					...
					if err := vs.sendAll(ctx, sgtid, eventss); err != nil {
						log.InfoS(fmt.Sprintf("vstream for %s/%s, error in sendAll, on copy completed event: %v", sgtid.Keyspace, sgtid.Shard, err))
				...
				case binlogdatapb.VEventType_JOURNAL:
					...
					if (vs.includeReshardJournalEvents || vs.stopOnReshard) &&
						...
						eventss = append(eventss, sendevents)
						if err := vs.sendAll(ctx, sgtid, eventss); err != nil {
							log.InfoS(fmt.Sprintf("vstream for %s/%s, error in sendAll, on journal event: %v", sgtid.Keyspace, sgtid.Shard, err))
							return vterrors.Wrap(err, sendingEventsErr)
						}
						eventss = nil
						sendevents = nil
			...
			if vs.isChunkingEnabled() && txLockHeld && !inTransaction {
				log.WarnS("Detected held lock but not in a transaction, releasing the lock")
				vs.mu.Unlock()
				txLockHeld = false
			}
			...
			if vs.isChunkingEnabled() && txLockHeld && len(eventss) > 0 {
				if err := vs.sendEventsLocked(ctx, sgtid, eventss); err != nil {
					log.InfoS(fmt.Sprintf("vstream for %s/%s, error in sendAll at end of callback: %v", sgtid.Keyspace, sgtid.Shard, err))
					return vterrors.Wrap(err, sendingEventsErr)
				}
				eventss = nil
			}
			...
			if vs.isChunkingEnabled() && inTransaction && !txLockHeld && accumulatedSize > vs.transactionChunkSizeBytes {
				log.InfoS(fmt.Sprintf("vstream for %s/%s: transaction size %d bytes exceeds chunk size %d bytes, acquiring lock for contiguous, chunked delivery", sgtid.Keyspace, sgtid.Shard, accumulatedSize, vs.transactionChunkSizeBytes))
				vs.vsm.vstreamsTransactionsChunked.Add(labelValues, 1)
				vs.mu.Lock()
				txLockHeld = true
				if len(eventss) > 0 {
					if err := vs.sendEventsLocked(ctx, sgtid, eventss); err != nil {
						log.InfoS(fmt.Sprintf("vstream for %s/%s, error sending events after acquiring lock: %v", sgtid.Keyspace, sgtid.Shard, err))
						return vterrors.Wrap(err, sendingEventsErr)
					}
					eventss = nil
				}
		...
		if !retry {
			log.InfoS(fmt.Sprintf("vstream for %s/%s error, no retry: %v", sgtid.Keyspace, sgtid.Shard, err))
		...
		if errCount >= 3 {
			log.ErrorS(fmt.Sprintf("vstream for %s/%s had three consecutive failures: %v", sgtid.Keyspace, sgtid.Shard, err))
			return vterrors.Wrapf(err, "persistent error in vstream for %s/%s on tablet %s; giving up",
				sgtid.Keyspace, sgtid.Shard, tabletAliasString)
		}
		log.InfoS(fmt.Sprintf("vstream for %s/%s error, retrying: %v", sgtid.Keyspace, sgtid.Shard, err))
	}
}
...
func (vs *vstream) getJournalEvent(ctx context.Context, sgtid *binlogdatapb.ShardGtid, journal *binlogdatapb.Journal) (*journalEvent, error) {
	...
	if !ok {
		log.InfoS(fmt.Sprintf("Journal event received: %v", journal))
	...
	if !vs.stopOnReshard { // stop streaming from current shards and start streaming the new shards
		...
		log.InfoS(fmt.Sprintf("Removing shard gtids: %v", je.participants))
		...
		log.InfoS(fmt.Sprintf("Adding shard gtids: %v", je.journal.ShardGtids))
