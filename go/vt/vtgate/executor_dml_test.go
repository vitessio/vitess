...
func TestUpdateEqual(t *testing.T) {
	...
	wantQueries = []*querypb.BoundQuery{{
		...
		BindVariables: map[string]*querypb.BindVariable{
			"music_id": sqltypes.Int64BindVar(2),
		},
	}}
...
func TestDeleteEqual(t *testing.T) {
	...
	wantQueries = []*querypb.BoundQuery{{
		...
		BindVariables: map[string]*querypb.BindVariable{
			"user_id": sqltypes.Uint64BindVar(1),
			"name":    sqltypes.StringBindVar("myname"),
		},
	}}
	if !reflect.DeepEqual(sbclookup.Queries, wantQueries) {
	...
	wantQueries = []*querypb.BoundQuery{{
		Sql: "select user_id from music_user_map where music_id = :music_id",
		BindVariables: map[string]*querypb.BindVariable{
			"music_id": sqltypes.Int64BindVar(1),
		},
	}}
	if !reflect.DeepEqual(sbclookup.Queries, wantQueries) {
		t.Errorf("sbclookup.Queries: %+v, want %+v\n", sbclookup.Queries, wantQueries)
...
func TestInsertSharded(t *testing.T) {
	...
	wantQueries := []*querypb.BoundQuery{{
		...
		BindVariables: map[string]*querypb.BindVariable{
			"_Id0":   sqltypes.Int64BindVar(1),
			"_name0": sqltypes.BytesBindVar([]byte("myname")),
			"__seq0": sqltypes.Int64BindVar(1),
		},
	}}
	if !reflect.DeepEqual(sbc1.Queries, wantQueries) {
	...
	wantQueries = []*querypb.BoundQuery{{
		...
		BindVariables: map[string]*querypb.BindVariable{
			"name0":    sqltypes.BytesBindVar([]byte("myname")),
			"user_id0": sqltypes.Uint64BindVar(1),
		},
	}}
	if !reflect.DeepEqual(sbclookup.Queries, wantQueries) {
	...
	wantQueries = []*querypb.BoundQuery{{
		...
		BindVariables: map[string]*querypb.BindVariable{
			"_Id0":   sqltypes.Int64BindVar(3),
			"__seq0": sqltypes.Int64BindVar(3),
			"_name0": sqltypes.BytesBindVar([]byte("myname2")),
		},
	}}
	if !reflect.DeepEqual(sbc2.Queries, wantQueries) {
	...
	wantQueries = []*querypb.BoundQuery{{
		Sql: "insert into name_user_map(name, user_id) values (:name0, :user_id0)",
		BindVariables: map[string]*querypb.BindVariable{
			"name0":    sqltypes.BytesBindVar([]byte("myname2")),
			"user_id0": sqltypes.Uint64BindVar(3),
		},
	}}
	if !reflect.DeepEqual(sbclookup.Queries, wantQueries) {
		t.Errorf("sbclookup.Queries: \n%+v, want \n%+v\n", sbclookup.Queries, wantQueries)
...
func TestInsertGeneratorSharded(t *testing.T) {
	...
	wantQueries = []*querypb.BoundQuery{{
		...
		BindVariables: map[string]*querypb.BindVariable{"n": sqltypes.Int64BindVar(1)},
	}, {
		Sql: "insert into name_user_map(name, user_id) values (:name0, :user_id0)",
		BindVariables: map[string]*querypb.BindVariable{
			"name0":    sqltypes.BytesBindVar([]byte("myname")),
...
func TestInsertAutoincSharded(t *testing.T) {
	...
	wantQueries := []*querypb.BoundQuery{{
		...
		BindVariables: map[string]*querypb.BindVariable{
			"_user_id0": sqltypes.Int64BindVar(2),
		},
	}}
	if !reflect.DeepEqual(sbc.Queries, wantQueries) {
		t.Errorf("sbc.Queries:\n%+v, want\n%+v\n", sbc.Queries, wantQueries)
...
func TestInsertLookupOwned(t *testing.T) {
	...
	wantQueries = []*querypb.BoundQuery{{
		...
		BindVariables: map[string]*querypb.BindVariable{
			"music_id0": sqltypes.Int64BindVar(3),
			"user_id0":  sqltypes.Uint64BindVar(2),
		},
	}}
	if !reflect.DeepEqual(sbclookup.Queries, wantQueries) {
		t.Errorf("sbclookup.Queries: %+v, want %+v\n", sbclookup.Queries, wantQueries)
...
func TestInsertLookupOwnedGenerator(t *testing.T) {
	...
	wantQueries := []*querypb.BoundQuery{{
		...
		BindVariables: map[string]*querypb.BindVariable{
			"_user_id0": sqltypes.Int64BindVar(2),
			"_id0":      sqltypes.Int64BindVar(4),
			"__seq0":    sqltypes.Int64BindVar(4),
		},
	}}
	if !reflect.DeepEqual(sbc.Queries, wantQueries) {
		t.Errorf("sbc.Queries:\n%+v, want\n%+v\n", sbc.Queries, wantQueries)
	...
	wantQueries = []*querypb.BoundQuery{{
		...
		Sql: "insert into music_user_map(music_id, user_id) values (:music_id0, :user_id0)",
		BindVariables: map[string]*querypb.BindVariable{
			"music_id0": sqltypes.Int64BindVar(4),
			"user_id0":  sqltypes.Uint64BindVar(2),
		},
	}}
	if !reflect.DeepEqual(sbclookup.Queries, wantQueries) {
...
func TestInsertLookupUnowned(t *testing.T) {
	...
	wantQueries := []*querypb.BoundQuery{{
		...
		BindVariables: map[string]*querypb.BindVariable{
			"_user_id0":  sqltypes.Int64BindVar(2),
			"_music_id0": sqltypes.Int64BindVar(3),
		},
	}}
	if !reflect.DeepEqual(sbc.Queries, wantQueries) {
		t.Errorf("sbc.Queries: %+v, want %+v\n", sbc.Queries, wantQueries)
...
func TestInsertLookupUnownedUnsupplied(t *testing.T) {
	...
	wantQueries := []*querypb.BoundQuery{{
		...
		BindVariables: map[string]*querypb.BindVariable{
			"_user_id0":  sqltypes.Uint64BindVar(1),
			"_music_id0": sqltypes.Int64BindVar(3),
		},
	}}
	if !reflect.DeepEqual(sbc.Queries, wantQueries) {
	...
	wantQueries = []*querypb.BoundQuery{{
		Sql: "select user_id from music_user_map where music_id = :music_id",
		BindVariables: map[string]*querypb.BindVariable{
			"music_id": sqltypes.Int64BindVar(3),
		},
	}}
	if !reflect.DeepEqual(sbclookup.Queries, wantQueries) {
		t.Errorf("sbclookup.Queries: %+v, want %+v\n", sbclookup.Queries, wantQueries)
...
func TestMultiInsertSharded(t *testing.T) {
	...
	wantQueries1 := []*querypb.BoundQuery{{
		...
		BindVariables: map[string]*querypb.BindVariable{
			"_Id0":   sqltypes.Int64BindVar(1),
			"_name0": sqltypes.BytesBindVar([]byte("myname1")),
			"__seq0": sqltypes.Int64BindVar(1),
			"_Id1":   sqltypes.Int64BindVar(3),
			"_name1": sqltypes.BytesBindVar([]byte("myname3")),
			"__seq1": sqltypes.Int64BindVar(3),
		},
	}}
	...
	wantQueries1 = []*querypb.BoundQuery{{
		...
		BindVariables: map[string]*querypb.BindVariable{
			"name0":    sqltypes.BytesBindVar([]byte("myname1")),
			"user_id0": sqltypes.Uint64BindVar(1),
			"name1":    sqltypes.BytesBindVar([]byte("myname3")),
			"user_id1": sqltypes.Uint64BindVar(3),
		},
	}}
	...
	wantQueries := []*querypb.BoundQuery{{
		...
		BindVariables: map[string]*querypb.BindVariable{
			...
			"__seq0": sqltypes.Int64BindVar(1),
			"_name0": sqltypes.BytesBindVar([]byte("myname1")),
			"_Id1":   sqltypes.Int64BindVar(2),
			"__seq1": sqltypes.Int64BindVar(2),
			"_name1": sqltypes.BytesBindVar([]byte("myname2")),
		},
	}}
	...
	wantQueries = []*querypb.BoundQuery{{
		...
		BindVariables: map[string]*querypb.BindVariable{
			"name0":    sqltypes.BytesBindVar([]byte("myname1")),
			"user_id0": sqltypes.Uint64BindVar(1),
			"name1":    sqltypes.BytesBindVar([]byte("myname2")),
			"user_id1": sqltypes.Uint64BindVar(2),
		},
	}}
	if !reflect.DeepEqual(sbclookup.Queries, wantQueries) {
		t.Errorf("sbclookup.Queries: \n%+v, want \n%+v\n", sbclookup.Queries, wantQueries)
...
func TestMultiInsertGenerator(t *testing.T) {
	...
	result, err := executorExec(executor, "insert into music(user_id, name) values (:u, 'myname1'),(:u, 'myname2')", map[string]*querypb.BindVariable{"u": sqltypes.Int64BindVar(2)})
	if err != nil {
		t.Error(err)
	}
	wantQueries := []*querypb.BoundQuery{{
		...
		BindVariables: map[string]*querypb.BindVariable{
			"u":         sqltypes.Int64BindVar(2),
			"_id0":      sqltypes.Int64BindVar(1),
			"__seq0":    sqltypes.Int64BindVar(1),
			"_user_id0": sqltypes.Int64BindVar(2),
			"_id1":      sqltypes.Int64BindVar(2),
			"__seq1":    sqltypes.Int64BindVar(2),
			"_user_id1": sqltypes.Int64BindVar(2),
		},
	}}
	if !reflect.DeepEqual(sbc.Queries, wantQueries) {
		t.Errorf("sbc.Queries:\n%+v, want\n%+v\n", sbc.Queries, wantQueries)
	...
	wantQueries = []*querypb.BoundQuery{{
		...
		BindVariables: map[string]*querypb.BindVariable{
			"user_id0":  sqltypes.Uint64BindVar(2),
			"music_id0": sqltypes.Int64BindVar(1),
			"user_id1":  sqltypes.Uint64BindVar(2),
			"music_id1": sqltypes.Int64BindVar(2),
		},
	}}
	if !reflect.DeepEqual(sbclookup.Queries, wantQueries) {
		t.Errorf("sbclookup.Queries: \n%#v, want \n%#v\n", sbclookup.Queries, wantQueries)
...
func TestMultiInsertGeneratorSparse(t *testing.T) {
	...
	result, err := executorExec(executor, "insert into music(id, user_id, name) values (NULL, :u, 'myname1'),(2, :u, 'myname2'), (NULL, :u, 'myname3')", map[string]*querypb.BindVariable{"u": sqltypes.Int64BindVar(2)})
	if err != nil {
		t.Error(err)
	}
	wantQueries := []*querypb.BoundQuery{{
		...
		BindVariables: map[string]*querypb.BindVariable{
			...
			"__seq1":    sqltypes.Int64BindVar(2),
			"_user_id1": sqltypes.Int64BindVar(2),
			"_id2":      sqltypes.Int64BindVar(2),
			"__seq2":    sqltypes.Int64BindVar(2),
			"_user_id2": sqltypes.Int64BindVar(2),
		},
	}}
	if !reflect.DeepEqual(sbc.Queries, wantQueries) {
		t.Errorf("sbc.Queries:\n%+v, want\n%+v\n", sbc.Queries, wantQueries)
	...
	wantQueries = []*querypb.BoundQuery{{
		...
		BindVariables: map[string]*querypb.BindVariable{
			...
			"user_id1":  sqltypes.Uint64BindVar(2),
			"music_id1": sqltypes.Int64BindVar(2),
			"user_id2":  sqltypes.Uint64BindVar(2),
			"music_id2": sqltypes.Int64BindVar(2),
		},
	}}
	if !reflect.DeepEqual(sbclookup.Queries, wantQueries) {
		t.Errorf("sbclookup.Queries: \n%#v, want \n%#v\n", sbclookup.Queries, wantQueries)
