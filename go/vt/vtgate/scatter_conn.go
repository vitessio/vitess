			...
			case begin:
				...
				if err != nil {
					retryRequest(func() {
						...
						innerqr, transactionID, reservedID, alias, err = qs.ReserveBeginExecute(ctx, rs.Target, session.SetPreQueriesWithSavepoint(), queries[i].Sql, queries[i].BindVariables, opts)
					})
				}
			case reserve:
