...
func (stc *ScatterConn) SplitQuery(ctx context.Context, query tproto.BoundQuery, splitCount int, keyRangeByShard map[string]kproto.KeyRange, keyspace string) ([]proto.QuerySplit, error) {
	...
	for s := range allSplits {
		splits = append(splits, s.([]proto.QuerySplit)...)
	}
	if allErrors.HasErrors() {
