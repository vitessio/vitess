...
import (
	...
	"vitess.io/vitess/go/vt/vtgate/utils"
...
func (vm *VSchemaManager) buildAndEnhanceVSchema(v *vschemapb.SrvVSchema) *vindexes.VSchema {
	...
	if vm.schema != nil {
		...
		// We also order the tables so that that all CASCADE foreign keys have child tables coming before parent tables.
		utils.MarkErrorIfCyclesInFkAndOrderTables(vschema)
	}
	return vschema
}
