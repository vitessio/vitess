...
// (LIKE/REGEXP matching, the REGEXP_* functions, JSON_UNQUOTE, CAST/CONVERT
// to a character type, the LENGTH family) are rejected when the merged
// document can contain a JSON object or is subtype-lossy: the evalengine
// re-serializes such a document with lexicographic object key order (MySQL
// orders keys by length, then bytes) and float-formatted numbers, so the
// operation's result can differ from MySQL. Provenance is resolved through
// output offsets, and unknown provenance never silently degrades to
// independent.
...
func (v *mergedJSONValidator) validateExpr(expr sqlparser.Expr) error {
	return sqlparser.Walk(func(node sqlparser.SQLNode) (bool, error) {
		...
		case *sqlparser.FuncExpr:
			...
			case "length", "octet_length", "char_length", "character_length", "bit_length":
				if len(node.Exprs) == 1 {
					err = v.rejectTextSerialization(strings.ToUpper(node.Name.Lowered()),
						[]sqlparser.Expr{node.Exprs[0]}, node)
				}
			}
		}
...
// a character type and the LENGTH family: the evalengine serializes a JSON
// operand with lexicographic object key order (MySQL orders keys by length,
// then bytes) and reformats numbers whose scalar subtype the shard protocol
// erased, so merged operands able to contain an object or subtype-lossy
// reject, as does unproved provenance.
