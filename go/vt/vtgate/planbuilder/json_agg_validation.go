...
// those subtypes. Operations that consume the document's text serialization
// (LIKE/REGEXP matching, the REGEXP_* functions, JSON_UNQUOTE, CAST/CONVERT to
// a character or binary type, COLLATE, and every function that coerces its
// argument to text, such as LOWER, CONCAT, SUBSTRING, HEX or the LENGTH
// family) are rejected when the merged document can contain a JSON object or
// is subtype-lossy: the evalengine re-serializes such a document with
// lexicographic object key order (MySQL orders keys by length, then bytes) and
// float-formatted numbers, so the operation's result can differ from MySQL.
// Other generic function calls fail closed; only COALESCE, IFNULL, IF and
// ISNULL are exempt. LOWER and UPPER are rejected over every merged aggregate,
// since the evalengine keeps the JSON type on their result, and JSON_KEYS is
// rejected under the text rule, since vtgate lists object members in a
// different order. JSON_EXTRACT is not: vtgate sees a normalized path only as
// a bind variable when it plans, extraction by key is order-independent, and a
// wildcard path inherits the key-order divergence any JSON value evaluated at
// vtgate already has. Provenance is resolved through output offsets, and
// unknown provenance never silently degrades to independent.
...
const (
	...
	riskMergedJSON
	// riskObjectCapableMergedJSON: as riskMergedJSON, and the merged document
	// can contain a JSON object, whose keys the evalengine re-serializes in
	// lexicographic rather than MySQL's length-then-bytes order.
	riskObjectCapableMergedJSON
...
// mergedJSON reports whether the risk denotes a proved merge dependency.
func (r outputRisk) mergedJSON() bool {
	return r == riskMergedJSON || r == riskObjectCapableMergedJSON || r == riskSubtypeLossyMergedJSON
}
...
func combineRisk(a, b outputRisk) outputRisk {
	...
	case a == riskObjectCapableMergedJSON || b == riskObjectCapableMergedJSON:
		return riskObjectCapableMergedJSON
...
func (r *outputRiskResolver) aggregatorRisk(agg *operators.Aggregator, offset int) outputRisk {
	...
	for _, a := range agg.Aggregations {
		...
		case opcode.AggregateJSONArrayMerge, opcode.AggregateJSONObjectMerge:
			...
			if a.OpCode == opcode.AggregateJSONObjectMerge {
				// The merged document is an object. A json_arrayagg whose
				// elements could be objects (JSON or unknown value type) is
				// already subtype-lossy above.
				return riskObjectCapableMergedJSON
			}
			return riskMergedJSON
...
// textDivergentJSONOperand: a proved merge dependency whose text
// serialization at vtgate can differ from MySQL's bytes — the merged document
// can contain a JSON object (lexicographic instead of length-then-bytes key
// order) or is subtype-lossy (numbers reformat through float parsing) — and
// that is JSON or of unresolvable type.
func (c candidateAnalysis) textDivergentJSONOperand() bool {
	return (c.risk == riskObjectCapableMergedJSON || c.risk == riskSubtypeLossyMergedJSON) &&
		(!c.typeKnown || c.isJSON())
}
	...
	return sqlparser.Walk(func(node sqlparser.SQLNode) (bool, error) {
		...
		case *sqlparser.ComparisonExpr:
			...
			case sqlparser.LikeOp, sqlparser.NotLikeOp, sqlparser.RegexpOp, sqlparser.NotRegexpOp:
				// String matching consumes the text serialization of a JSON
				// operand, which the evalengine reproduces byte-identically
				// to MySQL only for merged documents that cannot contain an
				// object and are not subtype-lossy.
				err = v.rejectTextSerialization(strings.ToUpper(node.Operator.ToString()),
					[]sqlparser.Expr{node.Left, node.Right}, node)
		...
		case *sqlparser.RegexpLikeExpr:
			err = v.rejectTextSerialization("REGEXP_LIKE", []sqlparser.Expr{node.Expr, node.Pattern}, node)
		case *sqlparser.RegexpInstrExpr:
			err = v.rejectTextSerialization("REGEXP_INSTR", []sqlparser.Expr{node.Expr, node.Pattern}, node)
		case *sqlparser.RegexpSubstrExpr:
			err = v.rejectTextSerialization("REGEXP_SUBSTR", []sqlparser.Expr{node.Expr, node.Pattern}, node)
		case *sqlparser.RegexpReplaceExpr:
			err = v.rejectTextSerialization("REGEXP_REPLACE", []sqlparser.Expr{node.Expr, node.Pattern, node.Repl}, node)
		case *sqlparser.JSONUnquoteExpr:
			err = v.rejectTextSerialization("JSON_UNQUOTE", []sqlparser.Expr{node.JSONValue}, node)
		case *sqlparser.JSONKeysExpr:
			err = v.rejectMemberOrder("JSON_KEYS", node.JSONDoc, node)
		case *sqlparser.CastExpr:
			if classifyConvertType(node.Type) == wrapperReturnsText {
				err = v.rejectTextSerialization("CAST", []sqlparser.Expr{node.Expr}, node)
			}
		case *sqlparser.ConvertExpr:
			if classifyConvertType(node.Type) == wrapperReturnsText {
				err = v.rejectTextSerialization("CONVERT", []sqlparser.Expr{node.Expr}, node)
			}
		case *sqlparser.ConvertUsingExpr:
			err = v.rejectTextSerialization("CONVERT", []sqlparser.Expr{node.Expr}, node)
		case *sqlparser.CollateExpr:
			err = v.rejectTextSerialization("COLLATE", []sqlparser.Expr{node.Expr}, node)
		case *sqlparser.SubstrExpr:
			err = v.rejectTextSerialization("SUBSTRING", []sqlparser.Expr{node.Name}, node)
		case *sqlparser.TrimFuncExpr:
			err = v.rejectTextSerialization("TRIM", []sqlparser.Expr{node.StringArg, node.TrimArg}, node)
		case *sqlparser.LocateExpr:
			err = v.rejectTextSerialization("LOCATE", []sqlparser.Expr{node.SubStr, node.Str}, node)
		case *sqlparser.InsertExpr:
			err = v.rejectTextSerialization("INSERT", []sqlparser.Expr{node.Str, node.NewStr}, node)
		case *sqlparser.WeightStringFuncExpr:
			err = v.rejectTextSerialization("WEIGHT_STRING", []sqlparser.Expr{node.Expr}, node)
		case *sqlparser.FuncExpr:
			...
			case "lower", "lcase", "upper", "ucase":
				if err = v.rejectTextSerialization(strings.ToUpper(node.Name.Lowered()), node.Exprs, node); err == nil {
					err = v.validateChangeCase(node)
				}
			case "coalesce", "ifnull", "if", "isnull":
				// The operand is returned, null-tested or truth-tested,
				// never serialized.
			default:
				// Every other function consumes a JSON argument through its
				// text serialization (CONCAT, HEX, MD5, STRCMP, the LENGTH family,
				// ...); functions unknown here fail closed.
				err = v.rejectTextSerialization(strings.ToUpper(node.Name.Lowered()), node.Exprs, node)
			}
		}
...
// validateChangeCase rejects LOWER and UPPER over any merged JSON operand:
// the evalengine keeps the JSON type on the case-mapped result.
func (v *mergedJSONValidator) validateChangeCase(node *sqlparser.FuncExpr) error {
	op := strings.ToUpper(node.Name.Lowered())
	for _, arg := range node.Exprs {
		c := v.candidate(arg)
		if c.mergedJSONOperand() {
			return vterrors.VT12001(fmt.Sprintf(
				"%s over a vtgate-merged JSON aggregate is not yet supported (the evalengine returns a JSON-typed result where MySQL returns text): %s",
				op, sqlparser.String(printableExpr(node))))
		}
		if c.unknownMergeProvenance() {
			return v.unknownProvenanceError(op, node)
	...
	for _, expr := range []sqlparser.Expr{left, right} {
		...
		if c.lossyJSONOperand() {
			return vterrors.VT12001(
				"comparison consumes a vtgate-merged JSON aggregate whose MySQL binary, bit, decimal, or temporal scalar subtype is not preserved across shard serialization: " +
...
// rejectTextSerialization applies the shared text-consumer policy of
// LIKE/REGEXP matching, the REGEXP_* functions, JSON_UNQUOTE, CAST/CONVERT to
// a character or binary type, COLLATE and every function that coerces its
// argument to text: the evalengine serializes a JSON operand with
// lexicographic object key order (MySQL orders keys by length, then bytes)
// and reformats numbers whose scalar subtype the shard protocol erased, so
// merged operands able to contain an object or subtype-lossy reject, as does
// unproved provenance. A nil candidate is an omitted optional argument.
func (v *mergedJSONValidator) rejectTextSerialization(op string, candidates []sqlparser.Expr, node sqlparser.Expr) error {
	for _, expr := range candidates {
		if expr == nil {
			continue
		}
		c := v.candidate(expr)
		if c.textDivergentJSONOperand() {
			return vterrors.VT12001(fmt.Sprintf(
				"%s consumes a vtgate-merged JSON aggregate whose text serialization can differ from MySQL's (object key order, number formatting): %s",
				op, sqlparser.String(printableExpr(node))))
		}
		if c.unknownMergeProvenance() {
			return v.unknownProvenanceError(op, node)
...
// rejectMemberOrder rejects operations whose result lists object members in
// document order, which vtgate keeps lexicographic.
func (v *mergedJSONValidator) rejectMemberOrder(op string, doc sqlparser.Expr, node sqlparser.Expr) error {
	c := v.candidate(doc)
	if c.textDivergentJSONOperand() {
		return vterrors.VT12001(fmt.Sprintf(
			"%s lists the members of a vtgate-merged JSON aggregate in a different order than MySQL (object key order): %s",
			op, sqlparser.String(printableExpr(node))))
	}
	if c.unknownMergeProvenance() {
		return v.unknownProvenanceError(op, node)
