...
func (ms *mergeSort) Wireup(bldr builder, jt *jointab) error {
	...
	rb := ms.input.(*route)
	for i, orderby := range rb.ro.eroute.OrderBy {
		...
		if sqltypes.IsText(rc.column.typ) {
			...
			if colNumber, ok := ms.weightStrings[rc]; ok {
				rb.ro.eroute.OrderBy[i].Col = colNumber
				continue
			}
			var err error
			rb.ro.eroute.OrderBy[i].Col, err = rb.SupplyWeightString(orderby.Col)
			if err != nil {
				return err
			}
	...
	rb.ro.eroute.TruncateColumnCount = ms.truncateColumnCount
