...
func planOrdering(pb *primitiveBuilder, input logicalPlan, orderBy sqlparser.OrderBy) (logicalPlan, error) {
	...
	case *orderedAggregate:
		...
		referenced := make([]bool, len(node.eaggr.Keys))
		postSort := false
		...
		for _, order := range orderBy {
			...
			switch expr := order.Expr.(type) {
			case *sqlparser.Literal:
				num, err := ResultFromNumber(node.resultColumns, expr)
				if err != nil {
					return nil, err
				}
				orderByCol = node.resultColumns[num].column
			case *sqlparser.ColName:
			...
			found := false
			for j, key := range node.eaggr.Keys {
				if node.resultColumns[key].column != orderByCol {
					continue
				}
		...
		for i, key := range node.eaggr.Keys {
			if referenced[i] {
				continue
			}
			// Build a brand new reference for the key.
			col, err := BuildColName(node.input.ResultColumns(), key)
			if err != nil {
		...
		if node.extraDistinct != nil {
			selOrderBy = append(selOrderBy, &sqlparser.Order{Expr: node.extraDistinct, Direction: sqlparser.AscOrder})
		}
		...
		bldr, err := planOrdering(pb, node.input, selOrderBy)
		if err != nil {
			return nil, err
		}
		node.input = bldr
