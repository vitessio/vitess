...
func (jp *joinTree) pushPredicate(ctx *plancontext.PlanningContext, expr sqlparser.Expr) error {
	isPushed := false
...
func (jp *joinTree) removePredicate(ctx *plancontext.PlanningContext, expr sqlparser.Expr) error {
	isRemoved := false
	if ctx.SemTable.RecursiveDeps(expr).IsSolvedBy(jp.lhs.tableID()) {
