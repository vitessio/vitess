...
func TestSemTableDependenciesAfterExpandStar(t *testing.T) {
	...
	for _, tcase := range tcases {
		t.Run(tcase.sql, func(t *testing.T) {
			...
			if tcase.otherTbl != -1 {
				assert.NotEqual(t,
					semTable.RecursiveDependencies(expandedSelect.SelectExprs[tcase.otherTbl].(*sqlparser.AliasedExpr).Expr),
					semTable.RecursiveDependencies(expandedSelect.SelectExprs[tcase.expandedCol].(*sqlparser.AliasedExpr).Expr),
				)
			}
			if tcase.sameTbl != -1 {
				assert.Equal(t,
					semTable.RecursiveDependencies(expandedSelect.SelectExprs[tcase.sameTbl].(*sqlparser.AliasedExpr).Expr),
					semTable.RecursiveDependencies(expandedSelect.SelectExprs[tcase.expandedCol].(*sqlparser.AliasedExpr).Expr),
				)
			}
		})
