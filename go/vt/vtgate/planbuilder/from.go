...
func (pb *primitiveBuilder) processDMLTable(tableExprs sqlparser.TableExprs) (*routeOption, error) {
	if err := pb.processTableExprs(tableExprs, false, false); err != nil {
		return nil, err
	}
	rb, ok := pb.bldr.(*route)
	if !ok {
...
func (pb *primitiveBuilder) processTableExprs(tableExprs sqlparser.TableExprs, useHashJoin, useNestedHashJoin bool) error {
	if len(tableExprs) == 1 {
		return pb.processTableExpr(tableExprs[0], useHashJoin, useNestedHashJoin)
	}

	if err := pb.processTableExpr(tableExprs[0], useHashJoin, useNestedHashJoin); err != nil {
		return err
	}
	rpb := newPrimitiveBuilder(pb.vschema, pb.jt)
	if err := rpb.processTableExprs(tableExprs[1:], useHashJoin, useNestedHashJoin); err != nil {
		return err
	}
	return pb.join(rpb, nil, useHashJoin, useNestedHashJoin)
}

// processTableExpr produces a builder subtree for the given TableExpr.
func (pb *primitiveBuilder) processTableExpr(tableExpr sqlparser.TableExpr, useHashJoin, useNestedHashJoin bool) error {
	switch tableExpr := tableExpr.(type) {
	case *sqlparser.AliasedTableExpr:
		return pb.processAliasedTable(tableExpr)
	case *sqlparser.ParenTableExpr:
		err := pb.processTableExprs(tableExpr.Exprs, useHashJoin, useNestedHashJoin)
	...
	case *sqlparser.JoinTableExpr:
		return pb.processJoin(tableExpr, useHashJoin, useNestedHashJoin)
	}
...
func (pb *primitiveBuilder) processJoin(ajoin *sqlparser.JoinTableExpr, useHashJoin, useNestedHashJoin bool) error {
	switch ajoin.Join {
	...
	if err := pb.processTableExpr(ajoin.LeftExpr, useHashJoin, useNestedHashJoin); err != nil {
		return err
	}
	rpb := newPrimitiveBuilder(pb.vschema, pb.jt)
	if err := rpb.processTableExpr(ajoin.RightExpr, useHashJoin, useNestedHashJoin); err != nil {
		return err
	}
	return pb.join(rpb, ajoin, useHashJoin, useNestedHashJoin)
}
...
func (pb *primitiveBuilder) join(rpb *primitiveBuilder, ajoin *sqlparser.JoinTableExpr, useHashJoin, useNestedHashJoin bool) error {
	...
	if !leftIsRoute || !rightIsRoute {
		return newJoin(pb, rpb, ajoin, useHashJoin, useNestedHashJoin)
	}
