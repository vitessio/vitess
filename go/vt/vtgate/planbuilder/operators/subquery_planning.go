...
func isMergeable(ctx *plancontext.PlanningContext, query sqlparser.OutputsTable, op Operator) bool {
...
func (s *subqueryRouteMerger) rewriteASTExpression(ctx *plancontext.PlanningContext, inner *Route) Operator {
	...
	subqStmt, ok := stmt.(sqlparser.OutputsTable)
	if !ok {
