...
func (f *Filter) Compact(*plancontext.PlanningContext) (Operator, *ApplyResult, error) {
	if len(f.Predicates) == 0 {
		return f.Source, Rewrote("filter with no predicates removed"), nil
	}

	other, isFilter := f.Source.(*Filter)
	if !isFilter {
		return f, NoRewrite, nil
	}
	f.Source = other.Source
	f.Predicates = append(f.Predicates, other.Predicates...)
	return f, Rewrote("two filters merged into one"), nil
}
