...
func buildDeletePlan(stmt sqlparser.Statement, reservedVars *sqlparser.ReservedVars, vschema plancontext.VSchema) (engine.Primitive, error) {
	...
	if len(del.Targets) == 1 && del.Targets[0].Name != edel.Table[0].Name {
	...
	if len(edel.Table[0].Owned) > 0 {
		...
		tblExpr := &sqlparser.AliasedTableExpr{Expr: sqlparser.TableName{Name: edel.Table[0].Name}, As: aTblExpr.As}
		edel.OwnedVindexQuery = generateDMLSubquery(tblExpr, del.Where, del.OrderBy, del.Limit, edel.Table[0], ksidVindex.Columns)
