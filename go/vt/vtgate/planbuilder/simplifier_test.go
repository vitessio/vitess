...
func TestUnsupportedFile(t *testing.T) {
	...
	for _, tcase := range readJSONTests("unsupported_cases.txt") {
		t.Run(tcase.Query, func(t *testing.T) {
			...
			require.NoError(t, err)
			_, ok := stmt.(sqlparser.TableSubquery)
			if !ok {
				t.Skip()
				return
			}
...
func keepSameError(query string, reservedVars *sqlparser.ReservedVars, vschema *vschemawrapper.VSchemaWrapper, needs *sqlparser.BindVarNeeds) func(statement sqlparser.TableSubquery) bool {
	...
	return func(statement sqlparser.TableSubquery) bool {
...
func keepPanicking(query string, reservedVars *sqlparser.ReservedVars, vschema *vschemawrapper.VSchemaWrapper, needs *sqlparser.BindVarNeeds) func(statement sqlparser.TableSubquery) bool {
	cmp := func(statement sqlparser.TableSubquery) (res bool) {
		defer func() {
			r := recover()
			if r != nil {
	...
	if !cmp(stmt.(sqlparser.TableSubquery)) {
