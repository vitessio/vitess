...
func TestSimplifyBuggyQuery(t *testing.T) {
	query := "(select id from unsharded, unsharded_auto group by count(unsharded.predef1) union select id from unsharded_auto) union (select id from user union select name from unsharded)"
