...
func TestUnsupportedFile(t *testing.T) {
	...
	for _, tcase := range readJSONTests("unsupported_cases.txt") {
		t.Run(tcase.Query, func(t *testing.T) {
			...
			require.NoError(t, err)
			_, ok := stmt.(sqlparser.OutputsTable)
			if !ok {
				t.Skip()
				return
			}
...
func keepSameError(query string, reservedVars *sqlparser.ReservedVars, vschema *vschemawrapper.VSchemaWrapper, needs *sqlparser.BindVarNeeds) func(statement sqlparser.OutputsTable) bool {
	...
	return func(statement sqlparser.OutputsTable) bool {
...
func keepPanicking(query string, reservedVars *sqlparser.ReservedVars, vschema *vschemawrapper.VSchemaWrapper, needs *sqlparser.BindVarNeeds) func(statement sqlparser.OutputsTable) bool {
	cmp := func(statement sqlparser.OutputsTable) (res bool) {
		defer func() {
			r := recover()
			if r != nil {
	...
	if !cmp(stmt.(sqlparser.OutputsTable)) {
