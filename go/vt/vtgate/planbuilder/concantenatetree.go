...
func (c *concatenateTree) pushPredicate(ctx *plancontext.PlanningContext, expr sqlparser.Expr) error {
	return vterrors.Errorf(vtrpc.Code_INTERNAL, "add '%s' predicate not supported on concatenate trees", sqlparser.String(expr))
}

func (c *concatenateTree) removePredicate(ctx *plancontext.PlanningContext, expr sqlparser.Expr) error {
