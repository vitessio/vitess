...
func TestDiscoveryGatewayGetTablets(t *testing.T) {
	...
	tsl := dg.hsl.GetHealthyTabletStats(keyspace, shard, topodatapb.TabletType_REPLICA)
	...
	tsl = dg.hsl.GetHealthyTabletStats(keyspace, shard, topodatapb.TabletType_MASTER)
	if len(tsl) != 1 || !topo.TabletEquality(tsl[0].Tablet, ep1) {
		t.Errorf("want %+v, got %+v", ep1, tsl)
	}
}
