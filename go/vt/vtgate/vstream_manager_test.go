...
func TestVStreamStopOnReshardEndsWhenParticipantsConverge(t *testing.T) {
	...
	vstreamCtx, vstreamCancel := context.WithTimeout(ctx, 30*time.Second)
	defer vstreamCancel()

	var journalCount int
	err := vsm.VStream(vstreamCtx, topodatapb.TabletType_PRIMARY, vgtid, nil, &vtgatepb.VStreamFlags{StopOnReshard: true}, func(events []*binlogdatapb.VEvent) error {
