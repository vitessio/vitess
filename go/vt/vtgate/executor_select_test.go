...
func TestSystemVariablesMySQLBelow80(t *testing.T) {
	...
	sbc1.SetResults([]*sqltypes.Result{{
		Fields: []*querypb.Field{
			{Name: "orig", Type: sqltypes.VarChar, Charset: uint32(collations.Local().DefaultConnectionCharset())},
			{Name: "new", Type: sqltypes.VarChar, Charset: uint32(collations.Local().DefaultConnectionCharset())},
		},
		Rows: [][]sqltypes.Value{{
			sqltypes.NewVarChar(""),
...
func TestSetSystemVariables(t *testing.T) {
	...
	lookup.SetResults([]*sqltypes.Result{{
		Fields: []*querypb.Field{
			{Name: "sql_safe_updates", Type: sqltypes.VarChar, Charset: uint32(collations.Local().DefaultConnectionCharset())},
		},
		Rows: [][]sqltypes.Value{{
	...
	lookup.SetResults([]*sqltypes.Result{{
		Fields: []*querypb.Field{
			{Name: "max_tmp_tables", Type: sqltypes.VarChar, Charset: uint32(collations.Local().DefaultConnectionCharset())},
		},
		Rows: [][]sqltypes.Value{{
...
func TestStreamBuffering(t *testing.T) {
	...
	sbclookup.SetResults([]*sqltypes.Result{{
		Fields: []*querypb.Field{
			...
			{Name: "col", Type: sqltypes.VarChar, Charset: uint32(collations.Local().DefaultConnectionCharset())},
		},
		Rows: [][]sqltypes.Value{{
			sqltypes.NewInt32(1),
	...
	executor, _ := createExecutorEnvCallback(t, func(shard, ks string, tabletType topodatapb.TabletType, conn *sandboxconn.SandboxConn) {
		if ks == KsTestSharded {
			conn.SetResults([]*sqltypes.Result{{
				Fields: []*querypb.Field{
					{Name: "id", Type: sqltypes.Int32, Charset: collations.CollationBinaryID, Flags: uint32(querypb.MySqlFlag_NUM_FLAG)},
					{Name: "textcol", Type: sqltypes.VarChar, Charset: uint32(collations.Local().DefaultConnectionCharset())},
...
func TestSelectSystemVariables(t *testing.T) {
	...
	wantResult := &sqltypes.Result{
		Fields: []*querypb.Field{
			...
			{Name: "@@transaction_mode", Type: sqltypes.VarChar, Charset: uint32(collations.Local().DefaultConnectionCharset())},
			{Name: "@@workload", Type: sqltypes.VarChar, Charset: uint32(collations.Local().DefaultConnectionCharset())},
			{Name: "@@read_after_write_gtid", Type: sqltypes.VarChar, Charset: uint32(collations.Local().DefaultConnectionCharset())},
			...
			{Name: "@@session_track_gtids", Type: sqltypes.VarChar, Charset: uint32(collations.Local().DefaultConnectionCharset())},
			{Name: "@@ddl_strategy", Type: sqltypes.VarChar, Charset: uint32(collations.Local().DefaultConnectionCharset())},
			{Name: "@@migration_context", Type: sqltypes.VarChar, Charset: uint32(collations.Local().DefaultConnectionCharset())},
			{Name: "@@socket", Type: sqltypes.VarChar, Charset: uint32(collations.Local().DefaultConnectionCharset())},
...
func TestSelectUserDefinedVariable(t *testing.T) {
	...
	wantResult = &sqltypes.Result{
		Fields: []*querypb.Field{
			{Name: "@foo", Type: sqltypes.VarChar, Charset: uint32(collations.Local().DefaultConnectionCharset())},
		},
		Rows: [][]sqltypes.Value{{
...
func TestSelectDatabase(t *testing.T) {
	...
	wantResult := &sqltypes.Result{
		Fields: []*querypb.Field{
			{Name: "database()", Type: sqltypes.VarChar, Charset: uint32(collations.Local().DefaultConnectionCharset())},
		},
		Rows: [][]sqltypes.Value{{
...
func createExecutor(ctx context.Context, serv *sandboxTopo, cell string, resolver *Resolver) *Executor {
	...
	ex := NewExecutor(ctx, serv, cell, resolver, false, false, testBufferSize, plans, nil, false, querypb.ExecuteOptions_Gen4, 0, collations.Local())
...
func TestStreamOrderByLimitWithMultipleResults(t *testing.T) {
	...
	queryLogger := streamlog.New[*logstats.LogStats]("VTGate", queryLogBufferSize)
	plans := DefaultPlanCache()
	executor := NewExecutor(ctx, serv, cell, resolver, true, false, testBufferSize, plans, nil, false, querypb.ExecuteOptions_Gen4, 0, collations.Local())
