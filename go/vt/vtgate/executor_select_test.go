...
func TestSelectBindvars(t *testing.T) {
	...
	sbc1.Queries = nil
	testQueryLog(t, logChan, "VindexLookup", "SELECT", "select name, user_id from name_user_map where name in ::name for update", 1)
	testQueryLog(t, logChan, "TestExecute", "SELECT", sql, 1)
	...
	wantLookupQueries := []*querypb.BoundQuery{{
		Sql: "select name, user_id from name_user_map where name in ::name for update",
		BindVariables: map[string]*querypb.BindVariable{
			"name": vars,
		},
	}}
