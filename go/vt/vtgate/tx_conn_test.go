...
var oneQuery = []*querypb.BoundQuery{{Sql: "query1"}}
...
func TestTxConnBegin(t *testing.T) {
	...
	_, errors := sc.ExecuteMultiShard(ctx, nil, rss0, oneQuery, safeSession, false, false, nullResultsObserver{}, false)
...
func TestTxConnCommitFailure(t *testing.T) {
	...
	sc.ExecuteMultiShard(ctx, nil, rssm[0], oneQuery, session, false, false, nullResultsObserver{}, false)
	...
	sc.ExecuteMultiShard(ctx, nil, rssm[1], oneQuery, session, false, false, nullResultsObserver{}, false)
	wantSession = vtgatepb.Session{
		InTransaction: true,
		ShardSessions: []*vtgatepb.Session_ShardSession{{
			Target: &querypb.Target{
...
func TestTxConnCommitFailureAfterNonAtomicCommitMaxShards(t *testing.T) {
	...
	for i := 0; i < 18; i++ {
		sc.ExecuteMultiShard(ctx, nil, rssm[i], oneQuery, session, false, false, nullResultsObserver{}, false)
...
func TestTxConnCommitSuccess(t *testing.T) {
	...
	// Sequence the executes to ensure commit order
	session := econtext.NewSafeSession(&vtgatepb.Session{InTransaction: true})
	sc.ExecuteMultiShard(ctx, nil, rss0, oneQuery, session, false, false, nullResultsObserver{}, false)
	wantSession := vtgatepb.Session{
		InTransaction: true,
		ShardSessions: []*vtgatepb.Session_ShardSession{{
			Target: &querypb.Target{
...
func TestTxConnReservedOn2ShardTxOn1ShardAndCommit(t *testing.T) {
	...
	_, errs := sc.ExecuteMultiShard(ctx, nil, rss1, oneQuery, session, false, false, nullResultsObserver{}, false)
	require.Empty(t, errs)
	_, errs = sc.ExecuteMultiShard(ctx, nil, rss0, oneQuery, session, false, false, nullResultsObserver{}, false)
	require.Empty(t, errs)
...
func TestTxConnCommit2PCCreateTransactionFail(t *testing.T) {
	...
	session := econtext.NewSafeSession(&vtgatepb.Session{InTransaction: true})
	sc.ExecuteMultiShard(ctx, nil, rss0, oneQuery, session, false, false, nullResultsObserver{}, false)
	sc.ExecuteMultiShard(ctx, nil, rss1, oneQuery, session, false, false, nullResultsObserver{}, false)
