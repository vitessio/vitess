...
func SimplifyStatement(
	in sqlparser.TableSubquery,
	currentDB string,
	si semantics.SchemaInformation,
	testF func(sqlparser.TableSubquery) bool,
) sqlparser.TableSubquery {
	tables, err := getTables(in, currentDB, si)
	if err != nil {
		panic(err)
	}

	test := func(s sqlparser.TableSubquery) bool {
...
func trySimplifyDistinct(in sqlparser.TableSubquery, test func(statement sqlparser.TableSubquery) bool) sqlparser.TableSubquery {
	simplified := false
...
func trySimplifyExpressions(in sqlparser.TableSubquery, test func(sqlparser.TableSubquery) bool) sqlparser.TableSubquery {
	simplified := false
...
func trySimplifyUnions(in sqlparser.TableSubquery, test func(subquery sqlparser.TableSubquery) bool) (res sqlparser.TableSubquery) {
...
func tryRemoveTable(tables []semantics.TableInfo, in sqlparser.TableSubquery, currentDB string, si semantics.SchemaInformation, test func(sqlparser.TableSubquery) bool) sqlparser.TableSubquery {
...
func getTables(in sqlparser.TableSubquery, currentDB string, si semantics.SchemaInformation) ([]semantics.TableInfo, error) {
	// Since our semantic analysis changes the AST, we clone it first, so we have a pristine AST to play with
	clone := sqlparser.Clone(in)
...
func simplifyStarExpr(in sqlparser.TableSubquery, test func(sqlparser.TableSubquery) bool) sqlparser.TableSubquery {
	simplified := false
	alwaysVisitChildren := func(node, parent sqlparser.SQLNode) bool {
		return true
	}
...
func removeTable(clone sqlparser.TableSubquery, searchedTS semantics.TableSet, db string, si semantics.SchemaInformation) bool {
...
func visitAllExpressionsInAST(clone sqlparser.TableSubquery, visit func(expressionCursor) bool) {
	alwaysVisitChildren := func(node, parent sqlparser.SQLNode) bool {
		return true
	}
	up := func(cursor *sqlparser.Cursor) bool {
