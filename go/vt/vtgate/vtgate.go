...
func registerFlags(fs *pflag.FlagSet) {
	...
	utils.SetFlagIntVar(fs, &streamBufferSize, "stream-buffer-size", streamBufferSize, "the number of bytes sent in each gRPC streaming API response. Results delivered over the MySQL protocol are not re-chunked and follow vttablet's query-server-config-stream-buffer-size.")
