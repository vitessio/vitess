...
func registerFlags(fs *pflag.FlagSet) {
	...
	utils.SetFlagDurationVar(fs, &tempTableHeartbeatTime, "temp-table-heartbeat-time", tempTableHeartbeatTime, "How often to send a keepalive on a connection that created temporary tables, refreshing the tablet's reserved-connection timers without sending anything to mysqld (mysqld's wait_timeout still applies to real session traffic, as in MySQL). Must be below the tablets' --queryserver-config-transaction-timeout so the connection is not reclaimed between heartbeats. Reserved connections on shards with an open transaction are not kept alive — those remain subject to the transaction timeout as usual — while the session's reserved connections on other shards keep their keepalives.")
