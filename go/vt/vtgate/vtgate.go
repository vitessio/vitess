...
func registerFlags(fs *pflag.FlagSet) {
	...
	utils.SetFlagDurationVar(fs, &tempTableHeartbeatTime, "temp-table-heartbeat-time", tempTableHeartbeatTime, "How often to send a keepalive on a connection that created temporary tables. Must be below the tablets' --queryserver-config-transaction-timeout so the connection is not reclaimed between heartbeats. Sessions with an open transaction are not kept alive: transactions remain subject to the transaction timeout as usual.")
