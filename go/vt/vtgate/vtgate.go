...
func Init(ctx context.Context, serv srvtopo.Server, cell string, retryCount int, tabletTypesToWait []topodatapb.TabletType) *VTGate {
	if rpcVTGate != nil {
	...
	gw := NewTabletGateway(ctx, serv, cell, retryCount)
	gw.RegisterStats()
