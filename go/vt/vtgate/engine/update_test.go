...
func TestUpdateEqualChangedVindex(t *testing.T) {
	...
	vc.ExpectLog(t, []string{
		...
		fmt.Sprintf(`Execute insert into lkp2(from1, from2, toc) values(:from1_0, :from2_0, :toc_0) from1_0: %v from2_0: %v toc_0: %v true`, sqltypes.Int64BindVariable(1), sqltypes.Int64BindVariable(2), &querypb.BindVariable{Type: querypb.Type_VARBINARY, Value: []byte("\x16k@\xb4J\xbaK\xd6")}),
		...
		fmt.Sprintf(`Execute insert into lkp1(from, toc) values(:from_0, :toc_0) from_0: %v toc_0: %v true`, sqltypes.Int64BindVariable(3), &querypb.BindVariable{Type: querypb.Type_VARBINARY, Value: []byte("\x16k@\xb4J\xbaK\xd6")}),
...
func TestUpdateEqualMultiColChangedVindex(t *testing.T) {
	...
	vc.ExpectLog(t, []string{
		...
		fmt.Sprintf(`Execute insert into lkp_rg_tbl(from, toc) values(:from_0, :toc_0) from_0: %v toc_0: %v true`, sqltypes.Int64BindVariable(5), &querypb.BindVariable{Type: querypb.Type_VARBINARY, Value: []byte("\x01\x06\xe7\xea\"Βp\x8f")}),
		// Finally, the actual update, which is also sent to -20, same route as the subquery.
		`ExecuteMultiShard sharded.-20: dummy_update {} true true`,
	})
	...
	vc.ExpectLog(t, []string{
		...
		fmt.Sprintf(`Execute delete from lkp_rg_tbl where from = :from and toc = :toc from: %v toc: %v true`, sqltypes.Int64BindVariable(7), &querypb.BindVariable{Type: querypb.Type_VARBINARY, Value: []byte("\x01\x06\xe7\xea\"Βp\x8f")}),
		fmt.Sprintf(`Execute insert into lkp_rg_tbl(from, toc) values(:from_0, :toc_0) from_0: %v toc_0: %v true`, sqltypes.Int64BindVariable(5), &querypb.BindVariable{Type: querypb.Type_VARBINARY, Value: []byte("\x01\x06\xe7\xea\"Βp\x8f")}),
		// Finally, the actual update, which is also sent to -20, same route as the subquery.
		`ExecuteMultiShard sharded.-20: dummy_update {} true true`,
	})
...
func TestUpdateIn(t *testing.T) {
	...
	require.NoError(t, err)
	vc.ExpectLog(t, []string{
		fmt.Sprintf(`ResolveDestinations sharded [%v %v] Destinations:DestinationKeyspaceID(166b40b44aba4bd6),DestinationKeyspaceID(06e7ea22ce92708f)`, sqltypes.Int64BindVariable(1), sqltypes.Int64BindVariable(2)),
		// ResolveDestinations is hard-coded to return -20.
		fmt.Sprintf(`ExecuteMultiShard sharded.-20: dummy_update {__vals: %v} true true`, &querypb.BindVariable{Type: querypb.Type_TUPLE, Values: []*querypb.Value{sqltypes.ValueToProto(sqltypes.NewInt64(1)), sqltypes.ValueToProto(sqltypes.NewInt64(2))}}),
	})
}
...
func TestUpdateInStreamExecute(t *testing.T) {
	...
	vc := newTestVCursor("-20", "20-")
	err := upd.TryStreamExecute(t.Context(), vc, map[string]*querypb.BindVariable{}, false, func(result *sqltypes.Result) error {
		return nil
	})
	require.NoError(t, err)
	vc.ExpectLog(t, []string{
...
func TestUpdateInChangedVindex(t *testing.T) {
	...
	vc.ExpectLog(t, []string{
		...
		fmt.Sprintf(`Execute delete from lkp2 where from1 = :from1 and from2 = :from2 and toc = :toc from1: %v from2: %v toc: %v true`, sqltypes.Int64BindVariable(21), sqltypes.Int64BindVariable(22), &querypb.BindVariable{Type: querypb.Type_VARBINARY, Value: []byte("\x06\xe7\xea\"Βp\x8f")}),
		fmt.Sprintf(`Execute insert into lkp2(from1, from2, toc) values(:from1_0, :from2_0, :toc_0) from1_0: %v from2_0: %v toc_0: %v true`, sqltypes.Int64BindVariable(1), sqltypes.Int64BindVariable(2), &querypb.BindVariable{Type: querypb.Type_VARBINARY, Value: []byte("\x06\xe7\xea\"Βp\x8f")}),
		// 23 has to be replaced by 3.
		fmt.Sprintf(`Execute delete from lkp1 where from = :from and toc = :toc from: %v toc: %v true`, sqltypes.Int64BindVariable(23), &querypb.BindVariable{Type: querypb.Type_VARBINARY, Value: []byte("\x06\xe7\xea\"Βp\x8f")}),
		fmt.Sprintf(`Execute insert into lkp1(from, toc) values(:from_0, :toc_0) from_0: %v toc_0: %v true`, sqltypes.Int64BindVariable(3), &querypb.BindVariable{Type: querypb.Type_VARBINARY, Value: []byte("\x06\xe7\xea\"Βp\x8f")}),
		// Finally, the actual update, which is also sent to -20, same route as the subquery.
		fmt.Sprintf(`ExecuteMultiShard sharded.-20: dummy_update {__vals: %v} true true`, &querypb.BindVariable{Type: querypb.Type_TUPLE, Values: []*querypb.Value{sqltypes.ValueToProto(sqltypes.NewInt64(1)), sqltypes.ValueToProto(sqltypes.NewInt64(2))}}),
	})
...
func TestUpdateInChangedVindexMultiCol(t *testing.T) {
	...
	vc.ExpectLog(t, []string{
		...
		fmt.Sprintf(`Execute insert into lkp_rg_tbl(from, toc) values(:from_0, :toc_0) from_0: %v toc_0: %v true`, sqltypes.Int64BindVariable(5), &querypb.BindVariable{Type: querypb.Type_VARBINARY, Value: []byte("\x01N\xb1\x90ɢ\xfa\x16\x9c")}),
		...
		fmt.Sprintf(`Execute insert into lkp_rg_tbl(from, toc) values(:from_0, :toc_0) from_0: %v toc_0: %v true`, sqltypes.Int64BindVariable(5), &querypb.BindVariable{Type: querypb.Type_VARBINARY, Value: []byte("\x02N\xb1\x90ɢ\xfa\x16\x9c")}),
		// Finally, the actual update, which is also sent to -20, same route as the subquery.
		fmt.Sprintf(`ExecuteMultiShard sharded.-20: dummy_update {__vals0: %v} true true`, &querypb.BindVariable{Type: querypb.Type_TUPLE, Values: []*querypb.Value{sqltypes.ValueToProto(sqltypes.NewInt64(1)), sqltypes.ValueToProto(sqltypes.NewInt64(2))}}),
	})
}
...
func TestUpdateInUnique(t *testing.T) {
	...
	vc.ExpectLog(t, []string{
		...
		fmt.Sprintf(`ExecuteMultiShard sharded.-20: update t set n = 'b' where id in ::__vals {__vals: %v} sharded.20-: update t set n = 'b' where id in ::__vals {__vals: %v} true false`, &querypb.BindVariable{Type: querypb.Type_TUPLE, Values: []*querypb.Value{sqltypes.ValueToProto(sqltypes.NewInt64(1)), sqltypes.ValueToProto(sqltypes.NewInt64(4))}}, &querypb.BindVariable{Type: querypb.Type_TUPLE, Values: []*querypb.Value{sqltypes.ValueToProto(sqltypes.NewInt64(2))}}),
	})
}
