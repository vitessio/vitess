...
func TestDeleteShardedStreaming(t *testing.T) {
	...
	vc := newTestVCursor("-20", "20-")
	err := del.TryStreamExecute(t.Context(), vc, map[string]*querypb.BindVariable{}, false, func(result *sqltypes.Result) error {
		return nil
	})
	require.NoError(t, err)
	vc.ExpectLog(t, []string{
...
func TestDeleteInUnique(t *testing.T) {
	...
	vc := newTestVCursor("-20", "20-")
	vc.shardForKsid = []string{"-20", "20-"}
	_, err := upd.TryExecute(t.Context(), vc, map[string]*querypb.BindVariable{"__vals": tupleBV}, false)
	require.NoError(t, err)
	vc.ExpectLog(t, []string{
