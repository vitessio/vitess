...
func TestSelectEqualUniqueScatter(t *testing.T) {
	vindex, _, _ := vindexes.NewLookupUnique("", map[string]string{
		"table":      "lkp",
		"from":       "from",
		"to":         "toc",
		"write_only": "true",
...
func TestSelectEqual(t *testing.T) {
	vindex, _, _ := vindexes.CreateVindex("lookup", "", map[string]string{
		"table": "lkp",
		"from":  "from",
		"to":    "toc",
	})
...
func TestSelectLike(t *testing.T) {
	subshard, _, _ := vindexes.CreateVindex("cfc", "cfc", map[string]string{"hash": "md5", "offsets": "[1,2]"})
