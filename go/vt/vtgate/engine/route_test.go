...
func TestSelectWithReservedConnection(t *testing.T) {
	sel := NewRoute(
		Unsharded,
		&vindexes.Keyspace{
			Name:    "ks",
	...
	vc := &loggingVCursor{
		shards:          []string{"0"},
		results:         []*sqltypes.Result{defaultSelectResult},
		systemVariables: map[string]string{"sql_mode": "''"},
	}
	result, err := sel.TryExecute(vc, map[string]*querypb.BindVariable{}, false)
	require.NoError(t, err)
	vc.ExpectLog(t, []string{
		`ResolveDestinations ks [] Destinations:DestinationAllShards()`,
		`ExecuteMultiShard ks.0: select /*+ SET_VAR(sql_mode = '') */ 1 from dual {} false false`,
	})
	expectResult(t, "sel.Execute", result, defaultSelectResult)
...
func TestSelectWithCommentsInInput(t *testing.T) {
	sel := NewRoute(
		Unsharded,
		&vindexes.Keyspace{
			Name:    "ks",
	...
	vc.ExpectLog(t, []string{
		`ResolveDestinations ks [] Destinations:DestinationAllShards()`,
		`Needs Reserved Conn`,
		`ExecuteMultiShard ks.0: select /* hello */ 1 {} false false`,
	})
	expectResult(t, "sel.Execute", result, defaultSelectResult)
	...
	vc.ExpectLog(t, []string{
		`ResolveDestinations ks [] Destinations:DestinationAllShards()`,
		`StreamExecuteMulti select /* hello */ 1 ks.0: {} `,
	})
	expectResult(t, "sel.StreamExecute", result, defaultSelectResult)
}
