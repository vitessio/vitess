...
func TestUpdateTargetTable(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.targetString, func(t *testing.T) {
			...
			vc := &loggingVCursor{}
			_, err := updateTarget.TryExecute(t.Context(), vc, map[string]*querypb.BindVariable{}, false)
...
func TestUpdateTargetGetFields(t *testing.T) {
	updateTarget := &UpdateTarget{}
	vc := &noopVCursor{}
	_, err := updateTarget.GetFields(t.Context(), vc, map[string]*querypb.BindVariable{})
