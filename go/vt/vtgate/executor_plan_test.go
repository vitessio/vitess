...
func TestDeferredOptimization(t *testing.T) {
	...
	for _, tcase := range readJSONTests("prepared_statements.json") {
		...
		t.Run(testName, func(t *testing.T) {
			...
			_, _, _, _, _ = executor.fetchOrCreatePlan(ctx, sess, tcase.Query, nil, false, true, ls, false)
			...
			plan, _, _, _, err := executor.fetchOrCreatePlan(ctx, sess, tcase.Query, bv, false, true, ls, true)
