...
func TestPickReparent(t *testing.T) {
	b, hcChan := newSessionBalancer(t)

	// Create a primary and a replica tablet
	localTablet := &discovery.TabletHealth{
		Tablet: &topodatapb.Tablet{
			Alias: &topodatapb.TabletAlias{
				Cell: "local",
	...
	tablet := b.Pick(target, nil, opts)
	require.NotNil(t, tablet)
	require.Equal(t, uint32(101), tablet.Tablet.Alias.Uid)

	// Now perform a reparent and change the tablet types
	localTablet = &discovery.TabletHealth{
		Tablet: &topodatapb.Tablet{
			Alias: &topodatapb.TabletAlias{
				Cell: "local",
	...
	// Should pick the old primary/new replica instead
	tablet = b.Pick(target, nil, opts)
	require.NotNil(t, tablet)
	require.Equal(t, uint32(100), tablet.Tablet.Alias.Uid)
}
