...
func (b *builtinDateFormat) eval(env *ExpressionEnv) (eval, error) {
	...
	default:
		t = evalToDateTime(date, datetime.DefaultPrecision, env.now, env.allowZeroDate)
		if t == nil || t.isZero() {
			return nil, nil
		}
	}
...
func (call *builtinConvertTz) eval(env *ExpressionEnv) (eval, error) {
	...
	dt := evalToDateTime(n, -1, env.now, env.allowZeroDate)
	if dt == nil || dt.isZero() {
		return nil, nil
	}
	...
	return newEvalDateTime(out, int(dt.prec), env.allowZeroDate), nil
}
...
func (b *builtinDate) eval(env *ExpressionEnv) (eval, error) {
	...
	d := evalToDate(date, env.now, env.allowZeroDate)
	if d == nil {
		return nil, nil
	}
	return d, nil
...
func (call *builtinDate) compile(c *compiler) (ctype, error) {
	...
	case sqltypes.Date:
	default:
		c.asm.Convert_xD(1, c.allowZeroDate)
	}
...
func (b *builtinFromUnixtime) eval(env *ExpressionEnv) (eval, error) {
	...
	dt := newEvalDateTime(datetime.NewDateTimeFromStd(t), prec, env.allowZeroDate)
...
func (b *builtinMakedate) eval(env *ExpressionEnv) (eval, error) {
	...
	return newEvalDate(datetime.NewDateTimeFromStd(t).Date, env.allowZeroDate), nil
}
...
func dateTimeUnixTimestamp(env *ExpressionEnv, date eval) evalNumeric {
	...
	default:
		dt = evalToDateTime(date, -1, env.now, env.allowZeroDate)
		if dt == nil || dt.isZero() {
			var prec int32
			switch d := date.(type) {
...
func (call *builtinUnixTimestamp) compile(c *compiler) (ctype, error) {
	...
	case sqltypes.VarChar, sqltypes.VarBinary:
		...
		if lit, ok := call.Arguments[0].(*Literal); ok {
			if dt := evalToDateTime(lit.inner, -1, time.Now(), c.allowZeroDate); dt != nil {
				if dt.prec == 0 {
					return ctype{Type: sqltypes.Int64, Col: collationNumeric, Flag: arg.Flag}, nil
				}
...
func (call *builtinDateMath) eval(env *ExpressionEnv) (eval, error) {
	...
	if tmp := evalToTemporal(date, env.allowZeroDate); tmp != nil {
