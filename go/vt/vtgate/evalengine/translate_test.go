...
// -race, this catches evaluations sharing the literal's document. The literal
// is settled at fold time by Resolve, so evaluations and compilations only read
// it; this pins that each evaluation then mutates its own clone rather than the
// shared document.
