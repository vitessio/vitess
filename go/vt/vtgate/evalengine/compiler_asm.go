...
func (asm *assembler) PushColumn_bin(offset int) {
	...
	asm.emit(func(env *ExpressionEnv) int {
		env.vm.stack[env.vm.sp] = newEvalBinary(env.Row[offset].Raw())
		env.vm.sp++
		return 1
...
func (asm *assembler) PushColumn_d(offset int) {
	...
	asm.emit(func(env *ExpressionEnv) int {
		...
		dec, env.vm.err = decimal.NewFromMySQL(env.Row[offset].Raw())
...
func (asm *assembler) PushColumn_f(offset int) {
	...
	asm.emit(func(env *ExpressionEnv) int {
		var fval float64
		fval, env.vm.err = env.Row[offset].ToFloat64()
...
func (asm *assembler) PushColumn_hexnum(offset int) {
	...
	asm.emit(func(env *ExpressionEnv) int {
		var raw []byte
		raw, env.vm.err = parseHexNumber(env.Row[offset].Raw())
...
func (asm *assembler) PushColumn_hexval(offset int) {
	...
	asm.emit(func(env *ExpressionEnv) int {
		hex := env.Row[offset].Raw()
		var raw []byte
		raw, env.vm.err = parseHexLiteral(hex[2 : len(hex)-1])
		env.vm.stack[env.vm.sp] = newEvalBytesHex(raw)
		env.vm.sp++
		return 1
...
func (asm *assembler) PushColumn_i(offset int) {
	...
	asm.emit(func(env *ExpressionEnv) int {
		var ival int64
		ival, env.vm.err = env.Row[offset].ToInt64()
...
func (asm *assembler) PushColumn_json(offset int) {
	...
	asm.emit(func(env *ExpressionEnv) int {
		...
		env.vm.stack[env.vm.sp], env.vm.err = parser.ParseBytes(env.Row[offset].Raw())
		env.vm.sp++
		return 1
	}, "PUSH JSON(:%d)", offset)
}
...
func (asm *assembler) PushColumn_text(offset int, col collations.TypedCollation) {
	...
	asm.emit(func(env *ExpressionEnv) int {
		env.vm.stack[env.vm.sp] = newEvalText(env.Row[offset].Raw(), col)
		env.vm.sp++
		return 1
...
func (asm *assembler) PushColumn_u(offset int) {
	...
	asm.emit(func(env *ExpressionEnv) int {
		var uval uint64
		uval, env.vm.err = env.Row[offset].ToUint64()
