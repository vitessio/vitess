...
func compare(collationEnv *collations.Environment, v1, v2 sqltypes.Value, collationID collations.ID) (int, error) {
	v1t := v1.Type()
	...
	out, err := evalCompare(collationEnv, v1eval, v2eval)
	if err != nil {
		return 0, err
	}
	if out == 0 {
...
func NullsafeCompare(collationEnv *collations.Environment, v1, v2 sqltypes.Value, collationID collations.ID) (int, error) {
	...
	return compare(collationEnv, v1, v2, collationID)
}
...
func (obp *OrderByParams) Compare(r1, r2 []sqltypes.Value) int {
	...
	if cmp == 0 {
		var err error
		cmp, err = NullsafeCompare(obp.CollationEnv, v1, v2, obp.Type.Collation())
		if err != nil {
			...
			cmp, err = NullsafeCompare(obp.CollationEnv, r1[obp.Col], r2[obp.Col], obp.Type.Collation())
			if err != nil {
				panic(err)
			}
		}
