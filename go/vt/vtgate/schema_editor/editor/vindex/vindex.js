...
function VindexController($scope, $routeParams, vindexInfo, curSchema) {
  init();
  ...
  function init() {
    ...
    if (!$routeParams.vindexName || !$scope.keyspace.Vindexes[$routeParams.vindexName]) {
      return;
    }
    $scope.vindexName = $routeParams.vindexName;
    $scope.vindex = $scope.keyspace.Vindexes[$routeParams.vindexName];
  }
