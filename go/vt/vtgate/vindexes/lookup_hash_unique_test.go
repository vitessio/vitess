...
func TestLookupHashUniqueMap(t *testing.T) {
	...
	got, err := lhu.Map2(vc, []sqltypes.Value{sqltypes.NewInt64(1), sqltypes.NewInt64(2)})
	if err != nil {
		t.Error(err)
	}
	...
	vc.numRows = 2
	_, err = lhu.Map2(vc, []sqltypes.Value{sqltypes.NewInt64(1), sqltypes.NewInt64(2)})
	...
	got, err = lhu.Map2(vc, []sqltypes.Value{sqltypes.NewInt64(1)})
	if err != nil {
		t.Error(err)
	}
	want = []key.Destination{
	...
	// Test query fail.
	vc.mustFail = true
	_, err = lhu.Map2(vc, []sqltypes.Value{sqltypes.NewInt64(1)})
