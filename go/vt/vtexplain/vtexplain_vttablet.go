...
func inferColTypeFromExpr(node sqlparser.Expr, tableColumnMap map[sqlparser.IdentifierCS]map[string]querypb.Type, colNames []string, colTypes []querypb.Type) ([]string, []querypb.Type) {
	...
	case *sqlparser.ColName:
		if node.Qualifier.Name.IsEmpty() {
			...
			for _, colTypeMap := range tableColumnMap {
				if colTypeMap[col] != querypb.Type_NULL_TYPE {
					if colType != querypb.Type_NULL_TYPE {
						log.ErrorS("vtexplain: ambiguous column " + col)
						return colNames, colTypes
					}
			...
			if colType == querypb.Type_NULL_TYPE {
				log.ErrorS(fmt.Sprintf("vtexplain: invalid column %s.%s, tableColumnMap +%v", node.Qualifier.Name, col, tableColumnMap))
			}
	...
	case *sqlparser.Literal:
		...
		default:
			log.ErrorS("vtexplain: unsupported sql value " + sqlparser.String(node))
		}
	...
	default:
		log.ErrorS(fmt.Sprintf("vtexplain: unsupported select expression type +%v node %s", reflect.TypeOf(node), sqlparser.String(node)))
	}
