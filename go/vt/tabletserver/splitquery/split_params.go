...
//
// 'numRowsPerQueryPart' the desired number of rows per query part returned. The actual number
...
func NewSplitParamsGivenNumRowsPerQueryPart(
	...
	if numRowsPerQueryPart <= 0 {
		return nil, fmt.Errorf("splitquery: numRowsPerQueryPart must be nonnegative. Got: %v",
			numRowsPerQueryPart)
	}
...
// If splitColumns is empty, the split columns used are the primary key columns (in order).
//
// 'splitCount' the desired splitCount to use. The actual number may be different depending on
...
func NewSplitParamsGivenSplitCount(
	...
	if splitCount <= 0 {
		return nil, fmt.Errorf("splitquery: splitCount must be nonnegative. Got: %v",
			splitCount)
	}
	result, err := newSplitParams(sql, bindVariables, splitColumns, schema)
	if err != nil {
...
func (splitParams *SplitParams) areSplitColumnsPrimaryKey() bool {
	return reflect.DeepEqual(splitParams.splitColumns,
		getPrimaryKeyColumns(splitParams.splitTableSchema))
}
