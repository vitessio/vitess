...
// in the above query is actually re-written to use only scalar comparisons since MySQL does not
...
// element of the tuple. In this tuple, the bind variable containing the value of a split-column
// named 'x' is called <prevBoundVariable><x>, where prevBoundVariable is the string constant
// defined below.
...
func (algorithm *FullScanAlgorithm) generateBoundaries() ([]tuple, error) {
	var prevTuple = algorithm.executeQuery(algorithm.firstQuery)
	...
	for iteration := 0; prev != nil; iteration++ {
		...
		prev = algorithm.executeQuery(algorithm.noninitialQuery)
	}
	return result
}
