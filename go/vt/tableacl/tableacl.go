...
func (tacl *tableACL) init(configFile string, aclCB func()) error {
	...
	if err != nil {
		log.ErrorS(fmt.Sprintf("unable to read tableACL config file: %v  Error: %v", configFile, err))
		return err
	}
	if len(data) == 0 {
	...
	if err := config.UnmarshalVT(data); err != nil {
		...
		if jsonErr := json2.UnmarshalPB(data, config); jsonErr != nil {
			log.ErrorS(fmt.Sprintf("unable to parse tableACL config file as a protobuf or json file.  protobuf err: %v  json err: %v", err, jsonErr))
