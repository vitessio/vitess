...
var customComparators = map[string]bool{
	"*ColName": true,
}
...
func (e *equalsGen) genFile() (string, *jen.File) {
	e.file.Type().Id(Comparator).StructFunc(func(g *jen.Group) {
		for field, tt := range e.comparators {
			tArg := types.TypeString(tt, noQualifier)
			g.Add(jen.Id(field).Func().Call(jen.List(jen.Id("a"), jen.Id("b")).Id(tArg)).Bool())
		}
	})
