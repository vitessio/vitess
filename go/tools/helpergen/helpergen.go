...
type output interface {
	finalizeFile(file []codeImpl, out *jen.File)
	implForStruct(name *types.TypeName, st *types.Struct, sizes types.Sizes, debugTypes bool) (jen.Code, codeFlag)
	fileName() string
}
...
func (gen *helpergen) generateType(pkg *types.Package, file *codeFile, named *types.Named) {
	...
	case *types.Struct:
		if impl, flag := gen.out.implForStruct(named.Obj(), tt, gen.sizes, gen.DebugTypes); impl != nil {
...
func (gen *helpergen) finalize() map[string]*jen.File {
	...
	for pkg, file := range gen.codegen {
		...
		gen.out.finalizeFile(file.impls, out)
