...
type StreamLogger struct {
	...
	// it atomically in sync (atomically) with the size of
	// subscribed.
	size uint32
...
func New(url string, size int) *StreamLogger {
	logger := &StreamLogger{
		dataQueue:  make(chan string, size),
...
func (logger *StreamLogger) stream() {
	...
	for message := range logger.dataQueue {
		...
		message += "\n"
		for subscription, done := range logger.subscribed {
			if _, err := io.WriteString(subscription, message); err != nil {
				done <- true
...
// Send does not block.
func (logger *StreamLogger) Send(message string) {
