...
func WaitForThrottlerStatusEnabled(t *testing.T, vtctldProcess *cluster.VtctldClientProcess, tablet *cluster.Vttablet, enabled bool, config *Config, timeout time.Duration) {
	...
	for {
		...
		if class == "unhappy" && strings.Contains(value, "not serving") {
			log.InfoS(fmt.Sprintf("tablet %s is Not Serving, so ignoring throttler status as the throttler will not be Opened", tablet.Alias))
			return
		}
		...
		good := func() bool {
			if err != nil {
				log.ErrorS(fmt.Sprintf("GetThrottlerStatus failed: %v", err))
				return false
			}
...
func getHTTPBody(url string) string {
	...
	if err != nil {
		log.InfoS(fmt.Sprintf("http Get returns %+v", err))
		return ""
	}
	defer resp.Body.Close()
	if resp.StatusCode != 200 {
		log.InfoS(fmt.Sprintf("http Get returns status %d", resp.StatusCode))
		return ""
	}
