...
func TestRequiredPositionRejectsLostRelayLogs(t *testing.T) {
	...
	// Use an ID beyond the test writes to avoid a collision. clusterInfo.lastUsedValue
	// starts at 100 and grows by one per write, far below 1000000 in this suite.
	...
	require.EventuallyWithT(t, func(c *assert.CollectT) {
		rows, readErr := utils.ReadVTOrcTable(vtorc, "database_instance")
	...
	// Stop MySQL before discarding relay logs so the IO threads cannot refill them.
	...
	for _, replica := range replicas {
		...
		for i, field := range res.Fields {
			if field.Name == "Retrieved_Gtid_Set" {
				require.Empty(t, res.Rows[0][i].ToString(), "RESET REPLICA must discard received transactions")
			}
		}

		// Restore replication intent so ERS can use both tablets for semi-sync.
		require.NoError(t, utils.RunSQLs(t, []string{"START REPLICA"}, replica, ""))
	}

	utils.EnableGlobalRecoveries(t, vtorc)

	// Read recovery steps to verify both the requirement and the ERS failure.
	requiredLine := "required position: MySQL56/" + position
	failureLine := "required position MySQL56/" + position
	require.EventuallyWithT(t, func(c *assert.CollectT) {
		steps, readErr := utils.ReadVTOrcTable(vtorc, "topology_recovery_steps")
		if !assert.NoError(c, readErr) {
			return
		}
	...
	}, 60*time.Second, time.Second, "VTOrc must audit the requirement and the receipt failure")

	// Observe multiple recovery polls to exclude a later stale promotion.
