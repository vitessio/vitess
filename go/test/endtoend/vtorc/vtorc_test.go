...
func TestPrimaryElection(t *testing.T) {
	...
	setupVttabletsAndVtorc(t, 1, 1, 0, 0, nil, "test_config.json")
...
func TestSingleKeyspace(t *testing.T) {
	defer cluster.PanicHandler(t)
	setupVttabletsAndVtorc(t, 1, 1, 0, 0, []string{"-clusters_to_watch", "ks"}, "test_config.json")
	keyspace := &clusterInstance.Keyspaces[0]
	shard0 := &keyspace.Shards[0]
...
func TestKeyspaceShard(t *testing.T) {
	defer cluster.PanicHandler(t)
	setupVttabletsAndVtorc(t, 1, 1, 0, 0, []string{"-clusters_to_watch", "ks/0"}, "test_config.json")
	keyspace := &clusterInstance.Keyspaces[0]
	shard0 := &keyspace.Shards[0]
...
func TestReplicationFromOtherReplica(t *testing.T) {
	defer cluster.PanicHandler(t)
	setupVttabletsAndVtorc(t, 3, 0, 0, 0, nil, "test_config.json")
	keyspace := &clusterInstance.Keyspaces[0]
	shard0 := &keyspace.Shards[0]
