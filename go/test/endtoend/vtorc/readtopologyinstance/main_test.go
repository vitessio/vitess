...
func TestReadTopologyInstanceBufferable(t *testing.T) {
	...
	assert.Equal(t, topodatapb.TabletType_PRIMARY, primaryInstance.DisplayTabletType)
	...
	assert.Equal(t, topodatapb.TabletType_REPLICA, replicaInstance.DisplayTabletType)
