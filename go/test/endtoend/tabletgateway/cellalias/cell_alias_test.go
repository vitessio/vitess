...
func TestMain(m *testing.M) {
	...
	exitcode, err := func() (int, error) {
		...
		shard1Primary = localCluster.NewVttabletInstance("primary", 0, "")
		...
		shard2Primary = localCluster.NewVttabletInstance("primary", 0, "")
		...
		for _, tablet := range []*cluster.Vttablet{shard1Primary, shard1Replica, shard1Rdonly, shard2Primary, shard2Replica, shard2Rdonly} {
			...
			tablet.VttabletProcess = cluster.VttabletProcessInstance(tablet.HTTPPort, tablet.GrpcPort, tablet.TabletUID, tablet.Cell, "", keyspaceName, localCluster.VtctldProcess.Port, tablet.Type, localCluster.TopoPort, hostname, localCluster.TmpDirectory, commonTabletArg, true, localCluster.DefaultCharset)
