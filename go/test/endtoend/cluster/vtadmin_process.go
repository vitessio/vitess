...
func (vp *VtAdminProcess) Setup() (err error) {
	...
	if err != nil {
		log.ErrorS(fmt.Sprintf("cannot create log directory for vtadmin: %v", err))
		return err
	}
...
`, vp.VtctldWebPort, vp.VtctldGrpcPort, vp.VtGateWebPort, vp.VtGateGrpcPort), "json")
	...
	if err != nil {
		log.ErrorS(fmt.Sprintf("cannot create error log file for vtadmin: %v", err))
		return err
	}
	...
	log.InfoS(fmt.Sprintf("Running vtadmin with command: %v", strings.Join(vp.proc.Args, " ")))
	...
	go func() {
		if vp.proc != nil {
			...
			if exitErr != nil {
				log.ErrorS(fmt.Sprintf("vtadmin process exited with error: %v", exitErr))
				data, _ := os.ReadFile(logFile)
				log.ErrorS("vtadmin stderr - " + string(data))
			}
			vp.exit <- exitErr
			close(vp.exit)
		}
...
func (vp *VtAdminProcess) CreateAndWriteFile(prefix string, content string, extension string) (string, error) {
	...
	if err != nil {
		log.ErrorS(fmt.Sprintf("cannot create file for vtadmin: %v", err))
		return "", err
	}
