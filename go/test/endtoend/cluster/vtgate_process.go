...
func (vtgate *VtgateProcess) Setup() (err error) {
	...
	if err != nil {
		log.ErrorS(fmt.Sprintf("cannot create config file for vtgate: %v", err))
		return err
	}
	...
	if err != nil {
		log.ErrorS(fmt.Sprintf("cannot create error log file for vtgate: %v", err))
		return err
	}
	...
	log.InfoS(fmt.Sprintf("Running vtgate with command: %v", strings.Join(vtgate.proc.Args, " ")))
	...
	for time.Now().Before(timeout) {
		...
		case err := <-vtgate.exit:
			...
			if ferr == nil {
				log.ErrorS("vtgate error log contents:\n" + string(errBytes))
			} else {
				log.ErrorS(fmt.Sprintf("Failed to read the vtgate error log file %q: %v", vtgate.ErrorLog, ferr))
			}
...
func (vtgate *VtgateProcess) WaitForStatusOfTabletInShard(name string, endPointsCount int, timeout time.Duration) error {
	log.InfoS(fmt.Sprintf("Waiting for healthy status of %d %s tablets in cell %s", endPointsCount, name, vtgate.Cell))
