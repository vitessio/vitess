...
func (mysqlctld *MysqlctldProcess) Start() error {
	...
	if err != nil {
		log.ErrorS(fmt.Sprintf("Failed to create directory for mysqlctld logs: %v", err))
		return err
	}
	errFile, err := os.Create(path.Join(mysqlctld.LogDirectory, "mysqlctld-stderr.txt"))
	if err != nil {
		log.ErrorS(fmt.Sprintf("Failed to create directory for mysqlctld stderr: %v", err))
	}
	...
	log.InfoS(strings.Join(tempProcess.Args, " "))
	...
	go func(mysqlctld *MysqlctldProcess) {
		...
		if !mysqlctld.exitSignalReceived {
			...
			if ferr == nil {
				log.ErrorS("mysqlctld error log contents:\n" + string(errBytes))
			} else {
				log.ErrorS(fmt.Sprintf("Failed to read the mysqlctld error log file %q: %v", mysqlctld.ErrorLog, ferr))
			}
