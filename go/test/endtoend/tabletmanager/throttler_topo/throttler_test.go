...
func TestInitialThrottler(t *testing.T) {
	...
	t.Run("enabling throttler, again", func(t *testing.T) {
		_, err := throttler.UpdateThrottlerTopoConfig(clusterInstance, true, false, unreasonablyLowThreshold.Seconds(), useDefaultQuery, true)
		assert.NoError(t, err)
	...
	t.Run("setting low threshold", func(t *testing.T) {
		_, err := throttler.UpdateThrottlerTopoConfig(clusterInstance, true, false, throttler.DefaultThreshold.Seconds(), useDefaultQuery, true)
		assert.NoError(t, err)
	...
	t.Run("validating OK response from throttler with low threshold, heartbeats running", func(t *testing.T) {
		...
		require.NoError(t, err)
		defer resp.Body.Close()
		assert.Equalf(t, http.StatusOK, resp.StatusCode, "Unexpected response from throttler: %s", getResponse)
	})
...
func TestLag(t *testing.T) {
	...
	t.Run("accumulating lag, expecting throttler push back", func(t *testing.T) {
		...
		require.NoError(t, err)
		defer resp.Body.Close()
		assert.Equalf(t, http.StatusTooManyRequests, resp.StatusCode, "Unexpected response from throttler: %s", getResponse)
	})
