...
func TestMain(m *testing.M) {
	...
	exitCode := func() int {
		...
		if err != nil {
			log.InfoS(fmt.Sprintf("Skipping clone tests: unable to get MySQL version: %v", err))
			return 0
		}
		log.InfoS("Detected MySQL version: " + versionStr)

		flavor, version, err := mysqlctl.ParseVersionString(versionStr)
		if err != nil {
			log.InfoS(fmt.Sprintf("Skipping clone tests: unable to parse MySQL version: %v", err))
			return 0
		}
		log.InfoS(fmt.Sprintf("Parsed flavor: %v, version: %d.%d.%d", flavor, version.Major, version.Minor, version.Patch))

		// Clone is only supported on MySQL 8.0.17+
		if flavor != mysqlctl.FlavorMySQL && flavor != mysqlctl.FlavorPercona {
			log.InfoS(fmt.Sprintf("Skipping clone tests: MySQL CLONE requires MySQL or Percona, got flavor: %v", flavor))
			return 0
		}
		if version.Major < 8 || (version.Major == 8 && version.Minor == 0 && version.Patch < 17) {
			log.InfoS(fmt.Sprintf("Skipping clone tests: MySQL CLONE requires version 8.0.17+, got: %d.%d.%d", version.Major, version.Minor, version.Patch))
			return 0
		}
		...
		if capableOf == nil {
			log.InfoS("Skipping clone tests: unable to get capability checker for version " + cleanVersion)
			return 0
		}
		hasClone, err := capableOf(capabilities.MySQLClonePluginFlavorCapability)
		if err != nil || !hasClone {
			log.InfoS(fmt.Sprintf("Skipping clone tests: MySQL version %s does not support CLONE plugin", cleanVersion))
			return 0
		}
		log.InfoS(fmt.Sprintf("MySQL version %s supports CLONE plugin, proceeding with tests", cleanVersion))

		// Setup EXTRA_MY_CNF for clone plugin
		if err := setupExtraMyCnf(); err != nil {
			log.ErrorS(fmt.Sprintf("Failed to setup extra MySQL config: %v", err))
			return 1
		}
		...
		if err := clusterInstance.StartTopo(); err != nil {
			log.ErrorS(fmt.Sprintf("Failed to start topo: %v", err))
			return 1
		}

		// Initialize cluster with 2 tablets for clone testing
		if err := initClusterForClone(); err != nil {
			log.ErrorS(fmt.Sprintf("Failed to init cluster: %v", err))
			return 1
		}
			...
			for _, tablet := range []*cluster.Vttablet{donorTablet, recipientTablet} {
				if tablet != nil {
					if err := tablet.MysqlctlProcess.Stop(); err != nil {
						log.ErrorS(fmt.Sprintf("Failed to stop MySQL for tablet %d: %v", tablet.TabletUID, err))
					}
				}
			}
		}()
...
func setupExtraMyCnf() error {
	...
	log.InfoS("Set EXTRA_MY_CNF to include clone plugin: " + os.Getenv("EXTRA_MY_CNF"))
	return nil
}
...
func initClusterForClone() error {
	...
	log.InfoS("Created combined init file at: " + initDBWithClone)
	...
	log.InfoS("MySQL processes started successfully")
