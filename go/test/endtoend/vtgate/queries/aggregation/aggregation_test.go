...
func TestJsonAggregationCrossShard(t *testing.T) {
	...
	// LIKE and REGEXP on this merged value stay supported: a scalar,
	// non-lossy array aggregate is the one class whose text serialization
	// the evalengine reproduces byte-identically to MySQL.
	mcmp.Exec("select json_arrayagg(id5) like '[%' from t3")
	mcmp.Exec("select json_arrayagg(id5) regexp '.*' from t3")

	// Every function that coerces the merged document to text is rejected
	// over a document that can contain an object: the evalengine serializes
	// object keys in lexicographic rather than MySQL's length-then-bytes
	// order, so LOWER would return different text than MySQL.
	utils.AssertContainsError(t, mcmp.VtConn,
		"select lower(json_objectagg(id5, id7)) from t3",
		"LOWER consumes a vtgate-merged JSON aggregate")
