...
// if true then execution will always stop on "must fix" error: a mismatched results or EOF
...
func TestKnownFailures(t *testing.T) {
	...
	helperTest(t, "select /*vt+ PLANNER=Gen4 */ count(*) as caggr0 from dept as tbl0, (select count(*) as caggr0 from emp as tbl0, emp as tbl1 limit 18) as tbl1")
	...
	for time.Now().Before(endBy) && (!t.Failed() || TestFailingQueries) {
		...
		if vtErr != nil {
			...
			if StopOnMustFixError {
				...
				// mismatched results
				if strings.Contains(vtErr.Error(), "results mismatched") && false {
					break
				}
			}
