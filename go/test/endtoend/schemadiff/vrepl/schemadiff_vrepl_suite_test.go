...
func testSingle(t *testing.T, testName string) {
	...
	// Run the DDL!
	_, err := onlineddl.VtgateExecQuery(t.Context(), &vtParams, alterStatement)
	require.NoError(t, err)
...
func validateDiff(t *testing.T, fromCreateTable string, toCreateTable string, allowSchemadiffNormalization bool, hints *schemadiff.DiffHints) {
	...
	_ = mysqlExec(t, "drop table if exists "+tableName, "")
	_, err = onlineddl.VtgateExecQuery(t.Context(), &vtParams, fromCreateTable)
	require.NoError(t, err)
	if diffedAlterQuery != "" {
		_, err := onlineddl.VtgateExecQuery(t.Context(), &vtParams, diffedAlterQuery)
		require.NoError(t, err)
	}
