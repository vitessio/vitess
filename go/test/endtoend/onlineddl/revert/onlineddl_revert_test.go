...
func testRevert(t *testing.T) {
	...
	testPostponedRevert := func(t *testing.T, expectStatuses ...tabletmanagerdatapb.OnlineDDL_Status) {
		...
		status := onlineddl.WaitForMigrationStatus(t, &vtParams, shards, uuid, 60*time.Second, tabletmanagerdatapb.OnlineDDL_COMPLETE, tabletmanagerdatapb.OnlineDDL_FAILED)
		fmt.Printf("# Migration status (for debug purposes): <%s>\n", status)
		onlineddl.CheckMigrationStatus(t, &vtParams, shards, uuid, tabletmanagerdatapb.OnlineDDL_COMPLETE)
	}
	t.Run("postponed revert", func(t *testing.T) {
		testPostponedRevert(t, tabletmanagerdatapb.OnlineDDL_RUNNING)
		checkMigratedTable(t, tableName, alterHints[1])
		testSelectTableMetrics(t)
	})
	...
	t.Run("postponed revert view", func(t *testing.T) {
		...
		t.Run("ALTER VIEW, postpone completion", func(t *testing.T) {
			...
			onlineddl.CheckMigrationStatus(t, &vtParams, shards, uuid, tabletmanagerdatapb.OnlineDDL_QUEUED, tabletmanagerdatapb.OnlineDDL_READY)
			// Issue a complete and wait for successful completion
			onlineddl.CheckCompleteMigration(t, &vtParams, shards, uuid, true)
			// This part may take a while, because we depend on vreplication polling
			status := onlineddl.WaitForMigrationStatus(t, &vtParams, shards, uuid, 60*time.Second, tabletmanagerdatapb.OnlineDDL_COMPLETE, tabletmanagerdatapb.OnlineDDL_FAILED)
		...
		testPostponedRevert(t, tabletmanagerdatapb.OnlineDDL_QUEUED, tabletmanagerdatapb.OnlineDDL_READY)
		checkTable(t, viewName, true)
	})
...
func testRevertMigration(t *testing.T, revertUUID string, ddlStrategy string) (uuid string) {
	...
	_ = onlineddl.WaitForMigrationStatus(t, &vtParams, shards, uuid, 20*time.Second, tabletmanagerdatapb.OnlineDDL_COMPLETE, tabletmanagerdatapb.OnlineDDL_FAILED)
	return uuid
}
