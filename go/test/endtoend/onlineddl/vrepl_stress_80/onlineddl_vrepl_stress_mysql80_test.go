...
func TestSchemaChange(t *testing.T) {
	...
	// Validate MySQL version is 8.0
	{
		row := onlineddl.VtgateExecQuery(t, &vtParams, "select version() as v", "").Named().Row()
		version := row["v"].ToString()
		require.Contains(t, version, "8.0.", "This test requires MySQL 8.0 specifically. Found version: %s", version)
	}
