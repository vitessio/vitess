...
var dryRunResultsSwitchWritesCustomerShard = []string{
	...
	"Stop writes on keyspace product, tables [Lead,Lead-1,customer,db_order_test,json_tbl]:",
	...
	"Enable writes on keyspace customer tables [Lead,Lead-1,customer,db_order_test,json_tbl]",
	"Switch routing from keyspace product to keyspace customer",
	"Routing rules for tables [Lead,Lead-1,customer,db_order_test,json_tbl] will be updated",
...
var dryRunResultsReadCustomerShard = []string{
	"Lock keyspace product",
	"Switch reads for tables [Lead,Lead-1,customer,db_order_test,json_tbl] to keyspace customer for tablet types [RDONLY,REPLICA]",
	"Routing rules for tables [Lead,Lead-1,customer,db_order_test,json_tbl] will be updated",
	"Unlock keyspace product",
}
