...
func TestWorkflowDuplicateKeyBackoff(t *testing.T) {
	...
	queries := []string{
		"update admins set email = null where team_id = 2",            // -80
		"update admins set email = 'b@example.com' where team_id = 1", // 80-
		"update admins set email = 'a@example.com' where team_id = 2", // -80
	}

	vc.VtctlClient.ExecuteCommandWithOutput("VReplicationExec", "zone1-100", "update _vt.vreplication set state = 'Stopped' where id = 1")
	...
	waitForResult(t, productTab, "product", "select * from admins order by team_id",
		"[[INT32(1) VARCHAR(\"b@example.com\")] [INT32(2) VARCHAR(\"a@example.com\")]]", 1*time.Minute)
