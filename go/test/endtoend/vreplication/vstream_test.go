...
func TestVStreamWithTablesToSkipCopyFlag(t *testing.T) {
	...
	vc.AddKeyspace(t, []*Cell{defaultCell}, sourceKs, "0", initialProductVSchema, initialProductSchema, defaultReplicas, defaultRdonly, 100, nil)
	...
	for range 10 {
		id++
		execVtgateQuery(t, vtgateConn, sourceKs, fmt.Sprintf("insert into customer (cid, name) values (%d, 'customer%d')", id+100, id))
		execVtgateQuery(t, vtgateConn, sourceKs, fmt.Sprintf("insert into product (pid, description) values (%d, 'description%d')", id+100, id))
		execVtgateQuery(t, vtgateConn, sourceKs, fmt.Sprintf("insert into merchant (mname, category) values ('mname%d', 'category%d')", id+100, id))
	}
	...
	qr1 := execVtgateQuery(t, vtgateConn, sourceKs, "select count(*) from customer")
	qr2 := execVtgateQuery(t, vtgateConn, sourceKs, "select count(*) from product")
	qr3 := execVtgateQuery(t, vtgateConn, sourceKs, "select count(*) from merchant")
...
func testVStreamWithFailover(t *testing.T, failover bool) {
	...
	for {
		...
		case 1:
			if failover {
				insertMu.Lock()
				output, err := vc.VtctldClient.ExecuteCommandWithOutput("PlannedReparentShard", fmt.Sprintf("%s/0", sourceKs), "--new-primary=zone1-101")
				insertMu.Unlock()
		...
		case 2:
			if failover {
				insertMu.Lock()
				output, err := vc.VtctldClient.ExecuteCommandWithOutput("PlannedReparentShard", fmt.Sprintf("%s/0", sourceKs), "--new-primary=zone1-100")
				insertMu.Unlock()
	...
	qr := execVtgateQuery(t, vtgateConn, sourceKs, "select count(*) from customer")
	require.NotNil(t, qr)
...
func testVStreamCopyMultiKeyspaceReshard(t *testing.T, baseTabletID int) numEvents {
	...
	for {
		...
		switch tickCount {
		case 1:
			reshard(t, "sharded", targetKs, "vstreamCopyMultiKeyspaceReshard", "-80,80-", "-40,40-", baseTabletID+400, nil, nil, nil, nil, defaultCellName, 1)
			reshardDone = true
		case 60:
			done = true
		}
...
func TestVStreamWithKeyspacesToWatch(t *testing.T) {
	extraVTGateArgs = append(extraVTGateArgs, []string{
		utils.GetFlagVariantForTests("--keyspaces-to-watch"), sourceKs,
	}...)
		...
		case nil:
			for _, ev := range evs {
				...
				case binlogdatapb.VEventType_ROW:
					...
					tableName := arr[1]
					require.Equal(t, sourceKs, rowEvent.Keyspace)
				...
				case binlogdatapb.VEventType_FIELD:
					...
					require.Equal(t, len(arr), 2)
					tableName := arr[1]
					require.Equal(t, sourceKs, fieldEvent.Keyspace)
...
func TestVStreamHeartbeats(t *testing.T) {
	...
	defaultCell := vc.Cells[vc.CellNames[0]]
	vc.AddKeyspace(t, []*Cell{defaultCell}, sourceKs, "0", initialProductVSchema, initialProductSchema,
