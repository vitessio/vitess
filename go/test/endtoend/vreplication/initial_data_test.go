...
func insertInitialData(t *testing.T) {
	t.Run("insertInitialData", func(t *testing.T) {
		...
		execMultipleQueries(t, vtgateConn, fmt.Sprintf("%s:0", sourceKs), string(lines))
		execVtgateQuery(t, vtgateConn, fmt.Sprintf("%s:0", sourceKs), "insert into customer_seq(id, next_id, cache) values(0, 100, 100);")
		execVtgateQuery(t, vtgateConn, fmt.Sprintf("%s:0", sourceKs), "insert into order_seq(id, next_id, cache) values(0, 100, 100);")
		execVtgateQuery(t, vtgateConn, fmt.Sprintf("%s:0", sourceKs), "insert into customer_seq2(id, next_id, cache) values(0, 100, 100);")
		log.Infof("Done inserting initial data")

		waitForRowCount(t, vtgateConn, fmt.Sprintf("%s:0", sourceKs), "product", 2)
		waitForRowCount(t, vtgateConn, fmt.Sprintf("%s:0", sourceKs), "customer", 3)
		waitForQueryResult(t, vtgateConn, fmt.Sprintf("%s:0", sourceKs), "select * from merchant",
...
func insertJSONValues(t *testing.T) {
	...
	vtgateConn, closeConn := getVTGateConn()
	defer closeConn()
	execVtgateQuery(t, vtgateConn, fmt.Sprintf("%s:0", sourceKs), "insert into json_tbl(id, j3) values(1, \"{}\")")
	execVtgateQuery(t, vtgateConn, fmt.Sprintf("%s:0", sourceKs), "insert into json_tbl(id, j1, j3) values(2, \"{}\", \"{}\")")
	execVtgateQuery(t, vtgateConn, fmt.Sprintf("%s:0", sourceKs), "insert into json_tbl(id, j2, j3) values(3, \"{}\", \"{}\")")
	execVtgateQuery(t, vtgateConn, fmt.Sprintf("%s:0", sourceKs), "insert into json_tbl(id, j1, j2, j3) values(4, NULL, 'null', '\"null\"')")
	execVtgateQuery(t, vtgateConn, fmt.Sprintf("%s:0", sourceKs), "insert into json_tbl(id, j3) values(5, JSON_QUOTE('null'))")
	execVtgateQuery(t, vtgateConn, fmt.Sprintf("%s:0", sourceKs), "insert into json_tbl(id, j3) values(6, '{}')")
	...
	for id <= NumJSONRows {
		...
		execVtgateQuery(t, vtgateConn, fmt.Sprintf("%s:0", sourceKs), query)
	}
}
...
func insertMoreCustomers(t *testing.T, numCustomers int) {
	...
	vtgateConn, closeConn := getVTGateConn()
	defer closeConn()
	maxID := waitForSequenceValue(t, vtgateConn, sourceKs, "customer_seq", numCustomers)
	...
	execVtgateQuery(t, vtgateConn, targetKs, sql)
}
...
func insertMoreProducts(t *testing.T) {
	...
	execVtgateQuery(t, vtgateConn, sourceKs, sql)
}
