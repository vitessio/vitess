...
func TestMultiTenantSimple(t *testing.T) {
	...
	log.InfoS(fmt.Sprintf("Migration completed, total rows in target: %d", actualRowsInserted))
...
func TestMultiTenantComplex(t *testing.T) {
	...
	t.Run("Verify all rows have been migrated", func(t *testing.T) {
		...
		log.InfoS(fmt.Sprintf("Migration completed, total rows inserted in target: %d", totalActualRowsInserted))
	})
}
...
func (mtm *multiTenantMigration) setup(tenantId int64) {
	log.InfoS(fmt.Sprintf("Creating MoveTables for tenant %d", tenantId))
