	...
	for {
		...
		case <-ctx.Done():
			require.FailNow(t, fmt.Sprintf("query %q did not succeed before the timeout of %s; last seen result: %v",
				query, timeout, qr))
		case <-ticker.C:
			log.InfoS(fmt.Sprintf("query %q failed with error %v, retrying in %ds", query, err, defaultTick))
		}
	}
}
...
func execQuery(t *testing.T, conn *mysql.Conn, query string) *sqltypes.Result {
	qr, err := conn.ExecuteFetch(query, 1000, false)
	if err != nil {
		log.ErrorS(fmt.Sprintf("Error executing query: %s: %v", query, err))
	}
...
func waitForRowCountInTablet(t *testing.T, vttablet *cluster.VttabletProcess, database string, table string, want int64) {
	...
	for {
		...
		if got == want {
			log.InfoS(fmt.Sprintf("waitForRowCountInTablet: found %d rows in table %s on tablet %s", want, table, vttablet.Name))
			return
		}
		select {
		case <-ctx.Done():
...
func waitForWorkflowState(t *testing.T, vc *VitessCluster, ksWorkflow string, wantState string, fieldEqualityChecks ...string) {
	...
	defer timer.Stop()
	log.InfoS(fmt.Sprintf("Waiting for workflow %q to fully reach %q state", ksWorkflow, wantState))
	for {
		output, err := vc.VtctldClient.ExecuteCommandWithOutput("Workflow", "--keyspace", keyspace, "show", "--workflow", workflow, "--compact", "--include-logs=false")
		...
		if done {
			log.InfoS(fmt.Sprintf("Workflow %q has fully reached the desired state of %q", ksWorkflow, wantState))
			return
		}
		select {
		case <-timer.C:
...
func getDebugVar(t *testing.T, port int, varPath []string) (string, error) {
	...
	log.InfoS(fmt.Sprintf("url: %s, varPath: %s", url, strings.Join(varPath, ":")))
	body := getHTTPBody(t, url)
...
func getShardRoutingRules(t *testing.T) string {
	...
	log.InfoS(fmt.Sprintf("GetShardRoutingRules err: %+v, output: %+v", err, output))
...
func (lg *loadGenerator) start() {
	...
	var id int64
	log.InfoS("loadGenerator: starting")
	...
	defer func() {
		defer lg.wg.Done()
		log.InfoS(fmt.Sprintf("loadGenerator: totalQueries: %d, successfulQueries: %d, deniedErrors: %d, ambiguousErrors: %d, reshardedErrors: %d, tableNotFoundErrors: %d, otherErrors: %d", totalQueries, successfulQueries, deniedErrors, ambiguousErrors, reshardedErrors, tableNotFoundErrors, otherErrors))
	}()
	for {
		select {
		case <-lg.ctx.Done():
			log.InfoS("loadGenerator: context cancelled")
			log.InfoS(fmt.Sprintf("loadGenerator: deniedErrors: %d, ambiguousErrors: %d, reshardedErrors: %d, tableNotFoundErrors: %d, otherErrors: %d", deniedErrors, ambiguousErrors, reshardedErrors, tableNotFoundErrors, otherErrors))
...
func appendToQueryLog(msg string) {
	...
	if err != nil {
		log.ErrorS(fmt.Sprintf("Error opening query log file: %v", err))
		return
	}
	defer file.Close()
	if _, err := file.WriteString(msg + "\n"); err != nil {
		log.ErrorS(fmt.Sprintf("Error writing to query log file: %v", err))
	}
}
