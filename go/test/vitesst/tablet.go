...
func (c *Cluster) startTablet(ctx context.Context, spec *TabletSpec) (*Tablet, error) {
	alias := fmt.Sprintf("vttablet-%d", spec.UID)
...
if [[ -d %[4]s ]]; then
  mysqlctl --tablet-uid %[5]d --mysql-port %[6]d --log-format text start
else
  mysqlctl --tablet-uid %[5]d --mysql-port %[6]d --log-format text init --init-db-sql-file %[7]s
fi
