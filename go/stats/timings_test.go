...
func TestTimingsDropDimension(t *testing.T) {
	clear()
	*dropDimensions = "a,c"

	t1 := NewTimings("timing_dropdim1", "help", "label")
	...
	t2 := NewTimings("timing_dropdim2", "help", "a")
	t2.Add("t1", 1)
	...
	t3 := NewMultiTimings("timing_dropdim3", "help", []string{"a", "b", "c"})
