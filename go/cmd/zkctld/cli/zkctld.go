...
func run(cmd *cobra.Command, args []string) error {
	...
	if zkd.Inited() {
		log.InfoS("already initialized, starting without init...")
		if err := zkd.Start(); err != nil {
			return fmt.Errorf("failed start: %v", err)
		}
	} else {
		log.InfoS("initializing...")
		if err := zkd.Init(); err != nil {
			return fmt.Errorf("failed init: %v", err)
		}
	}

	log.InfoS("waiting for signal or server shutdown...")
	sig := make(chan os.Signal, 1)
	signal.Notify(sig, syscall.SIGINT, syscall.SIGTERM)
	select {
	case <-zkd.Done():
		log.InfoS("server shut down on its own")
	case <-sig:
		log.InfoS("signal received, shutting down server")
