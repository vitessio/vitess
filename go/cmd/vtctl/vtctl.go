	...
	commandGroup{
		"Tablets", []command{
			...
			command{"MultiRestore", commandMultiRestore,
				"[-force] [-concurrency=4] [-fetch-concurrency=4] [-insert-table-concurrency=4] [-fetch-retry-count=3] [-write-bin-logs] [-skip-auto-increment-on-tables=] <destination zk path> <source zk path>...",
...
func commandMultiRestore(wrangler *wr.Wrangler, subFlags *flag.FlagSet, args []string) (status string, err error) {
	...
	err = wrangler.RestoreFromMultiSnapshot(destination, sources, *concurrency, *fetchConcurrency, *insertTableConcurrency, *fetchRetryCount, *writeBinLogs, saiot)
	return
}
