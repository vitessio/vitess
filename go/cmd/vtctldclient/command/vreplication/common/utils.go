...
func AddCommonSwitchTrafficFlags(cmd *cobra.Command, initializeTargetSequences bool) {
	...
	cmd.Flags().BoolVar(&SwitchTrafficOptions.Force, "force", false, "Force the traffic switch even if some non-critical actions cannot be performed; for example tablet refresh fails on some tablets in the keyspace.")
