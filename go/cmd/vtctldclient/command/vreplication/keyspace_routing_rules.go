...
func commandApplyKeyspaceRoutingRules(cmd *cobra.Command, args []string) error {
	...
	if applyKeyspaceRoutingRulesOptions.DryRun {
		...
		if applyKeyspaceRoutingRulesOptions.SkipRebuild {
			...
			if len(applyKeyspaceRoutingRulesOptions.Cells) == 0 {
	...
	_, err = common.GetClient().ApplyKeyspaceRoutingRules(common.GetCommandCtx(), &vtctldatapb.ApplyKeyspaceRoutingRulesRequest{
...
func commandGetKeyspaceRoutingRules(cmd *cobra.Command, args []string) error {
	...
	resp, err := common.GetClient().GetKeyspaceRoutingRules(common.GetCommandCtx(), &vtctldatapb.GetKeyspaceRoutingRulesRequest{})
	if err != nil {
		return err
	}
