...
func registerCommands(root *cobra.Command) {
	...
	create.Flags().DurationVar(&createOptions.MaxDiffDuration, "max-diff-duration", time.Duration(0), "How long should the VDiff run before being stopped -- and subsequently resumed unless --auto-retry=false -- in order to lessen the impact on tablets due to holding open database snapshots for long periods of time (0 is the default and means no time limit).")
