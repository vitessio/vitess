...
func init() {
	...
	EmergencyReparentShard.Flags().StringVar(&emergencyReparentShardOptions.RequiredPosition, "required-position", "", "Position the new primary must have received, applied or in its relay log, in Vitess position format (for example MySQL56/<uuid>:1-100). Fails if no candidate has received it. MySQL GTID shards only.")
