...
func init() {
	utils.SetFlagStringVar(ApplySchema.Flags(), &applySchemaOptions.DDLStrategy, "ddl-strategy", string(schema.DDLStrategyDirect), "Online DDL strategy and options, compatible with the @@ddl_strategy session variable. Examples: 'direct', 'mysql', 'vitess --postpone-completion', 'vitess --session-variable innodb_strict_mode=off'. The repeatable --session-variable name=value option applies variables in SESSION scope before DDL.")
