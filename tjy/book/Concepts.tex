...
A \vconcepts{\textbf{keyspace}} is a logical database.
...
A division within a Keyspace. All the instances inside a \vconcepts{Shard\textbf{Shard}} have the same data (or should have the same data,
...
A \vconcepts{\textbf{tablet}} is a single server that runs:
\begin{itemize}
...
A \vconcepts{\textbf{keyspace\_id}} is a column that is used to identify a primary entity of a keyspace, like user, video, order, etc.
...
The \vconcepts{\textbf{Replication Graph}}(~\ref{Replication-Graph}) represents the relationships between the master
...
The \vconcepts{\textbf{Serving Graph}}(~\ref{Serving-Graph}) is derived from the shard and replication graph.
...
The \vconcepts{\textbf{Topology Server}} is the backend service used to store the Topology data, and provide a locking service. The implementation we use in the tree is based on Zookeeper. Each Zookeeper process is run on a single server, but may share that server with other processes.
...
A \vconcepts{\textbf{Cell}} is a group of servers and network infrastructure collocated in an area. It is usually a full Data Center, or a subset of a full Data Center.
