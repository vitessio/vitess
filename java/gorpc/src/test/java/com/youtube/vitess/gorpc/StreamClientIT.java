 ...
 * 
 * These tests are mimicking the tests in go/rpcplus/streaming_test.go. Arith.Thrive is a streaming
 * method that returns a specified value "A" over "Count" fetches. Optionally pass in "ErrotAt" and
 * "BadTypeAt" to trigger corresponding errors.
 */
public class StreamClientIT {
  ...
  public void testStreamingBadTypeError() throws GoRpcException, IOException, ApplicationException {
    ...
    try {
      ...
      Assert.assertEquals("rpc: passing wrong type to sendReply", e.getMessage());
    }
    client.close();
  }
  ...
  public void testNewRequestWhileStreaming() throws GoRpcException, IOException,
    ...
    try {
      ...
      Assert.assertEquals("request not allowed as client is in the middle of streaming",
          e.getMessage());
    }
    client.close();
  }
