...
public class Util {
  ...
  public static void startFakeGoServer() throws IOException, InterruptedException {
  ...
  public static void startRealGoServer() throws IOException, InterruptedException {
  ...
  public static void stopRealGoServer() throws Exception {
    ...
    s.close();
    Assert.assertTrue("arith server failed to shutdown", sb.toString().contains("shutting down"));
    Assert.assertEquals("arith server failed to shutdown", 0, realGoServer.waitFor());
  }
}
