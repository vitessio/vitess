...
/**
 * Client represents an RPC client that can communicate with a Go RPC server. Same client can be
 * used for making multiple calls but concurrent calls are not supported. Multiple codecs can be
 * supported by extending ClientCodec interface.
 *
...
public class Client {
  ...
  /**
   * call makes a synchronous RPC call using the specified serviceMethod and args. It waits for a
   * response from the server indefinitely.
   */
  public Response call(String serviceMethod, Object args) throws GoRpcException,
  ...
  /**
   * Streaming equivalent of call(). This only initiates the request. Use streamNext() to fetch the
   * results.
   */
  public void streamCall(String serviceMethod, Object args) throws GoRpcException {
    writeRequest(serviceMethod, args);
    isStreaming = true;
  }
  ...
  public static Client dialHttp(String host, int port, String path, ClientCodecFactory cFactory)
