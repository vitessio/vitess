...
public class GoRpcBsonDecoder extends ExtendableBsonDecoder {
  /**
   * {@code 0x3f} is a valid type not supported by {@link org.bson.BasicBSONDecoder} but
   * {@link org.bson.BasicBSONDecoder#decodeElement()} is package private and deep inside.
   *
  ...
  protected boolean decodeElement() throws IOException {
    try {
      ...
      Matcher matcher =
          Pattern.compile("\\btype\\s*:\\s*([^\\s]+).*\\bname\\s*:\\s*([^\\s]+)").matcher(
              e.getMessage());
      if (matcher.find()) {
        if (GoRpcBsonEncoder.UNSIGNED_LONG.toString().equals(matcher.group(1))) {
          ((GoRpcBSONCallBack) _callback).gotULong(matcher.group(2), _in.readLong());
          return true;
        }
      }
      throw e;
