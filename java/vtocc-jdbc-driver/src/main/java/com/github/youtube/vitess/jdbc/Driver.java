 ...
 *
 * Expects jdbc url in {@code jdbc:vtocc://host:port/keyspace} form.
 * Can be used via {@link java.sql.DriverManager} or direct instantiation.
 *
...
public class Driver extends acolyte.Driver {
  ...
  public Connection connect(String url, Properties info) throws SQLException {
    ...
    try {
      if (!url.startsWith(jdbcPrefix)) {
        throw new IllegalArgumentException(
            "jdbc url specification differes in connect() and accptURL() methods");
      }
      ...
      logger.error("Invalid format for jdbc url, jdbc:vtocc://host:port/keyspace expected", e);
    ...
    return VtoccJdbcConnectionFactory.getInstance().create(
        jdbcUri.getQuery(), jdbcUri.getHost() + ":" + Integer.toString(jdbcUri.getPort()));
  }

  /** Acceps all urls starts with {@code jdbc:vtocc:}. */
  @Override
