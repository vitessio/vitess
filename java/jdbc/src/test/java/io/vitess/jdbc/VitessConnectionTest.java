...
public class VitessConnectionTest extends BaseTest {
    ...
    @Test
    public void testCreateStatementForClose()
            throws SQLException {
        VitessConnection vitessConnection = getVitessConnection();
    ...
    public void testCreatePreparedStatementForClose() throws SQLException {
        VitessConnection vitessConnection = getVitessConnection();
        assertFailsOnClosedConnection(vitessConnection, () -> vitessConnection.prepareStatement("query"));
    }
    ...
    @Test
    public void testDefaultSetAutoCommitForClose()
            throws SQLException {
        VitessConnection vitessConnection = getVitessConnection();
    ...
    public void testClientFoundRows2() throws SQLException {
        String url = "jdbc:vitess://locahost:9000/vt_keyspace/keyspace?TABLET_TYPE=replica&useAffectedRows=false";
        VitessConnection conn = new VitessConnection(url, new Properties());
    ...
    public void testTransactionIsolation() throws SQLException {
        ...
        doReturn(new DBProperties("random", "random", "random", Connection.TRANSACTION_REPEATABLE_READ, "random"))
                .when(conn)
                .getDbProperties();
        doReturn(new VitessMySQLDatabaseMetadata(conn)).when(conn).getMetaData();

        assertEquals(TransactionIsolation.DEFAULT, conn.getVtSession().getSession().getOptions().getTransactionIsolation());
        assertEquals(Connection.TRANSACTION_REPEATABLE_READ, conn.getTransactionIsolation());

        conn.setTransactionIsolation(Connection.TRANSACTION_READ_COMMITTED);

        assertEquals(TransactionIsolation.READ_COMMITTED, conn.getVtSession().getSession().getOptions().getTransactionIsolation());
        ...
        assertEquals(TransactionIsolation.READ_UNCOMMITTED, conn.getVtSession().getSession().getOptions().getTransactionIsolation());
    ...
    private void assertFailsOnClosedConnection(VitessConnection connection, Runthis failingRunnable) throws SQLException {
        connection.close();
        try {
            failingRunnable.run();
