...
public class Bsonify {
	...
	public static BSONObject batchQueryToBson(BatchQuery batchQuery) {
		...
		Iterator<Map<String, Object>> bvIter = batchQuery.getBindVarsList()
				.iterator();
	...
	public static QueryResult bsonToQueryResult(BSONObject result,
			List<Field> fields) {
		if (fields == null) {
			fields = bsonToFields(result);
		}
		List<Row> rows = bsonToRows(result, fields);
		long rowsAffected = ((UnsignedLong) result.get("RowsAffected"))
				.longValue();
	...
	public static List<Row> bsonToRows(BSONObject result,
			List<Field> fields) {
		...
		for (Object row : rows) {
			...
			for (Object col : cols) {
				...
				cells.add(new Cell(field.getName(), ft.convert(val),
						ft.javaType));
			}
	...
	public static SplitQueryResponse bsonToSplitQueryResponse(
			BSONObject reply) {
		String error = null;
		if (reply.containsField("Error")) {
			byte[] err = (byte[]) reply.get("Error");
		...
		for (Object split : result) {
			...
			BSONObject bindVars = (BasicBSONObject) query
					.get("BindVariables");
			...
			for (String key : bindVars.keySet()) {
				...
				if (bv == null) {
					throw new RuntimeException("invalid bind variable type: "
							+ val.getClass());
				}
			...
			Query q = new QueryBuilder(sql, keyspace, tabletType)
					.setKeyRanges(keyranges)
					.setBindVars(bindVariables)
					.setStreaming(true)
					.build();
