...
/**
 * This tests GrpcClient with a mock vtgate server (go/cmd/vtgateclienttest), over an SSL
 * connection.
 *
...
public class GrpcClientTlsTest extends RpcClientTest {
  ...
  private static void startVtgate() throws Exception {
    ...
    final String vtgate = String.format(
        "%s --grpc-cert %s --grpc-key %s --logtostderr --grpc-port %s --service-map grpc-vtgateservice",
