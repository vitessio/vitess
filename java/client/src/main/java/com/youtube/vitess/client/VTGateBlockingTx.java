...
public class VTGateBlockingTx {
  ...
  /**
   * Creates a new {@link VTGateTx} with the given {@link RpcClient} and {@link Session}
   * and wraps it in a synchronous API.
   */
  VTGateBlockingTx(RpcClient client, Session session) {
    tx = new VTGateTx(client, session);
  }
