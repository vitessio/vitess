...
EOF
  if [[ -n ${REDO_CAPACITY:-} ]]; then echo "innodb_redo_log_capacity = $REDO_CAPACITY" >>"$VTDATAROOT/extra.cnf"; fi
}
...
up() {
  ...
  vtctldclient CreateKeyspace --durability-policy=${DURABILITY:-none} "$KEYSPACE" >/dev/null 2>&1 || true
  ...
  for shard in $SHARDS; do
    for r in $(seq 0 "$REPLICAS"); do
      ...
      vttablet $TOPO --tablet-path "$alias" --tablet-hostname localhost --init-keyspace "$KEYSPACE" --init-shard "$shard" \
  ...
  vtgate $TOPO --cell "$CELL" --cells-to-watch "$CELL" --tablet-types-to-wait PRIMARY,REPLICA \
