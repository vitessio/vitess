...
- `HIGH_NOT_PRECEDENCE` is now also rejected. It is the remaining mode that changes how SQL text is interpreted and that the Vitess parser does not honor, so queries would be parsed differently at the VTGate than the session's `sql_mode` promises. `IGNORE_SPACE` is accepted: it is the one lexer mode the Vitess parser can read SQL under, and in this release it withholds the deprecation warning for a spaced aggregate call (see [the function-name section](#sqlparser-function-name-keywords)). Vitess-formatted SQL is lexed the same by MySQL with or without it.
...
The kept spaced aggregate produces a MySQL protocol warning, code 1287 (`ER_WARN_DEPRECATED_SYNTAX`), visible through `SHOW WARNINGS` and the warning count of the result, on every execution of the query, including through `EXECUTE` of a statement prepared with `PREPARE ... FROM`. VTGate counts such queries in the `VtGateWarnings` metric under the `SpacedAggrCall` label, and logs them, rate-limited, with the query redacted and therefore in its normalized form, with the parentheses attached; the log line names the aggregates. A session whose `sql_mode` includes `IGNORE_SPACE` gets no warning, since under that mode the reading is MySQL's. `IGNORE_SPACE` is accepted in `SET sql_mode` and in the VTGate `--sql-mode` default for that reason (see the [`sql_mode` validation section](#vtgate-sql-mode-rejection)). In this release that is all a session's `IGNORE_SPACE` does: VTGate reads every session's SQL the same way, so `now ()` is a stored-function call and a bare `now` an identifier for such a session too, as listed above. MySQL lexes the Vitess-formatted queries VTGate sends the same way with or without the mode, because every identifier that matches a keyword is quoted and every function call has its parenthesis attached.

**v26** removes the exception and reads each session's SQL under its own `sql_mode`. Without `IGNORE_SPACE`, `sum (x)` becomes a call of a stored function named `sum`, as in MySQL, once every tablet keeps the quoting. A session whose `sql_mode` includes `IGNORE_SPACE`, or a VTGate whose `--sql-mode` default does, gets the `IGNORE_SPACE` reading of every name on the list, which is the reading Vitess had before this release. To prepare, fix the queries the warning reports by attaching the parenthesis (`sum(x)`), or set `IGNORE_SPACE` in the session's `sql_mode`, or in the VTGate `--sql-mode` default for every session. `IGNORE_SPACE` is a documented MySQL mode with exactly this meaning, so a client that sets it behaves the same against MySQL directly.
