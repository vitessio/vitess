...
- `HIGH_NOT_PRECEDENCE` is now also rejected, alongside `ANSI_QUOTES`, `NO_BACKSLASH_ESCAPES`, `PIPES_AS_CONCAT` and `REAL_AS_FLOAT`. These are the modes that change how SQL text is interpreted and that the Vitess parser does not honor, so a query would be parsed differently at the VTGate than the session's `sql_mode` promises. The one lexer mode the parser can read SQL under is `IGNORE_SPACE`, which is accepted: VTGate reads a session's SQL under it (see [the function-name section](#sqlparser-function-name-keywords)). Vitess-formatted SQL is lexed the same by MySQL with or without it.
...
MySQL treats the function names on its "Function Name Parsing and Resolution" list (`ADDDATE`, `BIT_AND`, `BIT_OR`, `BIT_XOR`, `CAST`, `COUNT`, `CURDATE`, `CURTIME`, `DATE_ADD`, `DATE_SUB`, `EXTRACT`, `GROUP_CONCAT`, `MAX`, `MID`, `MIN`, `NOW`, `POSITION`, `SESSION_USER`, `STD`, `STDDEV`, `STDDEV_POP`, `STDDEV_SAMP`, `SUBDATE`, `SUBSTR`/`SUBSTRING`, `SUM`, `SYSDATE`, `SYSTEM_USER`, `TRIM`, `VARIANCE`, `VAR_POP`, `VAR_SAMP`, and `JSON_ARRAYAGG`, `JSON_OBJECTAGG` and `ST_COLLECT`, which MySQL 8.0 treats the same way although the page does not list them) as a keyword only when the name is immediately followed by `(`. Anywhere else the name is an ordinary identifier: `count (x)` with whitespace, a quoted `` `count`(x) `` and a qualified `db.count(x)` are all calls of a stored function named `count`, and a bare `now` is a column. Under `sql_mode=IGNORE_SPACE` the whitespace is permitted: the name is the keyword when a parenthesis follows it after whitespace too, so a table named after a function must be quoted where a parenthesis follows it, as in `create table CAST (a int)`; a name no parenthesis follows is still an identifier.

Vitess has always read these names as keywords wherever they appeared, whatever the session's `sql_mode`: `count (*)` and `sum (x)` were the built-ins, a bare `now`, `sysdate`, `curdate` or `curtime` was the function in any expression (`default now`, `where a < now`), and some of the names could not be used as table or column names. The parser now reads these names by a `sql_mode`. With `IGNORE_SPACE` set it reads exactly as MySQL does under that mode, which permits the whitespace Vitess permitted. Without it, it reads as MySQL does by default, with one exception kept for this release: **an aggregate on the list with whitespace or a comment before its parenthesis (`sum (x)`, `count/*c*/(*)`, `group_concat (distinct a)`) is still the aggregate**, and with `IGNORE_SPACE` the comment case is kept too, although MySQL reads it as a stored-function call under either mode. Read as a stored-function call, such a query could return a wrong result during a rolling upgrade: a VTTablet from an earlier release re-serializes the quoted `` `sum`(x) `` bare, so MySQL runs the built-in, and a scatter query would return one shard-local sum per shard, with no error, from a VTGate that no longer combines them. Keeping the aggregate reading for one release means no upgrade order can produce that result. Every other form fails or works loudly, so it follows MySQL now:

- A bare `now`, `sysdate`, `curdate` or `curtime` is an identifier, as in MySQL: `default now` / `on update now` without parentheses are rejected, write `now()`, and `select now from t` or `where a < now` names a column, which does not exist unless the table has one. During a rolling upgrade a VTGate from the earlier release still reads the function; a downgrade reads it again.
- `cast (1 as char)`, `trim (leading ...)`, `position (... in ...)`, `extract (... from ...)`, `substring (... from ...)` and `date_add (..., interval ...)` with whitespace before the parenthesis are syntax errors, since the built-in argument syntax needs the attached parenthesis.
- `now ()`, `curdate ()`, `substr (a, 1)` and the other non-aggregates with regular argument syntax still parse with whitespace, but as stored-function calls: VTGate sends them quoted, does not evaluate them itself, and MySQL reports that the function does not exist unless a stored function by that name does. During a rolling upgrade a VTTablet from an earlier release runs the built-in instead, which is what the query returned before.
- `session_user(1)` and `system_user(1)` are syntax errors.
- Tables and columns may be named after any listed function without quoting: `create table CAST (a int)`, `create table t (now int, extract int)` and `select now, sum from t` are valid, as in MySQL. Under `IGNORE_SPACE` the first is a syntax error, as in MySQL, because a parenthesis follows the name.

The kept spaced aggregate produces a MySQL protocol warning, code 1287 (`ER_WARN_DEPRECATED_SYNTAX`), visible through `SHOW WARNINGS` and the warning count of the result, on every execution of the query, including through `EXECUTE` of a statement prepared with `PREPARE ... FROM`. VTGate counts such queries in the `VtGateWarnings` metric under the `SpacedAggrCall` label, and logs them, rate-limited, with the query redacted and therefore in its normalized form, with the parentheses attached; the log line names the aggregates. A session whose `sql_mode` includes `IGNORE_SPACE` gets no warning for a call separated by whitespace, since under that mode the reading is MySQL's; a call separated by a comment (`count/*c*/(*)`) warns in every session, because `IGNORE_SPACE` skips whitespace, not comments, and MySQL reads such a call as a stored function under either mode. That warning says so, and that only attaching the parenthesis keeps the call. `IGNORE_SPACE` is accepted in `SET sql_mode` for that reason (see the [`sql_mode` validation section](#vtgate-sql-mode-rejection)); a VTGate-wide `sql_mode` default, which a later release adds, will accept it the same way. VTGate reads each session's SQL under the session's `sql_mode`: with `IGNORE_SPACE`, `now ()` and `cast (1 as char)` are the built-ins, and a table named after a listed function must be quoted where a parenthesis follows it, as in MySQL under that mode. The two readings never share a cached plan, since the lexer modes the parser honors are part of the plan cache key. A session that never set `sql_mode` reads under MySQL's default, without `IGNORE_SPACE`. MySQL lexes the Vitess-formatted queries VTGate sends the same way with or without the mode, because every identifier that matches a keyword is quoted and every function call has its parenthesis attached.

**v26** removes the exception. Without `IGNORE_SPACE`, `sum (x)` becomes a call of a stored function named `sum`, as in MySQL, once every tablet keeps the quoting. A session whose `sql_mode` includes `IGNORE_SPACE` keeps the `IGNORE_SPACE` reading of every name on the list, which for the whitespace form is the reading Vitess had before this release. To prepare, fix the queries the warning reports by attaching the parenthesis (`sum(x)`), or set `IGNORE_SPACE` in the session's `sql_mode`; a VTGate-wide `sql_mode` default, which a later release adds, will make that possible for every session at once. `IGNORE_SPACE` is a documented MySQL mode with exactly this meaning, so a client that sets it behaves the same against MySQL directly.

Three more things change in this release, whatever the reading. Each matches MySQL:

- A quoted call by one of these names, `` `sum`(x) ``, reaches MySQL quoted once both the VTGate and the tablet run this release. VTGate previously printed it bare, so MySQL ran the built-in `sum`, and on a scatter query that returned one row per shard, silently, because VTGate did not treat the call as an aggregate. It now takes MySQL's stored-function path, and MySQL reports that the function does not exist unless a stored function by that name does. VTGate does not evaluate such a call itself either. During a rolling upgrade a new VTGate with an old tablet keeps the previous result, because the old tablet prints the name bare; a keyspace switches once its tablets are upgraded. The order of the upgrade does not matter otherwise.
...
- `ST_Collect` is an aggregate in MySQL and is now treated as one: a single-shard query pushes it down, and a scatter query using it is rejected as unsupported rather than run per shard.

Programs that use the `go/vt/sqlparser` package directly: `sqlparser.Options` has a new `SQLMode` field, the `sql_mode` the parser reads SQL under; a zero value is MySQL's default, without `IGNORE_SPACE`, and of the lexer modes the parser honors `IGNORE_SPACE` only. A parser built with zero options therefore follows MySQL's default reading from this release on, with the aggregate exception above. `Parser.WithSQLMode` returns a parser for another `sql_mode`, and `Parser.ParseWithSpacedAggrCalls` parses like `Parse2` and also returns the aggregate calls read under that exception, each with whether a comment separates it from its parenthesis. A `FuncExpr` is a generic call, a call by an identifier, and serializes with the name quoted whenever the name is one of these built-ins (`IsFuncCallKeywordName`). `curdate()`, `current_date()`, `utc_date()`, `session_user()` and `system_user()` parse into the new `BuiltinFuncExpr`, and `st_collect(...)` into the new `STCollect` aggregate node; previously all six were `FuncExpr`.
