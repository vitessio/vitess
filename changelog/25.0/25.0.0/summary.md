...
`--temp-table-heartbeat-time` **must be set far enough below the tablets' effective reserved-connection timeout for the workload** — `--queryserver-config-transaction-timeout`, or `--queryserver-config-olap-transaction-timeout` for OLAP sessions — **to leave room for one keepalive round-trip**, or the connection can still be reclaimed between heartbeats. The tablet refreshes a connection's timer only when a keepalive reaches it, so the worst-case gap is the interval plus a brief per-tablet spread plus one round-trip (bounded by the per-tablet beat budget, which can reach three-quarters of the interval at short intervals); as a rule of thumb keep the interval under half of that timeout. A session that sets its own shorter transaction timeout (`SET @@transaction_timeout`) at or below that worst-case gap overrides the keepalive for that session — its temporary tables are then subject to that shorter timeout, by design. If VTGate is lost (crash or restart), the heartbeats stop and the tablet reclaims the connection at its normal timeout, so nothing leaks.
