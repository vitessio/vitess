...
**Schema change:** this PR adds a `UNIQUE (alias, analysis)` index to the internal `recovery_detection` table. VTOrc drops and recreates its SQLite database on startup, so no migration is needed. The behavioral effect is that repeated detections of the same ongoing failure on the same tablet now collapse to a single row (via `INSERT OR IGNORE`) rather than accumulating one row per poll cycle; the `detection_id` is stable for the lifetime of that failure. This change applies to all vtorc deployments, not only those using `--cells-no-recovery`.
