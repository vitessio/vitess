...
VTGate previously rejected `SET sql_mode = ...` statements that enable a mode the Vitess parser did not support (`ANSI_QUOTES`, `NO_BACKSLASH_ESCAPES`, `PIPES_AS_CONCAT`, `REAL_AS_FLOAT`), with a check that compared mode names textually. Two things change:

VTGate now implements MySQL 8.x's `sql_mode` assignment semantics (verified against MySQL 8.0.46) and validates every assignment against them:

- Unknown mode names and invalid numeric values fail with MySQL's own errors: `ER_WRONG_VALUE_FOR_VAR` (1231) and, for the bits of modes removed in MySQL 8.0, `ER_UNSUPPORTED_SQL_MODE` (3899). Numeric values are decoded against MySQL's `sql_mode` bitmask, so `SET sql_mode = 1048576` sets `NO_BACKSLASH_ESCAPES`.
- Constant values — literals, but also constant expressions such as `CONCAT` over literals — are validated at planning time with no shard round trip, including when `--enable-system-settings` is disabled. Non-constant expressions are validated at execution time, once their value is known.
- The session stores the canonical form MySQL would report back for `@@sql_mode` — names uppercased, combination modes expanded, in canonical order — regardless of how the assignment spelled the value (numeric bitmasks included), so the parser and the transports always decode what the session set.

And the modes that change how SQL text is interpreted — `ANSI_QUOTES`, `PIPES_AS_CONCAT`, `IGNORE_SPACE`, `NO_BACKSLASH_ESCAPES`, `HIGH_NOT_PRECEDENCE`, `REAL_AS_FLOAT`, and the `ANSI` combination — are now *supported* instead of rejected: the VTGate parses each query under the session's `sql_mode`, and plans are cached per parse-relevant mode combination. The SQL a backend receives is always VTGate's mode-independent serialization: strings are single-quoted, identifiers are backticked or bare (including MySQL's function-keyword names, always quoted as identifiers), `||` never survives serialization (it becomes `concat(...)` or `or` at parse time), and `REAL` resolves to `FLOAT` at parse time under `REAL_AS_FLOAT`, matching MySQL's own parse-time resolution.

The `sql_mode` value forwarded to backends (via `SET_VAR` hints or connection settings) has exactly two modes removed — `NO_BACKSLASH_ESCAPES` and `HIGH_NOT_PRECEDENCE`, under which a backend would lex that serialized text differently than it was written. Every other mode is forwarded, the `ANSI` combination included, so the backend enforces their resolution- and execution-time semantics itself — e.g. `ANSI`'s standard-SQL handling of aggregate functions with outer references, or `ONLY_FULL_GROUP_BY` — while their lexer aspects are inert on the canonical text.

Prepared statements keep the meaning their text had when they were prepared, like MySQL's own (which parses at prepare and never re-parses), however the session's `sql_mode` changes between PREPARE and EXECUTE; runtime modes stay the live session's at execute time, also matching MySQL. A statement prepared over the binary protocol records the parse-relevant `sql_mode` bits at prepare time and is parsed and plan-cached under them on every execute. A SQL-level prepared statement (`PREPARE ... FROM`) stores the canonical serialization of its prepare-time parse and executions parse that text under the canonical rules — the stored text itself carries the statement's meaning, so nothing else needs to travel with it. **Deployment note**: gRPC clients that inspect the `prepare_statement` entries of their session proto now see this canonical form (e.g. ``select * from `user` where id = :v1``) instead of the text as sent; entries stored by older vtgates keep reading correctly, in both rolling-upgrade directions.
...
VTTablet applies the same `sql_mode` validation as VTGate (see [the VTGate section](#vtgate-sql-mode-validation)) at its own entry points, returning the identical errors: connection settings (the settings pool and reserved connections) and session-scope `SET sql_mode` statements. Connection settings must parse as SET statements carrying constant `sql_mode` values — they are applied with no read-back afterwards, so a value that cannot be judged and stripped upfront is rejected. This concerns clients that reach the query service without a VTGate in front of it. `SET_VAR` optimizer hints in incoming queries are not judged: a hint applies to the hinted statement's execution only and cannot change how that statement's own text is lexed, so it is forwarded for MySQL to judge — MySQL warns about and ignores invalid hint values, exactly as if the client had sent the hint to it directly.

Valid values are accepted in full, parse-relevant modes included, with the same split VTGate applies: VTTablet parses queries under the parse-relevant modes of the connection they run on — whether those came in as connection settings or through a `SET sql_mode` statement on a dedicated connection — and its plan cache keys on them. The value applied to MySQL has only `NO_BACKSLASH_ESCAPES` and `HIGH_NOT_PRECEDENCE` removed; every other mode is forwarded for MySQL to enforce. A `SET sql_mode` with a constant value is judged and, when needed, stripped when it is planned; a non-constant expression executes as MySQL validates it, and the applied `@@sql_mode` is then read back so the connection records its parse-relevant modes, with the two unforwardable ones moved off the MySQL session the same way. Such a `SET` must assign `sql_mode` alone: the statement can still fail after it ran, MySQL applies none of a failed `SET`'s assignments, and a multi-assignment `SET` would already have applied its other assignments by then, so it is rejected upfront.

The `ApplySchema` `--session-variable` option keeps rejecting `sql_mode` values that change how SQL text is interpreted, both when the DDL strategy is parsed and on the tablet applying the variables: the migration statements executed under such a variable are Vitess-formatted SQL with no parser involved, so these modes cannot be honored there.

The server's own configuration is covered as well: MySQL lexes every statement under the session `sql_mode` in effect *before* the statement — a `SET_VAR` hint cannot influence the parsing of its own statement — so Vitess-formatted SQL should always be lexed under the same rules it was serialized with, regardless of how the backend happens to be configured. Every MySQL connection Vitess creates now strips the lexer modes from the session `sql_mode` it inherits from the server's global value, at the single connector choke point all components dial through — query serving, schema tracking and apply, heartbeats, replication management, Online DDL, vreplication and VDiff, VStream snapshots, and init scripts alike — while preserving all runtime modes (strict modes, zero-date handling, etc.). The settings-pool reset restores this neutralized value rather than `default` for the same reason.

Two deliberate consequences: connections serving `ExecuteFetchAsDba`-style admin RPCs also start neutralized — operator-supplied statements run with the server's lexer modes stripped, and a multi-statement batch can still set the session `sql_mode` explicitly; and connections to *external* MySQL servers (vreplication sources, point-in-time recovery) are neutralized too, since Vitess sends them the same Vitess-formatted SQL. Statements an operator's `sql_mode`-sensitive tooling sends outside Vitess are unaffected — the neutralization is session-scoped and never touches the global value.

**Impact**: Queries now always run with the server's lexer modes stripped from the MySQL session, so Vitess-formatted SQL parses consistently regardless of the backend's global configuration; sessions that want these modes set them explicitly and the Vitess parser honors them.
...
plus the rejection of modes that change how SQL text is interpreted — the statements
executed under these variables are Vitess-formatted SQL with no parser involved, so
those modes cannot be honored here. Validation runs both when the DDL strategy is
parsed and on the tablet applying the variables.
