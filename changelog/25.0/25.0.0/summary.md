...
- The session stores the canonical form MySQL would report back for `@@sql_mode`: names uppercased, combination modes expanded, in canonical order. This holds regardless of how the assignment spelled the value, numeric bitmasks included. The parser and the transports therefore always decode what the session set.
...
The SQL a backend receives is always VTGate's mode-independent serialization. Strings are single-quoted. Identifiers are backticked or bare, and MySQL's function-keyword names are always quoted when they are used as identifiers. `||` never survives serialization: it becomes `concat(...)` or `or` at parse time. `REAL` resolves to `FLOAT` at parse time under `REAL_AS_FLOAT`, which matches MySQL's own parse-time resolution.

The `sql_mode` value forwarded to backends, through `SET_VAR` hints or connection settings, has exactly two modes removed: `NO_BACKSLASH_ESCAPES` and `HIGH_NOT_PRECEDENCE`. Under those two modes, a backend would lex the serialized text differently than it was written. Every other mode is forwarded, the `ANSI` combination included. The backend then enforces their resolution-time and execution-time semantics itself — for example, `ANSI`'s standard-SQL handling of aggregate functions with outer references, or `ONLY_FULL_GROUP_BY`. Their lexer aspects have no effect on the canonical text.
...
Connection settings must parse as SET statements that carry constant `sql_mode` values. Settings are applied with no read-back afterwards, so a value that cannot be judged and stripped upfront is rejected.
...
Valid values are accepted in full, parse-relevant modes included, with the same split VTGate applies. VTTablet parses queries under the parse-relevant modes of the connection they run on. Those modes can come in as connection settings or through a `SET sql_mode` statement on a dedicated connection. The plan cache keys on them. The value applied to MySQL has only `NO_BACKSLASH_ESCAPES` and `HIGH_NOT_PRECEDENCE` removed. Every other mode is forwarded for MySQL to enforce. A `SET sql_mode` with a constant value is judged and, when needed, stripped at planning time. A non-constant expression executes, and MySQL validates it. The applied `@@sql_mode` is then read back: the connection records its parse-relevant modes, and the two unforwardable ones are moved off the MySQL session the same way. Such a `SET` must assign `sql_mode` alone. The statement can still fail after it ran, and MySQL applies none of a failed `SET`'s assignments, so a multi-assignment `SET` with a non-constant `sql_mode` is rejected upfront.
...
**Impact**: Queries always run with the server's lexer modes stripped from the MySQL session, so Vitess-formatted SQL parses consistently regardless of the backend's global configuration. Sessions that want these modes set them explicitly, and the Vitess parser honors them.
