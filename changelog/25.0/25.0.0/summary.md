...
- `cast (1 as char)`, `count (*)`, `trim (leading 'a' from b)`, `position ('a' in b)`, `extract (year from d)`, `substring (a from 1)`, `date_add (d, interval 1 day)` and `group_concat (distinct a)` with whitespace before the parenthesis are syntax errors, as in MySQL, since the built-in argument syntax needs the attached parenthesis. A comment between the name and the parenthesis (`count/*c*/(*)`) separates them the same way.
...
**Upgrade tablets before VTGates.** The stored-function reading of `now ()`, `sum (x)` and the quoted calls depends on both components. A VTTablet re-serializes every query before sending it to MySQL, and a VTTablet from an earlier release prints a quoted `` `sum`(x) `` bare as `sum(x)`, so MySQL runs the built-in. What a query returns during a rolling upgrade therefore depends on the order:

- Tablets first, then VTGates: nothing changes until a VTGate is upgraded. An old VTGate sends `sum(x)` for both forms and a new tablet passes it through, so each query keeps the earlier release's result and switches to MySQL's semantics the moment its VTGate is upgraded.
- VTGates first, then tablets: while a new VTGate talks to old tablets, `sum (x)` on a scatter query returns one shard-local sum per shard instead of the total it returned before, with no error, because the new VTGate no longer aggregates it and the old tablets hand MySQL the aggregate. The quoted `` `sum`(x) `` keeps the earlier release's result. The window closes for a keyspace once its tablets are upgraded, and rolling the VTGate back closes it too.

Upgrading tablets first is the order Vitess recommends in general, and here it avoids any intermediate result.

Programs that use the `go/vt/sqlparser` package directly: a `FuncExpr` is a generic call, a call by an identifier, and serializes with the name quoted whenever the name is one of these built-ins (`IsFuncCallKeywordName`). `curdate()`, `current_date()`, `utc_date()`, `session_user()` and `system_user()` parse into the new `BuiltinFuncExpr`, and `st_collect(...)` into the new `STCollect` aggregate node; previously all six were `FuncExpr`. `ST_Collect` is an aggregate in MySQL and is now treated as one: a scatter query using it is rejected as unsupported rather than run per shard.
