...
`RowChange` now has an additional `before_data_columns` bitmap, set only when the before image is partial (and `--vreplication_experimental_flags` allows NOBLOB row images, which is the default). A bit is set for every column that is present in the before image. The field is additive: VReplication ignores it and consumers that do not know about it are unaffected.
