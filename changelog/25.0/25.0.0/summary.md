...
`COM_RESET_CONNECTION` now resets vtgate's session the way MySQL resets its own: it releases the reserved connections and also clears the session's recorded system variables and reserved-connection mode. Previously the recorded `SET` values survived a reset and were silently re-applied on the next query; applications behind connection poolers that reset connections between checkouts (Connector/J, ProxySQL) will now observe MySQL-standard behavior — session variables revert to defaults on reset.
