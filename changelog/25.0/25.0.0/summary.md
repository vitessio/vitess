...
A session can now enable `PIPES_AS_CONCAT`, alone or with other modes: `SET sql_mode = 'PIPES_AS_CONCAT,STRICT_TRANS_TABLES'`. VTGate parses the session's SQL under the session's `sql_mode`, so `||` is the concatenation operator in that session, as in MySQL, and logical OR in a session without the mode. A result column produced by an unaliased `||` is named after the `concat()` call the operator is read as, `concat(a, b)`, where MySQL names it after the text as written, `a || b`. VTGate names an unaliased result column after the text it sends to the tablets in general, with bind variable placeholders where the client wrote literals, and making the names follow the text as written, for `||` and everything else, is planned as a change of its own; alias the expression for a stable name. The mode is forwarded to the tablets with the rest of the session's `sql_mode`, as every other mode is, and the tablets parse under it as well (see [the VTTablet section](#vttablet-sql-mode-parsing)); a `||` read under the mode reaches MySQL as a `concat()` call, which means the same thing under any `sql_mode`. Plans are cached per reading, so the two readings of the same text never share a plan.
