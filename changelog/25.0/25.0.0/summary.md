...
VTTablet applies the same `sql_mode` validation as VTGate (see [the VTGate section](#vtgate-sql-mode-validation)) at its own entry points, returning the identical errors: connection settings (the settings pool and reserved connections), `SET_VAR` optimizer hints in incoming queries, and session-scope `SET sql_mode` statements. This concerns clients that reach the query service without a VTGate in front of it.
