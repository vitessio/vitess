...
PRS prefers version over position on most paths because it catches the elected tablet up to a known position before promotion (so position is not a data-safety concern), and on the initialization path because no tablet has ever replicated (nothing to lose). On the no-clear-primary path, however, PRS promotes the elected tablet *without* catching it up, and demoted replicas can hold received-but-unapplied relay-log transactions — so there PRS keeps position first (matching ERS), using version only to break ties among equally-advanced candidates. When this causes a lower (more broadly compatible) version to be passed over because a higher-version candidate is more advanced, PRS logs a warning; run PRS again once the shard has a healthy primary to move to the preferred version if desired.
    ...
    **Behavior change:** PRS during a rolling MySQL upgrade will now prefer lower-version candidates even when they are slightly behind in replication position (on the graceful and initialization paths — see the no-clear-primary caveat above). The elected tablet will catch up to the old primary's demotion position before completing the reparent, which may increase reparent latency. Operators should ensure `--wait-replicas-timeout` is generous enough to accommodate this catch-up time.
...
**Behavior change (all tablets must be reachable for self-elected initial promotion):** the containment check above requires reading every non-`RESTORE` tablet's position, so on the initialization path (a shard that has never had a primary) PRS now **fails if any tablet is unreachable**, rather than promoting the elect regardless as before. This closes a data-loss window — a tablet seeded from a backup or external source can hold transactions the version-preferred elect lacks, which a blind `InitPrimary` would discard — but it does mean a single not-yet-started replica can now block initialization. Passing `--new-primary` does not exempt the reachability requirement (an explicitly-named primary must still be shown to contain every reachable tablet's position). PRS already assumes all tablets are healthy, so this aligns initialization with the rest of PRS; if a tablet is intentionally absent, bring it up (or remove it from the shard) before running PRS.

**Behavior change (ERS mixed GTID/non-GTID detection now includes former primaries):** ERS refuses to run on a shard that mixes GTID-based and non-GTID-based replication (their position semantics differ, so a unified most-advanced check is unsafe). This detection previously considered only reachable replicas; it now also folds in a reachable former primary's executed position, closing a hole where a GTID/non-GTID mix split across replicas and a former primary could slip past the guard. A former primary with an empty position does not contribute. Homogeneous shards are unaffected; only genuinely mixed shards that previously slipped through are now correctly rejected.
