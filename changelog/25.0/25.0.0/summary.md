...
- A trust anchor, that is, a CA certificate in the CA file that a chain ends at, is trusted as configured and not checked against a CRL of its own issuer, unless that issuer is in the CA file as well and its CRL lists the anchor: every chain that ends at such an anchor is then rejected, whether or not the peer presents it, and a warning names the revoked CA certificate when the configuration is built. An intermediate CA configured without its root is not checked against the root's CRL.
