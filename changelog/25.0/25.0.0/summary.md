...
The kept spaced aggregate produces a MySQL protocol warning, code 1287 (`ER_WARN_DEPRECATED_SYNTAX`), visible through `SHOW WARNINGS` and the warning count of the result, on every execution of the query, including through `EXECUTE` of a statement prepared with `PREPARE ... FROM`. VTGate counts such queries in the `VtGateWarnings` metric under the `SpacedAggrCall` label, and logs them, rate-limited, with the query redacted and therefore in its normalized form, with the parentheses attached; the log line names the aggregates. A session whose `sql_mode` includes `IGNORE_SPACE` gets no warning, since under that mode the reading is MySQL's. `IGNORE_SPACE` is accepted in `SET sql_mode` for that reason (see the [`sql_mode` validation section](#vtgate-sql-mode-rejection)); a VTGate-wide `sql_mode` default, which a later release adds, will accept it the same way. In this release that is all a session's `IGNORE_SPACE` does: VTGate reads every session's SQL the same way, so `now ()` is a stored-function call and a bare `now` an identifier for such a session too, as listed above. MySQL lexes the Vitess-formatted queries VTGate sends the same way with or without the mode, because every identifier that matches a keyword is quoted and every function call has its parenthesis attached.
