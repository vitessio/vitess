...
- The keyword form of a built-in keeps the spelling it was written with, so an unaliased `SELECT IF(a, 1, 2)`, `SELECT USER()` or `SELECT NOW()` returns a column named `IF(a, 1, 2)`, `USER()` or `NOW()`, as MySQL does; previously these were lower-cased, as were `CURRENT_TIMESTAMP` and the other date and time functions.
- An unaliased select expression that VTGate sends to MySQL spelled differently from how it was written (`COUNT(*)` is sent as `count(*)`, `sum( a )` as `sum(a)`, `a+1` as `a + 1`) now carries the text as written as its alias, so the result column is named as MySQL would name it for the original query. This closes the remaining half of [#18182](https://github.com/vitessio/vitess/issues/18182): cross-shard aggregations already kept the written name, single-route queries returned MySQL's name for the reprinted text. Expressions VTGate evaluates itself are named the same way.
