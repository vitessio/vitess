...
The keepalive applies to connections using the MySQL protocol. Sessions used via the VTGate gRPC API travel with each request and live client-side between calls, so VTGate cannot observe whether the client is still alive between calls and they do not receive keepalives. Their temporary tables remain subject to the tablet's reserved-connection timeout exactly as before this feature; regular query activity on the session within that timeout keeps them alive.
