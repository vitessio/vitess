...
A gRPC server started with `--grpc-enable-optional-tls` now reports its connections by transport, `tls` or `plaintext`, in two new stats: `GrpcOptionalTlsOpenConnections`, the connections currently open, and `GrpcOptionalTlsConnections`, the connections handshaken so far. Optional TLS serves plain-text connections unauthenticated so that clients can be moved to TLS one at a time, including when `--grpc-ca` is set, whose client certificate check only applies to the TLS connections. Every client has moved, and `--grpc-enable-optional-tls` can be dropped, when `GrpcOptionalTlsOpenConnections` is zero for `plaintext` and `GrpcOptionalTlsConnections` no longer increases for it: the first shows that no plain-text client is connected, which matters because gRPC connections are long-lived and a client that connected long ago does not handshake again, and the second that none is connecting anymore. A server that has both flags also says so in its startup warning now.
