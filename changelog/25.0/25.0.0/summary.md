...
VTGate already rejected `SET sql_mode = ...` statements that enable a mode the Vitess parser does not support (`ANSI_QUOTES`, `NO_BACKSLASH_ESCAPES`, `PIPES_AS_CONCAT`, `REAL_AS_FLOAT`), but the check compared mode names textually and could be bypassed. VTGate now implements MySQL 8.x's `sql_mode` assignment semantics (verified against MySQL 8.0.46) and validates every assignment against them:
...
VTTablet now applies the same `sql_mode` validation as VTGate (see [the VTGate section](#vtgate-sql-mode-rejection)) at its own entry points, returning the identical errors: connection settings (the settings pool and reserved connections), `SET_VAR` optimizer hints in incoming queries, and session-scope `SET sql_mode` statements. Constant values are judged before execution. Connection settings must parse as SET statements carrying constant `sql_mode` values — they are applied with no verification afterwards, so a value that cannot be judged upfront (e.g. a `CONCAT` expression) is rejected on both the settings-pool and reserved-connection paths. A non-constant expression in a `SET` statement executed on a dedicated connection is verified after execution instead — the applied `@@sql_mode` is read back and, if it fails validation, does not decode as MySQL 8.x modes, or cannot be read back at all, the statement is undone: a `SET` that only assigned `sql_mode` has the previous mode restored, making it a no-op, while a multi-assignment `SET` closes the connection, since MySQL applies none of a failed `SET`'s assignments and the other variables cannot be restored. The connection is also closed if the restore itself fails. The `ApplySchema` `--session-variable` option validates `sql_mode` values the same way, both when the DDL strategy is parsed and on the tablet applying the variables.

This check concerns clients that bypass VTGate's validation: older VTGates in a mixed-version cluster and clients that talk to the query service directly.
