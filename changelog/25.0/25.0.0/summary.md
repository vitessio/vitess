...
- Integrated: `vtadmin --ui=vtadmin2` serves the HTML UI (default listen address `:15001`, overridable with `--ui-addr`). `--ui-read-only` hides mutating actions. `--ui-debug-json` enables `?format=json` page dumps. `--ui-trust-proxy-https` marks cookies `Secure` when `X-Forwarded-Proto: https` is present; only enable this behind a trusted HTTPS-terminating proxy, because that header is otherwise spoofable.
