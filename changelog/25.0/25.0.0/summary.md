...
Programs that use the `go/vt/sqlparser` package directly: `sqlparser.Options` has a new `SQLMode` field, the `sql_mode` the parser reads SQL under; a zero value is MySQL's default, without `IGNORE_SPACE`, and of the lexer modes the parser honors `IGNORE_SPACE` only. A parser built with zero options therefore follows MySQL's default reading from this release on, with the aggregate exception above. `Parser.ParseWithSpacedAggrCalls` parses like `Parse2` and also returns the aggregate names read under that exception. A `FuncExpr` is a generic call, a call by an identifier, and serializes with the name quoted whenever the name is one of these built-ins (`IsFuncCallKeywordName`). `curdate()`, `current_date()`, `utc_date()`, `session_user()` and `system_user()` parse into the new `BuiltinFuncExpr`, and `st_collect(...)` into the new `STCollect` aggregate node; previously all six were `FuncExpr`.
