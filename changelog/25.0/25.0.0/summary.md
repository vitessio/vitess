...
Prepared statements keep the meaning their text had when they were prepared, like MySQL's own (which parses at prepare and never re-parses): a statement prepared over the binary protocol records the parse-relevant `sql_mode` bits at prepare time and is parsed and plan-cached under them on every execute, however the session's `sql_mode` changes in between; runtime modes stay the live session's at execute time, also matching MySQL. SQL-level prepared statements (`PREPARE ... FROM`) carry no such pin, so a `SET sql_mode` that changes the parse-relevant modes is rejected while any exist — runtime-only changes stay allowed.
