...
Rolling upgrades: the stored-function reading of `now ()`, `sum (x)` and the other whitespace-separated or quoted calls holds only once the VTTablets are upgraded too. A VTTablet re-serializes every query before sending it to MySQL, and a VTTablet from an earlier release prints a quoted `` `sum`(x) `` bare as `sum(x)`, so MySQL runs the built-in: during the window in which a new VTGate talks to old VTTablets, such a query returns shard-local aggregates instead of the stored function's per-row results. That is the behaviour of the earlier release, not a new failure, but it means the corrected semantics apply to a keyspace only after its tablets run this release.
