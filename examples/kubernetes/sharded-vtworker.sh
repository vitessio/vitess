...
cat vtworker-pod-template.yaml | sed -e "$sed_script" | $KUBECTL --server=$KUBERENETAPI create -f -

set +e

# Wait for vtworker pod to show up.
until [ $($KUBECTL --server=$KUBERENETAPI get pod -o template --template '{{.status.phase}}' vtworker 2> /dev/null) = "Running" ]; do
  echo "Waiting for vtworker pod to be created..."
	sleep 1
done

echo "Following vtworker logs until termination..."
$KUBECTL --server=$KUBERENETAPI logs -f vtworker
...
exit_code=$($KUBECTL --server=$KUBERENETAPI get -o template --template '{{(index .status.containerStatuses 0).state.terminated.exitCode}}' pods vtworker)

echo "Deleting vtworker pod..."
$KUBECTL --server=$KUBERENETAPI delete pod vtworker
