...
for cell in $cells; do
  ...
  cat vtgate-service-template.yaml | sed -e "$sed_script" | $KUBECTL --server=$KUBERENETAPI create --namespace=$VITESS_NAME -f -
  ...
  cat $VTGATE_TEMPLATE | sed -e "$sed_script" | $KUBECTL create --server=$KUBERENETAPI --namespace=$VITESS_NAME -f -
done
