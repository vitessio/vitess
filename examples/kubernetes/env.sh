...
get_vtctld_pod() {
  $KUBECTL --server=$KUBERENETAPI get -o template --template "{{if ge (len .items) 1 }}{{(index .items 0).metadata.name}}{{end}}" -l 'app=vitess,component=vtctld' pods --namespace=$VITESS_NAME
}
...
start_vtctld_forward() {
  ...
  tmpfile=`mktemp`
  $KUBECTL --server=$KUBERENETAPI port-forward -p $pod 0:15999 --namespace=$VITESS_NAME &> $tmpfile &
  vtctld_forward_pid=$!
