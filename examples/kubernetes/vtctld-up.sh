...
cat vtctld-service-template.yaml | sed -e "$sed_script" | $KUBECTL --server=$KUBERENETAPI create --namespace=$VITESS_NAME -f -
...
cat vtctld-controller-template.yaml | sed -e "$sed_script" | $KUBECTL --server=$KUBERENETAPI create --namespace=$VITESS_NAME -f -
