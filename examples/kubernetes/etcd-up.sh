...
for cell in 'global' $cells; do
  ...
  cat etcd-service-template.yaml | \
    sed -e "s/{{cell}}/$cell/g" | \
    $KUBECTL --server=$KUBERENETAPI create --namespace=$VITESS_NAME -f -
  ...
  cat etcd-controller-template.yaml | sed -e "$sed_script" | $KUBECTL --server=$KUBERENETAPI create --namespace=$VITESS_NAME -f -
done
