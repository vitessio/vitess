...
$KUBECTL --server=$KUBERENETAPI create --namespace=$VITESS_NAME configmap orchestrator-conf --from-file="${script_root}/../../docker/orchestrator/orchestrator.conf.json"

cat orchestrator-template.yaml | sed -e "$sed_script" | $KUBECTL --server=$KUBERENETAPI create --namespace=$VITESS_NAME -f -
