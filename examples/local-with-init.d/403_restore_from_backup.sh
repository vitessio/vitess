...
for shard in "${SHARDS[@]}"; do  # Loop through each shard defined earlier
    ...
    REPLICA_TABLETS=$(vtctldclient --server=localhost:15999  GetTablets --keyspace="$KEYSPACE" --shard="$shard" --tablet-type=replica | awk '{print $1}')  # Extract the first column containing tablet names
    ...
    vtctldclient --server=localhost:15999 RestoreFromBackup "$RESTORE_TABLET" || fail "Restore failed for tablet $RESTORE_TABLET"  # Attempt to restore from backup and log an error message if it fails
done
