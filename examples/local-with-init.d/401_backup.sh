...
for shard in "${SHARDS[@]}"; do
    ...
    # TODO: The following line is failing with an error:
    # TabletManager.Backup on zone1-0000000302: unable to get backup storage: no registered implementation of BackupStorage
    vtctldclient --server=localhost:15999 BackupShard "$KEYSPACE/$shard" || fail "Backup failed for shard $shard."
