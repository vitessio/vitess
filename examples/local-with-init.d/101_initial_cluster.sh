...
# The goal of this script is to use a configuration file (/etc/vitess.yaml).
# So we deliberately do not source the common env.sh file, or use aliases.
# Unfortunately this makes using vtcldclient a little bit more verbose
# because we have to add --server=localhost:15999, but this could be improved
# in future by having vtctlclient explicitly default to reading /etc/vitess.yaml.

# Copy the example configuration file to /etc/vitess.yaml
# or ~/.vitess.yaml. You may need to adjust to use sudo cp here.
cp ../common/config/vitess.yaml.101-example ~/.vitess.yaml
...
if vtctldclient --server=localhost:15999 GetKeyspace commerce > /dev/null 2>&1 ; then
...
# create the schema
vtctldclient --server=localhost:15999 ApplySchema --sql-file create_commerce_schema.sql commerce || fail "Failed to apply schema for the commerce keyspace"

# create the vschema
vtctldclient --server=localhost:15999 ApplyVSchema --vschema-file vschema_commerce_initial.json commerce || fail "Failed to apply vschema for the commerce keyspace"
...
echo "Access vtadmin at: http://localhost:14201"