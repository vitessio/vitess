...
A Priary Vindex uniqueness does not require the column to be unique. You can have multiple rows in the database that have the same primary vindex value. You can also have multiple rows in the database that have different column values, but map to the same keyspace ID. The Vindex uniqueness constraint is only used to make sure that all rows of a keyspace ID live in the same shard.
