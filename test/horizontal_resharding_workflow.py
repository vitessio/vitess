...
class TestHorizontalReshardingWorkflow(worker.TestBaseSplitClone):
  ...
  def test_successful_resharding(self):
    ...
    utils.run_vtctl(['WorkflowWait', hw_uuid])
  ...
  def verify(self):
    ...
    all_tablets = [worker.shard_rdonly1.tablet_alias, 
                   worker.shard_0_rdonly1.tablet_alias, 
                   worker.shard_1_rdonly1.tablet_alias]
    for tablet_alias in all_tablets:
      utils.run_vtctl(['RunHealthCheck', tablet_alias], auto_log=True)
    ...
    # dest shard -80, 80-: query service must be disabled after MigrateServedTypes.
