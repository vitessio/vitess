...
class TestNocache(framework.TestCase):
  ...
  def test_data(self):
    ...
    self.assertEqual(
        cu.description,
        [("intval", 3), ("floatval", 4), ("charval", 253), ("binval", 253)])
  ...
  def test_binary(self):
    ...
    self.env.execute(
        "insert into vtocc_test values(5, null, null, :bindata)", bvar)
  ...
  def test_simple_read(self):
    ...
    self.assertEqual(
        vstart.mget("Queries.TotalCount", 0) + 1,
        vend.Queries.TotalCount)
    self.assertEqual(
        vstart.mget("Queries.Histograms.PASS_SELECT.Count", 0) + 1,
        vend.Queries.Histograms.PASS_SELECT.Count)

  def test_nocache_list_arg(self):
    cu = self.env.execute(
        "select * from vtocc_test where intval in ::list",
        {"list": field_types.List([2, 3, 4])})
    self.assertEqual(cu.rowcount, 2)
    cu = self.env.execute(
        "select * from vtocc_test where intval in ::list",
  ...
  def test_commit(self):
    ...
    self.assertEqual(
        vstart.mget("Transactions.TotalCount", 0)+ 2,
        vend.Transactions.TotalCount)
    self.assertEqual(
        vstart.mget("Transactions.Histograms.Completed.Count", 0) + 2,
        vend.Transactions.Histograms.Completed.Count)
    self.assertEqual(
        vstart.mget("Queries.TotalCount", 0) + 8, vend.Queries.TotalCount)
    self.assertEqual(
        vstart.mget("Queries.Histograms.BEGIN.Count", 0) + 2,
        vend.Queries.Histograms.BEGIN.Count)
    self.assertEqual(
        vstart.mget("Queries.Histograms.COMMIT.Count", 0) + 2,
        vend.Queries.Histograms.COMMIT.Count)
    self.assertEqual(
        vstart.mget("Queries.Histograms.INSERT_PK.Count", 0) + 1,
        vend.Queries.Histograms.INSERT_PK.Count)
    self.assertEqual(
        vstart.mget("Queries.Histograms.DML_PK.Count", 0) + 1,
        vend.Queries.Histograms.DML_PK.Count)
    self.assertEqual(
        vstart.mget("Queries.Histograms.PASS_SELECT.Count", 0) + 2,
        vend.Queries.Histograms.PASS_SELECT.Count)
  ...
  def test_integrity_error(self):
    ...
    vend = self.env.debug_vars()
    self.assertEqual(
        vstart.mget("InfoErrors.DupKey", 0) + 1, vend.InfoErrors.DupKey)
  ...
  def test_rollback(self):
    ...
    self.assertEqual(
        txlog[5], "insert into vtocc_test values(4, null, null, null)")
    cu = self.env.execute("select * from vtocc_test")
    self.assertEqual(cu.rowcount, 4)
    vend = self.env.debug_vars()
    self.assertEqual(
        vstart.mget("Transactions.TotalCount", 0) + 1,
        vend.Transactions.TotalCount)
    self.assertEqual(
        vstart.mget("Transactions.Histograms.Aborted.Count", 0) + 1,
        vend.Transactions.Histograms.Aborted.Count)
    self.assertEqual(
        vstart.mget("Queries.Histograms.BEGIN.Count", 0) + 1,
        vend.Queries.Histograms.BEGIN.Count)
    self.assertEqual(
        vstart.mget("Queries.Histograms.ROLLBACK.Count", 0) + 1,
        vend.Queries.Histograms.ROLLBACK.Count)

  def test_nontx_dml(self):
    vstart = self.env.debug_vars()
    results = self.env.execute(
        "insert into vtocc_test values(444, null, null, null)")
    vend = self.env.debug_vars()
  ...
  def test_trailing_comment(self):
    ...
    vend = self.env.debug_vars()
    self.assertEqual(
        vstart.mget("QueryCacheLength", 0) + 1, vend.QueryCacheLength)
    # This should not increase the query cache size
    self.env.execute(
        "select * from vtocc_test where intval=:ival /* trailing comment */",
        bv)
    vend = self.env.debug_vars()
    self.assertEqual(
        vstart.mget("QueryCacheLength", 0) + 1, vend.QueryCacheLength)
  ...
  def test_pass_dml(self):
    ...
    try:
      ...
      self.env.execute(
          ...
          "(8, 2, '', '') on duplicate key update id = 2+1")
      self.env.execute("update vtocc_a set eid = 1+1 where eid = 1 and id = 1")
      self.env.execute("insert into vtocc_d(eid, id) values (1, 1)")
      self.env.execute(
          "update vtocc_a set eid = :eid where eid = 1 and id = 1", {"eid": 3})
    finally:
      self.env.conn.rollback()
  ...
  def test_select_lock(self):
    for lock_mode in ["for update", "lock in share mode"]:
      try:
        self.env.execute(
            "select * from vtocc_test where intval=2 %s" % lock_mode)
  ...
  def test_transaction_timeout(self):
    ...
    self.assertEqual(
        vstart.TransactionPoolAvailable, vmid.TransactionPoolAvailable + 1)
    time.sleep(0.3)
    try:
      self.env.conn.commit()
    except dbexceptions.DatabaseError as e:
    ...
    vend = self.env.debug_vars()
    self.assertEqual(
        vstart.TransactionPoolAvailable, vend.TransactionPoolAvailable)
    self.assertEqual(vend.TransactionPoolTimeout, 250000000)
    self.assertEqual(
        vstart.mget("Kills.Transactions", 0) + 1, vend.Kills.Transactions)
  ...
  def test_max_dml_rows(self):
    ...
    self.assertContains(
        log,
        "update vtocc_a set eid = 2 where (eid = 3 and id = 1) or "
        "(eid = 3 and id = 2) "
    ...
    self.assertContains(
        log,
        "delete from vtocc_a where (eid = 2 and id = 1) or "
        "(eid = 2 and id = 2) /* _stream vtocc_a (eid id ) (2 1 ) (2 2 )")
    self.assertContains(
        log,
