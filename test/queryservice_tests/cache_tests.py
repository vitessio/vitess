...
class TestWillNotBeCached(framework.TestCase):
  ...
  def test_nocache(self):
    self.env.execute(
        "create table vtocc_nocache(eid int, primary key (eid)) "
        "comment 'vtocc_nocache'")
  ...
  def test_charcol(self):
    self.env.execute(
        "create table vtocc_nocache(eid varchar(10), primary key (eid))")
    self.assertContains(self.env.log.read(), "Will not be cached")
...
class TestCache(framework.TestCase):
  ...
  def test_rename(self):
    try:
      ...
      # Verify row cache is working
      self.env.execute(
          "select * from vtocc_renamed where eid = 2 and bid = 'foo'")
  ...
  def test_nodata(self):
    ...
    cu = self.env.execute(
        "select * from vtocc_cached2 where eid = 6 and name = 'bar'")
    self.assertEqual(cu.rowcount, 0)

  def test_types(self):
    self._verify_mismatch(
        "select * from vtocc_cached2 where eid = 'str' and bid = 'str'")
    self._verify_mismatch(
        "select * from vtocc_cached2 where eid = :str and bid = :str",
        {"str": "str"})
    self._verify_mismatch(
        "select * from vtocc_cached2 where eid = 1 and bid = 1")
    self._verify_mismatch(
        "select * from vtocc_cached2 where eid = :id and bid = :id", {"id": 1})
    self._verify_mismatch(
        "select * from vtocc_cached2 where eid = 1.2 and bid = 1.2")
    self._verify_mismatch(
        "select * from vtocc_cached2 where eid = :fl and bid = :fl",
        {"fl": 1.2})

  def test_stats(self):
    self.env.execute(
        "select * from vtocc_cached2 where eid = 2 and bid = 'foo'")
    tstartHits = self._get_vars_table_stats(
        self.env.debug_vars()["RowcacheStats"], "vtocc_cached2", "Hits")
    self.env.execute(
        "select * from vtocc_cached2 where eid = 2 and bid = 'foo'")
    tendHits = self._get_vars_table_stats(
        self.env.debug_vars()["RowcacheStats"], "vtocc_cached2", "Hits")
    ...
    tendMisses = self._get_vars_table_stats(
        self.env.debug_vars()["RowcacheStats"], "vtocc_view", "Misses")
