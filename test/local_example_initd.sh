...
cd "$VTROOT/examples/local-with-init.d"
...
for shard in "customer/0"; do
 while true; do
  if (mysql -h 127.0.0.1 -P 15306 "$shard" -e 'show tables' &>/dev/null); then
    break
  fi
...
$(mysql -h 127.0.0.1 -P 15306 --table < ../common/select_commerce_data.sql &>/dev/null || true)
