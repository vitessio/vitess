...
{{end}}
    ...
    - name: Run cluster endtoend test
      ...
      run: |
        ...
        JUNIT_OUTPUT=report.xml JSON_OUTPUT=report.json go run test.go -docker={{if .Docker}}true -flavor={{.Platform}}{{else}}false{{end}} -follow -shard {{.Shard}}{{if .PartialKeyspace}} -partial-keyspace=true {{end}}{{if .BuildTag}} -build-tag={{.BuildTag}} {{end}}
