...
index by_msg (msg)
  ...
  def _check_startup_values(self):
    ...
    self._check_value(shard_3_master, 'resharding1', 1, 'msg1',
                      0x1000000000000000)
    self._check_value(shard_3_replica, 'resharding1', 1, 'msg1',
                      0x1000000000000000)
    self._check_value(shard_3_rdonly, 'resharding1', 1, 'msg1',
                      0x1000000000000000)
  ...
  def _insert_lots(self, count, base=0):
    for i in xrange(count):
      self._insert_value(shard_0_master, 'resharding1', 10000 + base + i,
                         'msg-range1-%d' % i, 0x2000000000000000 + base + i)
      self._insert_value(shard_1_master, 'resharding1', 20000 + base + i,
                         'msg-range2-%d' % i, 0x6000000000000000 + base + i)
  ...
  def _check_lots(self, count, base=0):
    ...
    for i in xrange(count):
      if self._is_value_present_and_correct(shard_3_replica, 'resharding1',
                                            ...
                                            i, 0x2000000000000000 + base + i):
        found += 1
      if self._is_value_present_and_correct(shard_3_replica, 'resharding1',
                                            20000 + base + i, 'msg-range2-%d' %
                                            i, 0x6000000000000000 + base + i):
        found += 1
  ...
  def test_resharding(self):
    ...
    shard_3_master.init_tablet('master', 'test_keyspace', '-80')
    shard_3_replica.init_tablet('replica', 'test_keyspace', '-80')
    shard_3_rdonly.init_tablet('rdonly', 'test_keyspace', '-80')

    # start vttablet on the split shards (no db created,
    # so they're all not serving)
    for t in [shard_3_master, shard_3_replica, shard_3_rdonly]:
      t.start_vttablet(wait_for_state=None)
    for t in [shard_3_master, shard_3_replica, shard_3_rdonly]:
    ...
    shard_3_master.wait_for_binlog_player_count(2)
    ...
    utils.run_vtctl(['RunHealthCheck', shard_3_rdonly.tablet_alias, 'rdonly'])
    ...
    shard_3_master_status = shard_3_master.get_status()
    self.assertIn('Binlog player state: Running', shard_3_master_status)
    self.assertIn(
        '<td><b>All</b>: 4000<br><b>Query</b>: 3000<br>'
        '<b>Transaction</b>: 1000<br></td>', shard_3_master_status)
    self.assertIn('</html>', shard_3_master_status)
    ...
    shard_3_master_status = shard_3_master.get_status()
    self.assertIn('No binlog player is running', shard_3_master_status)
    self.assertIn('</html>', shard_3_master_status)
